...
var DualApiService = class _DualApiService {
  ...
  authenticateCypherView() {
    ...
    return this.http.post(authUrl, {
      ...
      console.log("\u2705 CypherView authentication successful");
      return token;
    }), catchError((error) => {
      console.error("\u274C CypherView authentication failed:", error);
      throw error;
    }));
  }
  /**
  ...
  authenticateKeyApi() {
    ...
    console.log("\u{1F510} Attempting Key API authentication...");
    console.log("Auth URL:", authUrl);
    console.log("Username:", this.config.keyApiAuth.username);
    console.log("Password length:", this.config.keyApiAuth.password?.length || 0);
    return this.http.post(authUrl, {
      username: this.config.keyApiAuth.username,
      password: this.config.keyApiAuth.password
    }).pipe(map((response) => {
      console.log("\u{1F510} Key API auth response:", response);
      const token = response.accessToken ?? response.idToken;
      if (isNullOrUndefined(token)) {
        throw new Error("No access token received from Key API");
      }
      this.keyApiToken = token;
      console.log("\u2705 Key API authentication successful");
      console.log("\u{1F510} Token length:", token.length);
      console.log("\u{1F510} Token preview:", token.substring(0, 50) + "...");
      console.log("\u{1F510} Token type:", typeof token);
      return token;
    }), catchError((error) => {
      console.error("\u274C Key API authentication failed:", error);
  ...
  getCypherViewTelemetry(vehicleId, startDate, endDate, limit) {
    console.log("\u{1F504} Fetching CypherView telemetry...");
    ...
    console.log("\u{1F504} CypherView telemetry URL:", url);
    console.log("\u{1F504} CypherView telemetry params:", params);
    return this.http.get(url, { params }).pipe(map((response) => {
      console.log("\u2705 CypherView telemetry received:", response.items?.length || 0, "records");
  ...
  searchKeyApiVehicleByDeviceName(deviceName) {
    console.log("\u{1F50D} Searching Key API for device name:", deviceName);
    const authObs = isDefined(this.keyApiToken) ? (console.log("\u{1F50D} Reusing existing Key API token for search"), of(this.keyApiToken)) : (console.log("\u{1F50D} Authenticating to Key API for search"), this.authenticateKeyApi());
    return authObs.pipe(switchMap((token) => {
      ...
      console.log("\u{1F50D} Searching vehicles with URL:", url);
      console.log("\u{1F50D} Search params:", params);
      return this.http.get(url, { headers, params });
    }), map((response) => {
      console.log("\u{1F50D} Key API vehicle search response:", response);
      if (isNullOrUndefined(response["items"]) || !Array.isArray(response["items"])) {
        console.warn("\u26A0\uFE0F No vehicles found in Key API search response");
        return null;
      }
      const matchingVehicle = response["items"].find((vehicle) => {
        const vehicleDeviceName = vehicle.device?.name ?? vehicle.name ?? "";
        console.log("\u{1F50D} Checking vehicle:", vehicleDeviceName, "vs target:", deviceName);
        return vehicleDeviceName === deviceName;
      });
      if (isDefined(matchingVehicle)) {
        console.log("\u2705 Found matching vehicle in Key API:", matchingVehicle);
        return matchingVehicle.id ?? null;
      } else {
        console.warn("\u26A0\uFE0F No matching vehicle found for device name:", deviceName);
        console.log("Available vehicles:", response["items"].map((v) => ({
          id: v.id,
          name: v.name,
      ...
      console.error("\u274C Key API vehicle search error:", error);
  ...
  getKeyApiTelemetry(cypherViewVehicleId, startDate, endDate, limit, deviceName) {
    console.log("\u{1F504} Fetching Key API telemetry...");
    console.log("\u{1F504} CypherView Vehicle ID:", cypherViewVehicleId);
    console.log("\u{1F504} Device Name for fallback:", deviceName);
    return this.fetchKeyApiTelemetryData(cypherViewVehicleId, startDate, endDate, limit).pipe(catchError((error) => {
      if ((error.status === 404 || error.status === 401) && isDefined(deviceName)) {
        ...
        return this.searchKeyApiVehicleByDeviceName(deviceName).pipe(switchMap((keyVehicleId) => {
          ...
          console.log("\u2705 Found Key API vehicle ID:", keyVehicleId, "for device:", deviceName);
          return this.fetchKeyApiTelemetryData(keyVehicleId, startDate, endDate, limit);
        }), catchError((searchError) => {
          console.error("\u274C Device name mapping also failed:", searchError);
          return of([]);
        }));
      } else {
  ...
  fetchKeyApiTelemetryData(keyVehicleId, startDate, endDate, limit) {
    const authObs = isDefined(this.keyApiToken) ? (console.log("\u{1F504} Reusing existing Key API token"), of(this.keyApiToken)) : (console.log("\u{1F504} Authenticating to Key API"), this.authenticateKeyApi());
    return authObs.pipe(switchMap((token) => {
      ...
      console.log("\u{1F504} Fetching telemetry from Key API:");
      console.log("  URL:", url);
      ...
      console.log("\u2705 Key API telemetry received:", response.items?.length || 0, "records");
      return response.items ?? [];
    }), catchError((error) => {
      console.error("\u274C Key API telemetry error:", error);
  ...
  getKeyApiEvents(cypherViewVehicleId, startDate, endDate, limit, deviceName) {
    console.log("\u{1F504} Fetching Key API events...");
    console.log("\u{1F504} CypherView Vehicle ID:", cypherViewVehicleId);
    console.log("\u{1F504} Device Name for fallback:", deviceName);
    return this.fetchKeyApiEventsData(cypherViewVehicleId, startDate, endDate, limit).pipe(catchError((error) => {
      if ((error.status === 404 || error.status === 401) && isDefined(deviceName)) {
        ...
        return this.searchKeyApiVehicleByDeviceName(deviceName).pipe(switchMap((keyVehicleId) => {
          ...
          console.log("\u2705 Found Key API vehicle ID for events:", keyVehicleId, "for device:", deviceName);
  ...
  fetchKeyApiEventsData(keyVehicleId, startDate, endDate, limit) {
    const authObs = isDefined(this.keyApiToken) ? (console.log("\u{1F504} Reusing existing Key API token for events"), of(this.keyApiToken)) : (console.log("\u{1F504} Authenticating to Key API for events"), this.authenticateKeyApi());
    return authObs.pipe(switchMap((token) => {
      ...
      console.log("\u{1F504} Fetching events from Key API:");
      console.log("  URL:", url);
      console.log("  Vehicle ID:", keyVehicleId);
      console.log("  Params:", params);
      return this.http.get(url, { headers, params });
    }), map((response) => {
      console.log("\u2705 Key API events received:", response.items?.length || 0, "records");
      return response.items ?? [];
    }), catchError((error) => {
      console.error("\u274C Key API events error:", error);
  ...
  getKeyApiTrips(cypherViewVehicleId, startDate, endDate, limit, deviceName) {
    console.log("\u{1F504} Fetching Key API trips...");
    console.log("\u{1F504} CypherView Vehicle ID:", cypherViewVehicleId);
    console.log("\u{1F504} Device Name for fallback:", deviceName);
    return this.fetchKeyApiTripsData(cypherViewVehicleId, startDate, endDate, limit).pipe(catchError((error) => {
      if ((error.status === 404 || error.status === 401) && isDefined(deviceName)) {
        ...
        return this.searchKeyApiVehicleByDeviceName(deviceName).pipe(switchMap((keyVehicleId) => {
          ...
          console.log("\u2705 Found Key API vehicle ID for trips:", keyVehicleId, "for device:", deviceName);
  ...
  fetchKeyApiTripsData(keyVehicleId, startDate, endDate, limit) {
    const authObs = isDefined(this.keyApiToken) ? (console.log("\u{1F504} Reusing existing Key API token for trips"), of(this.keyApiToken)) : (console.log("\u{1F504} Authenticating to Key API for trips"), this.authenticateKeyApi());
    return authObs.pipe(switchMap((token) => {
      ...
      console.log("\u{1F504} Fetching trips from Key API:");
      console.log("  URL:", url);
      console.log("  Vehicle ID:", keyVehicleId);
      console.log("  Params:", params);
      return this.http.get(url, { headers, params });
    }), map((response) => {
      console.log("\u2705 Key API trips received:", response.items?.length || 0, "records");
      return response.items ?? [];
    }), catchError((error) => {
      console.error("\u274C Key API trips error:", error);
...
var TelemetryComparisonService = class _TelemetryComparisonService {
  ...
  getTelemetryComparison(vehicleId, startDate, endDate, limit, deviceName) {
    console.log("\u{1F680} Starting telemetry comparison for:", {
      vehicleId,
      startDate,
      endDate,
      limit,
...
var EventsComparisonService = class _EventsComparisonService {
  ...
  getEventsComparison(vehicleId, startDate, endDate, limit, deviceName) {
    console.log("\u{1F680} Starting events comparison for:", {
      vehicleId,
      startDate,
      endDate,
      limit,
...
var TripsComparisonService = class _TripsComparisonService {
  ...
  getTripsComparison(vehicleId, startDate, endDate, limit, deviceName) {
    console.log("\u{1F680} Starting trips comparison for:", {
      vehicleId,
      startDate,
      endDate,
      limit,
...
var UnifiedTimelineService = class _UnifiedTimelineService {
  ...
  getUnifiedTimeline(vehicleId, startDate, endDate, limit, deviceName) {
    console.log("\u{1F680} Starting unified timeline analysis for:", {
      vehicleId,
      startDate,
      endDate,
      limit,
    ...
    return forkJoin({
      ...
      console.log("\u{1F4CA} All data fetched:", {
      ...
      console.log("\u{1F4CA} Timeline records after merge:", {
        total: unifiedRecords.length,
        telemetry: unifiedRecords.filter((r) => r.type === "telemetry").length,
        events: unifiedRecords.filter((r) => r.type === "event").length,
        trip_starts: unifiedRecords.filter((r) => r.type === "trip_start").length,
        trip_ends: unifiedRecords.filter((r) => r.type === "trip_end").length
      });
      console.log("\u{1F550} First 10 timestamps:", unifiedRecords.slice(0, 10).map((r) => ({
        type: r.type,
...
var TelemetryComparisonComponent = class _TelemetryComparisonComponent {
  ...
  clients = signal([]);
  assets = signal([]);
  filteredClients = signal([]);
  filteredAssets = signal([]);
  selectedClient = signal(null);
  selectedAsset = signal(null);
  loadingClients = signal(false);
  loadingAssets = signal(false);
  errorMessage = signal(null);
  currentOwnerId = signal(null);
  // Date/time range signals
  startDate = signal(null);
  selectedTimeRange = signal(null);
  ...
  unifiedTimeline = signal([]);
  unifiedSummary = signal(null);
  isAnalyzing = signal(false);
  analysisComplete = signal(false);
  expandedRecords = signal(/* @__PURE__ */ new Set());
  // Legacy telemetry-only comparison (for fallback)
  alignedRecords = signal([]);
  comparisonSummary = signal(null);
    ...
    if (timeRange && this.startDate()) {
      ...
      if (currentStartDate) {
        ...
        if (endDate > now) {
          ...
          console.log("\u{1F4C5} Auto-adjusted start date from", currentStartDate.toISOString(), "to", adjustedStartDate.toISOString());
  ...
  getDeviceNameFromAsset(asset) {
    ...
    if (!assetData?.devices || !Array.isArray(assetData.devices) || assetData.devices.length === 0) {
      console.warn("\u26A0\uFE0F No devices found for asset:", asset.label);
      return void 0;
    }
  ...
  onAnalyzeTelemetry() {
    ...
    if (this.isReadyForAnalysis() && startDate && endDate && isDefined(asset?.value)) {
      console.log("\u{1F680} Analyzing unified timeline for:");
      ...
      console.log("\u{1F50D} Extracted device name:", deviceName);
      console.log("\u{1F4C5} Date conversion for API:");
      console.log("  User selected start (local):", startDate.toString());
      console.log("  User selected end (local):", endDate.toString());
      console.log("  Timezone offset (minutes):", startDate.getTimezoneOffset());
      ...
      this.unifiedTimelineService.getUnifiedTimeline(
        ...
        next: (result) => {
          console.log("\u{1F389} Unified timeline result:", result);
          console.log("\u{1F4CA} Timeline summary:", result.summary);
          console.log("\u{1F4C8} Total records:", result.records.length);
          console.log("\u{1F50D} Record types:", result.records.reduce((acc, r) => {
            acc[r.type] = (acc[r.type] || 0) + 1;
            return acc;
          }, {}));
          const cypherViewRecords = result.records.filter((r) => r.testRecord);
          ...
          console.log(`\u2705 Found ${totalCypherViewData} CypherView records (${cypherViewRecords.length} matched with Key)`);
        ...
        error: (error) => {
          console.error("\u274C Error analyzing unified timeline:", error);
...
// src/app/features/development/development-routing.module.ts
var routes = [
  {
    path: "",
...
var DevelopmentRoutingModule = class _DevelopmentRoutingModule {
  static \u0275fac = function DevelopmentRoutingModule_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _DevelopmentRoutingModule)();
  };
  static \u0275mod = /* @__PURE__ */ \u0275\u0275defineNgModule({ type: _DevelopmentRoutingModule });
  static \u0275inj = /* @__PURE__ */ \u0275\u0275defineInjector({ imports: [RouterModule.forChild(routes), RouterModule] });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(DevelopmentRoutingModule, [{
    type: NgModule,
    args: [{
...
// src/app/features/development/development.module.ts
var DevelopmentModule = class _DevelopmentModule {
  static \u0275fac = function DevelopmentModule_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _DevelopmentModule)();
  };
  static \u0275mod = /* @__PURE__ */ \u0275\u0275defineNgModule({ type: _DevelopmentModule });
  static \u0275inj = /* @__PURE__ */ \u0275\u0275defineInjector({ imports: [CommonModule, DevelopmentRoutingModule] });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(DevelopmentModule, [{
    type: NgModule,
    args: [{
      declarations: [],
...
//# sourceMappingURL=chunk-NDBHK7XT.js.map
