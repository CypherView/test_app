...
var EntityDetailsLoaderService = class _EntityDetailsLoaderService {
   ...
   * `sourceItem` lets callers preserve list-only fields (e.g. the hand-added
   * `sharedFrom` on assets) that the single-entity GET endpoint doesn't return.
   */
  loadEntityDetails(entityType, entityId, sourceItem) {
    const component = this.detailsFactory.getDetailsComponent(entityType);
    this.loadEntityWithCallbacks(entityType, entityId, (item) => this.handleLoadSuccess(this.mergeListOnlyFields(item, sourceItem), entityType, component), () => this.handleLoadError(entityType), `Failed to load ${entityType} details`);
  }
  /**
   * Merge fields that the list endpoint exposes but the single-entity GET does
   * not (currently just `sharedFrom`). Loaded values win when present.
   */
  mergeListOnlyFields(loaded, source) {
    if (!isDefined(source))
      return loaded;
    const sourceRecord = source;
    const loadedRecord = loaded;
    if (isDefined(sourceRecord["sharedFrom"]) && !isDefined(loadedRecord["sharedFrom"])) {
      return __spreadProps(__spreadValues({}, loaded), { sharedFrom: sourceRecord["sharedFrom"] });
    }
    return loaded;
  }
  /**
...
//# sourceMappingURL=chunk-CMRFU42O.js.map
