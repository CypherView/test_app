import {
  ...
  userProfileResolver
} from "./chunk-D5WZPA6V.js";
import {
  ThemeComponent
} from "./chunk-B3EHJWY7.js";
import {
  NO_RETRY,
  SILENT_ERROR,
  SKIP_AUTH_REFRESH
} from "./chunk-SVLC3WQO.js";
import {
  LayoutService
} from "./chunk-PJ4ITVQU.js";
import {
  VENDOR_ADMIN_FEATURES
} from "./chunk-T4FLB22E.js";
import {
...
import {
  ...
  QAToolsService
} from "./chunk-6ZQ6P7G5.js";
import {
  SplitterModule
} from "./chunk-A656OLHR.js";
import {
  ENTITY_ADAPTER_REGISTRY
} from "./chunk-DPL5CW2J.js";
import {
  VersionUpdateService,
  provideServiceWorker
} from "./chunk-H3FLBCZZ.js";
import {
  AlertsDataAdapter,
  ...
  UsersDataAdapter,
  VendorsDataAdapter
} from "./chunk-6CTNBDDM.js";
import "./chunk-AFE2YUEN.js";
import {
  Menu,
  MenuModule
} from "./chunk-7TLBKVGR.js";
import {
  Tag,
  TagModule
} from "./chunk-YQ3FBPOI.js";
...
import {
  ...
  ScrollPanel,
  ScrollPanelModule
} from "./chunk-T3M2L5QU.js";
import "./chunk-WW5JATKP.js";
import {
  Popover,
  PopoverModule,
  UtilityService
} from "./chunk-TXYEME2R.js";
...
var NavigationService = class _NavigationService {
  ...
  activeRoot = signal("overview", ...ngDevMode ? [{ debugName: "activeRoot" }] : (
    /* istanbul ignore next */
    []
  ));
  pageHistory = signal({}, ...ngDevMode ? [{ debugName: "pageHistory" }] : (
    /* istanbul ignore next */
    []
  ));
  urlHistory = signal([], ...ngDevMode ? [{ debugName: "urlHistory" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  mergedFlagsCache = /* @__PURE__ */ new Map();
  // Signal to track when inheritance has been loaded
  inheritanceLoaded = signal(false, ...ngDevMode ? [{ debugName: "inheritanceLoaded" }] : (
    /* istanbul ignore next */
    []
  ));
  // Tracks which client ID the inheritance cache was last loaded for
  lastInheritanceClientId = null;
  // Computed signals for reactive navigation
  navigationItems = computed(() => this.computeNavigationItems(), ...ngDevMode ? [{ debugName: "navigationItems" }] : (
    /* istanbul ignore next */
    []
  ));
  navigationGroups = computed(() => this.computeNavigationGroups(), ...ngDevMode ? [{ debugName: "navigationGroups" }] : (
    /* istanbul ignore next */
    []
  ));
  previousUrl = computed(() => {
    ...
    return history.at(-2) ?? "";
  }, ...ngDevMode ? [{ debugName: "previousUrl" }] : (
    /* istanbul ignore next */
    []
  ));
  constructor() {
   ...
   */
  loadInheritedFlagsOnInit() {
    const selectedClient = this.clientDataService.getSelectedClient();
    if (!selectedClient || !isDefined(selectedClient.id) || selectedClient.id?.length === 0) {
      this.inheritanceLoaded.set(true);
      return;
    }
    const clientId = selectedClient.id;
    const groupId = selectedClient?.["group"] ?? selectedClient?.["parent"]?.id ?? null;
    this.lastInheritanceClientId = `${clientId}:${groupId}`;
  ...
  setPath(url) {
    ...
    if (isNullOrUndefined(matchedKey)) {
      ...
      matchedPage = isDefined(matchedKey) ? pages[matchedKey] : void 0;
    }
    if (isNullOrUndefined(matchedKey) || !matchedPage)
      return;
    this.pageHistory.update((history) => __spreadProps(__spreadValues({}, history), {
      [matchedKey]: url
    }));
    const children = matchedPage.children ?? (matchedPage.getChildren ? matchedPage.getChildren(this.getFeatures()) : {});
    const childKey = Object.keys(children).find((childKey2) => {
      const child = children[childKey2];
      return isDefined(child.route) && this.routeMatches(url, child.route);
    });
    this.activeRoot.set(isDefined(childKey) ? `${matchedKey}/${childKey}` : matchedKey);
  }
  /**
   * Checks if a URL matches a route pattern
   * Handles query params, fragments, and exact path matching
   */
  routeMatches(url, routePattern) {
  ...
  getFeatures() {
    ...
    const result = cloneDeep_default(clientDataFeatures);
   ...
   */
  getCachedInheritedFlags(_clientId) {
    try {
      ...
      let groupId;
      if ("group" in client && typeof client["group"] === "string" && client["group"]) {
   ...
   */
  mergeInheritedFlags(result, inheritedFlags) {
    Object.entries(inheritedFlags).forEach(([key, value]) => {
      const parts = key.split(".");
      let current = result;
      for (let i29 = 0; i29 < parts.length - 1; i29++) {
        const part = parts[i29];
   ...
   */
  async mergeParentGroupFlags(clientId, clientObject) {
    try {
      ...
      let groupId;
      if (isDefined(client) && "group" in client && typeof client["group"] === "string" && client["group"]) {
        groupId = client["group"];
      } else if (isDefined(client?.["parent"])) {
        const parent = client["parent"];
        if (isDefined(parent?.id) && parent.id !== "root") {
  ...
  async getCompanyGroupMergedFlags(groupId) {
    ...
    while (isDefined(currentGroupId) && currentGroupId.length > 0 && currentGroupId !== DEFAULT_SYSTEM_ID && !visited.has(currentGroupId)) {
      visited.add(currentGroupId);
      try {
        const group = await this.cacheService.getOrFetchAsync(`company-group-${currentGroupId}`, () => firstValueFrom(this.accountsService.getCompanyGroup(currentGroupId)), 15e3);
        groups.push(group);
        const parentId = group.parent?.id;
        currentGroupId = isDefined(parentId) && parentId.length > 0 && parentId !== "root" ? parentId : void 0;
      } catch (error) {
    ...
    groups.forEach((g6) => {
      Object.entries(g6?.flags?.[APP_FEATURE_FLAGS] ?? {}).forEach(([key, value]) => {
  ...
  getPages() {
    ...
    return {
      ...
      appstore: {
        ...
        route: "app-store",
        group: (features) => {
          return isEmpty(features.page?.["moreapps"]?.group) ? "admin" : features.page?.["moreapps"]?.group ?? "admin";
        },
        visible: (features) => {
          return features.page?.["moreapps"]?.enabled ?? false;
        },
        order: (features) => {
      ...
      "telemetry-comparison": {
        ...
        group: "development",
        visible: () => {
          return !environment.production && this.apiSchemaService.usesModernSchema();
        },
        order: 1e3
  ...
  getExternalApplicationChildren(features) {
    ...
    for (const tab of externalTabs.split("\n")) {
      ...
      if (separatorIndex === -1)
        continue;
      const rawUrl = tab.substring(separatorIndex + 1).trim();
      ...
      if (!section) {
        ...
        children[tabName] = __spreadProps(__spreadValues({
          id,
          name: name.trim(),
          icon: finalIcon,
          group: "",
        ...
        }, isDefined(appStoreAppId) ? { appStoreAppId } : {}), {
          external: true
        });
      }
    }
  ...
  computeNavigationItems(group) {
    ...
    const items = [];
    const externalTabs = features.page?.["external"]?.tabs ?? "";
    if (isDefined(externalTabs) && isDefined(features.page?.["external"]?.enabled) && (features.page?.["external"]?.enabled ?? false)) {
      for (const tab of externalTabs.split("\n")) {
        if (!isDefined(tab) || !tab.trim())
          continue;
        const separatorIndex = tab.indexOf(":");
        if (separatorIndex === -1) {
        ...
        try {
          ...
          if (isDefined(appId)) {
            appStoreAppId = appId;
            parsedUrl.searchParams.delete("appId");
        ...
        if (group == null || pageGroup === group) {
          ...
          const navItem = {
            ...
            children: Object.keys(children).length > 0 ? Object.keys(children).map((childKey) => ({
              ...
              appStoreAppId: children[childKey].appStoreAppId
            })) : void 0
          };
  ...
  computeNavigationGroups() {
    ...
    const groupOrder = ["status", "history", "admin", "development"];
    regularGroups.sort((a43, b5) => {
      const aIndex = groupOrder.indexOf(a43.id);
      const bIndex = groupOrder.indexOf(b5.id);
  ...
  getGroupDisplayName(groupId) {
    const keyMap = {
      status: "NAV.GROUPS.STATUS",
      history: "NAV.GROUPS.HISTORY",
      admin: "NAV.GROUPS.ADMIN",
      development: "NAV.GROUPS.DEVELOPMENT"
    };
    const key = keyMap[groupId];
    return key ? this.translate.instant(key) : groupId;
  }
...
function isAllowedByFlags(features, flag, requireAll) {
  ...
  if (noFlagSpecified)
    return true;
  const evaluate = (path) => Boolean(get_default(features, path));
...
var ClientSwitcherComponent = class _ClientSwitcherComponent {
  ...
  onClosed = output();
  display = signal(true, ...ngDevMode ? [{ debugName: "display" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  setupClientsStream() {
    ...
    combineLatest([search$, offset$]).pipe(takeUntilDestroyed(this.destroyRef), tap(() => this.loading.set(true)), switchMap(([searchTerm, offset]) => {
      const userProfile = this.authDataService.getUserProfile();
      if (!isDefined(userProfile?.owner?.id)) {
        return of(null);
      }
      const ownerId = userProfile.owner.type === "client" ? this.clientDataService.getSelectedClient()?.id : userProfile.owner.id;
      ...
      return this.clientsService.listClients(ownerId, offset, DEFAULT_PAGE_SIZE, "name:asc", rqlFilter).pipe(catchError((error) => {
    ...
    this.clientsService.getClient(clientId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (response) => {
        ...
        if (isDefined(response.name)) {
          this.recentClientsService.addRecentClient(response.id, response.name);
        }
        this.closeDialog();
      },
      error: (error) => {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _ClientSwitcherComponent, selectors: [["app-client-switcher"]], outputs: { onClosed: "onClosed" }, decls: 2, vars: 11, consts: [["header", "Select Client", "styleClass", "w-[90vw] max-w-[800px]", 3, "visibleChange", "onHide", "visible", "modal", "maximizable", "contentStyleClass"], [3, "clientSelected", "searchQuery", "lazyLoad", "clients", "loading", "useServerSearch", "totalRecords", "enablePagination", "enableLazyLoad", "recentClients"]], template: function ClientSwitcherComponent_Template(rf, ctx) {
    if (rf & 1) {
      ...
      \u0275\u0275listener("visibleChange", function ClientSwitcherComponent_Template_p_dialog_visibleChange_0_listener($event) {
        return ctx.display.set($event);
      })("onHide", function ClientSwitcherComponent_Template_p_dialog_onHide_0_listener() {
        return ctx.closeDialog();
      });
      \u0275\u0275elementStart(1, "app-client-selection-form", 1);
      \u0275\u0275listener("clientSelected", function ClientSwitcherComponent_Template_app_client_selection_form_clientSelected_1_listener($event) {
        return ctx.selectClient($event);
      })("searchQuery", function ClientSwitcherComponent_Template_app_client_selection_form_searchQuery_1_listener($event) {
        return ctx.onClientSearch($event);
      })("lazyLoad", function ClientSwitcherComponent_Template_app_client_selection_form_lazyLoad_1_listener($event) {
        return ctx.onLazyLoadClients($event);
      });
      \u0275\u0275elementEnd()();
    }
    if (rf & 2) {
      \u0275\u0275property("visible", ctx.display())("modal", true)("maximizable", false)("contentStyleClass", "max-h-[70vh] overflow-visible p-6");
      \u0275\u0275advance();
      \u0275\u0275property("clients", ctx.clients())("loading", ctx.loading())("useServerSearch", ctx.useServerSearch())("totalRecords", ctx.totalRecords())("enablePagination", ctx.useServerSearch())("enableLazyLoad", ctx.useServerSearch())("recentClients", ctx.recentClients());
    }
  }, dependencies: [DialogModule, Dialog, ClientSelectionFormComponent], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ClientSwitcherComponent, [{
    type: Component,
    args: [{ selector: "app-client-switcher", imports: [DialogModule, ClientSelectionFormComponent], template: `<p-dialog
...
` }]
  }], () => [], { onClosed: [{ type: Output, args: ["onClosed"] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ClientSwitcherComponent, { className: "ClientSwitcherComponent", filePath: "src/app/features/clients/components/client-switcher/client-switcher.component.ts", lineNumber: 42 });
})();

// src/app/layouts/components/sidebar/menu-item/small-caps.pipe.ts
var SmallCapsPipe = class _SmallCapsPipe {
  transform(value) {
  ...
  static \u0275fac = function SmallCapsPipe_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _SmallCapsPipe)();
  };
  static \u0275pipe = /* @__PURE__ */ \u0275\u0275definePipe({ name: "smallCaps", type: _SmallCapsPipe, pure: true });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(SmallCapsPipe, [{
    type: Pipe,
    args: [{
      name: "smallCaps"
    }]
...
var _c02 = (a0) => [a0, "!text-2xl flex items-center justify-center"];
...
function MenuItemComponent_Conditional_3_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    let tmp_2_0;
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275property("ngClass", \u0275\u0275pureFunction1(2, _c02, (tmp_1_0 = ctx_r0.item()) == null ? null : tmp_1_0["icon"]))("pTooltip", (tmp_2_0 = ctx_r0.item()) == null ? null : tmp_2_0["label"]);
  }
}
function MenuItemComponent_Conditional_4_For_2_Conditional_0_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "span", 5);
  }
}
function MenuItemComponent_Conditional_4_For_2_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275textInterpolate(segment_r2.letter);
  }
}
function MenuItemComponent_Conditional_4_For_2_Conditional_2_Template(rf, ctx) {
  if (rf & 1) {
...
function MenuItemComponent_Conditional_4_For_2_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275conditionalCreate(0, MenuItemComponent_Conditional_4_For_2_Conditional_0_Template, 1, 0, "span", 5)(1, MenuItemComponent_Conditional_4_For_2_Conditional_1_Template, 2, 1, "span", 6)(2, MenuItemComponent_Conditional_4_For_2_Conditional_2_Template, 2, 1, "span", 7);
  }
  if (rf & 2) {
    const segment_r2 = ctx.$implicit;
    \u0275\u0275conditional(segment_r2.isSpace ? 0 : segment_r2.isCapital ? 1 : 2);
  }
}
function MenuItemComponent_Conditional_4_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span", 4);
    \u0275\u0275repeaterCreate(1, MenuItemComponent_Conditional_4_For_2_Template, 3, 1, null, null, \u0275\u0275repeaterTrackByIndex);
    \u0275\u0275pipe(3, "smallCaps");
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    ...
    let tmp_3_0;
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275property("pTooltip", (tmp_1_0 = ctx_r0.item()) == null ? null : tmp_1_0["label"])("showDelay", 1e3);
    \u0275\u0275advance();
    \u0275\u0275repeater(\u0275\u0275pipeBind1(3, 2, (tmp_3_0 = ctx_r0.item()) == null ? null : tmp_3_0["label"]));
  }
}
var MenuItemComponent = class _MenuItemComponent {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _MenuItemComponent, selectors: [["app-menu-item"]], inputs: { item: [1, "item"], isCollapsed: [1, "isCollapsed"] }, decls: 5, vars: 5, consts: [["routerLinkActive", "p-menuitem-link-active", "pRipple", "", 1, "menu-item-link", "flex", "items-center", "p-menu-item-link", 3, "click", "routerLink"], [1, "custom-icon-svg", 3, "pTooltip"], [1, "!text-2xl", "flex", "items-center", "justify-center", 3, "icon", "pTooltip"], [3, "ngClass", "pTooltip"], ["tooltipPosition", "right", 1, "small-caps-text", "ml-2", "truncate", "flex-1", "min-w-0", 3, "pTooltip", "showDelay"], [1, "ml-0.5"], [1, "text-xl", "leading-none"], [1, "text-sm", "leading-none"]], template: function MenuItemComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "a", 0);
      \u0275\u0275listener("click", function MenuItemComponent_Template_a_click_0_listener($event) {
      ...
      \u0275\u0275conditionalCreate(4, MenuItemComponent_Conditional_4_Template, 4, 4, "span", 4);
      \u0275\u0275elementEnd();
    }
    if (rf & 2) {
      let tmp_1_0;
      ...
      \u0275\u0275property("routerLink", ((tmp_1_0 = ctx.item()) == null ? null : tmp_1_0["routerLink"]) ?? ((tmp_1_0 = ctx.item()) == null ? null : tmp_1_0["route"]));
      \u0275\u0275advance();
...
`, styles: ["/* src/app/layouts/components/sidebar/menu-item/menu-item.component.scss */\n.custom-icon-svg {\n  width: 1.5rem;\n  height: 1.5rem;\n  fill: currentColor;\n  transition: fill 0.2s ease;\n}\n:host-context(.p-menuitem-link-active) .custom-icon-svg {\n  fill: var(--primary-color);\n}\n.small-caps-text {\n  font-weight: var(--navigation-font-weight);\n  letter-spacing: 0.05em;\n}\n.menu-item-link {\n  position: relative;\n  transition: background-color 0.2s ease;\n}\n.menu-item-link:hover {\n  background-color: rgba(var(--color-primary-rgb, 66, 133, 244), 0.1);\n}\n/*# sourceMappingURL=menu-item.component.css.map */\n"] }]
  }], null, { item: [{ type: Input, args: [{ isSignal: true, alias: "item", required: false }] }], isCollapsed: [{ type: Input, args: [{ isSignal: true, alias: "isCollapsed", required: false }] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(MenuItemComponent, { className: "MenuItemComponent", filePath: "src/app/layouts/components/sidebar/menu-item/menu-item.component.ts", lineNumber: 21 });
})();

// src/app/layouts/components/sidebar/toggle-button/toggle-button.component.ts
var _c03 = (a0, a1) => ({ "text-right": a0, "text-center": a1 });
var _c1 = (a0, a1) => ({ "pi-chevron-left": a0, "pi-chevron-right": a1 });
var ToggleButtonComponent = class _ToggleButtonComponent {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _ToggleButtonComponent, selectors: [["app-toggle-button"]], inputs: { isCollapsed: [1, "isCollapsed"] }, outputs: { toggleChange: "toggleChange" }, decls: 3, vars: 11, consts: [[1, "block", "p-2", "w-full", "text-nav-client-font-color", "cursor-pointer", 3, "click", "keydown.enter", "pTooltip", "ngClass"], [1, "pi", 3, "ngClass"]], template: function ToggleButtonComponent_Template(rf, ctx) {
    if (rf & 1) {
    ...
    if (rf & 2) {
      \u0275\u0275property("pTooltip", \u0275\u0275pipeBind1(1, 3, ctx.isCollapsed() ? "NAV.SIDEBAR.TOGGLE.EXPAND" : "NAV.SIDEBAR.TOGGLE.COLLAPSE"))("ngClass", \u0275\u0275pureFunction2(5, _c03, !ctx.isCollapsed(), ctx.isCollapsed()));
      \u0275\u0275advance(2);
      \u0275\u0275property("ngClass", \u0275\u0275pureFunction2(8, _c1, !ctx.isCollapsed(), ctx.isCollapsed()));
    }
  }, dependencies: [CommonModule, NgClass, TooltipModule, Tooltip, TranslatePipe], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ToggleButtonComponent, [{
    type: Component,
    args: [{ selector: "app-toggle-button", imports: [CommonModule, TooltipModule, TranslatePipe], template: `<span
  (click)="onToggle()"
  (keydown.enter)="onToggle()"
  [pTooltip]="
    (isCollapsed()
      ? 'NAV.SIDEBAR.TOGGLE.EXPAND'
      : 'NAV.SIDEBAR.TOGGLE.COLLAPSE'
    ) | translate
  "
...
` }]
  }], null, { isCollapsed: [{ type: Input, args: [{ isSignal: true, alias: "isCollapsed", required: false }] }], toggleChange: [{ type: Output, args: ["toggleChange"] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ToggleButtonComponent, { className: "ToggleButtonComponent", filePath: "src/app/layouts/components/sidebar/toggle-button/toggle-button.component.ts", lineNumber: 12 });
})();
...
function UserProfileComponent_Conditional_1_Conditional_4_Template(rf, ctx) {
  if (rf & 1) {
...
function UserProfileComponent_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(3);
    \u0275\u0275conditionalCreate(4, UserProfileComponent_Conditional_1_Conditional_4_Template, 2, 3, "div", 5);
    \u0275\u0275domElementEnd();
  }
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275advance();
    \u0275\u0275domProperty("src", ctx_r0.clientLogo(), \u0275\u0275sanitizeUrl);
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate1(" ", ctx_r0.clientName(), " ");
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r0.showApiUrl ? 4 : -1);
  }
}
var UserProfileComponent = class _UserProfileComponent {
  apiUrlService = inject(ApiUrlService);
  showApiUrl = environment.showApiDropdown;
  isCollapsed = input(false, ...ngDevMode ? [{ debugName: "isCollapsed" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  clientLogo = input("", ...ngDevMode ? [{ debugName: "clientLogo" }] : (
    /* istanbul ignore next */
    []
  ));
  clientName = input("", ...ngDevMode ? [{ debugName: "clientName" }] : (
    /* istanbul ignore next */
    []
  ));
  currentApiUrl() {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _UserProfileComponent, selectors: [["app-user-profile"]], inputs: { isCollapsed: [1, "isCollapsed"], userProfile: [1, "userProfile"], clientLogo: [1, "clientLogo"], clientName: [1, "clientName"] }, decls: 2, vars: 1, consts: [[1, "flex", "items-center", "justify-center", "gap-1", "px-2", "py-2", "user-profile"], [1, "w-10", "h-10", "rounded-full", "bg-gray-500", "text-white", "flex", "items-center", "justify-center"], [1, "flex", "items-center", "justify-center", "gap-1", "sidebar-brand"], ["alt", "", 3, "src"], [1, "user-profile", "text-center", "text-ellipsis", "overflow-hidden", "p-2"], [3, "class"]], template: function UserProfileComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275conditionalCreate(0, UserProfileComponent_Conditional_0_Template, 2, 1, "div", 0)(1, UserProfileComponent_Conditional_1_Template, 5, 3);
    }
    if (rf & 2) {
...
(() => {
  ...
  }], null, { isCollapsed: [{ type: Input, args: [{ isSignal: true, alias: "isCollapsed", required: false }] }], userProfile: [{ type: Input, args: [{ isSignal: true, alias: "userProfile", required: false }] }], clientLogo: [{ type: Input, args: [{ isSignal: true, alias: "clientLogo", required: false }] }], clientName: [{ type: Input, args: [{ isSignal: true, alias: "clientName", required: false }] }] });
})();
(() => {
...
function AccountSwitcherComponent_ng_template_2_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span", 6);
    \u0275\u0275text(1);
    \u0275\u0275pipe(2, "translate");
  ...
  if (rf & 2) {
    \u0275\u0275advance();
    \u0275\u0275textInterpolate(\u0275\u0275pipeBind1(2, 1, "AUTH.CONTEXT_SELECTION.STEP_SELECT_ACCOUNT"));
  }
}
function AccountSwitcherComponent_Conditional_5_Template(rf, ctx) {
  ...
  if (rf & 2) {
    \u0275\u0275advance();
    \u0275\u0275property("ariaLabel", \u0275\u0275pipeBind1(2, 2, "SHARED.COMMON.STATUS.LOADING"));
    \u0275\u0275advance(3);
    \u0275\u0275textInterpolate(\u0275\u0275pipeBind1(5, 4, "AUTH.CONTEXT_SELECTION.SWITCHING_ACCOUNT"));
  }
}
function AccountSwitcherComponent_Conditional_6_Template(rf, ctx) {
  if (rf & 1) {
    const _r1 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "app-client-selector", 8);
    \u0275\u0275listener("accountSelected", function AccountSwitcherComponent_Conditional_6_Template_app_client_selector_accountSelected_0_listener($event) {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.onAccountSelected($event.id));
    });
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
...
function AccountSwitcherComponent_ng_template_7_Template(rf, ctx) {
  if (rf & 1) {
    const _r3 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "p-button", 9);
    \u0275\u0275pipe(1, "translate");
    \u0275\u0275listener("onClick", function AccountSwitcherComponent_ng_template_7_Template_p_button_onClick_0_listener() {
      \u0275\u0275restoreView(_r3);
      const ctx_r1 = \u0275\u0275nextContext();
  ...
  if (rf & 2) {
    \u0275\u0275property("label", \u0275\u0275pipeBind1(1, 2, "SHARED.COMMON.BUTTONS.CANCEL"))("text", true);
  }
}
var AccountSwitcherComponent = class _AccountSwitcherComponent {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _AccountSwitcherComponent, selectors: [["app-account-switcher"]], outputs: { closeModal: "closeModal" }, decls: 9, vars: 8, consts: [["header", ""], ["footer", ""], ["styleClass", "w-[90vw] md:w-[50vw] sm:w-[75vw]", 3, "visibleChange", "onHide", "header", "visible", "modal", "closable", "closeOnEscape"], [1, "my-3"], [1, "flex", "flex-col", "items-center", "justify-center", "gap-3"], [3, "clientAccounts", "maxHeightClass"], [1, "font-bold", "whitespace-nowrap"], [3, "ariaLabel"], [3, "accountSelected", "clientAccounts", "maxHeightClass"], ["severity", "secondary", 3, "onClick", "label", "text"]], template: function AccountSwitcherComponent_Template(rf, ctx) {
    if (rf & 1) {
      ...
      \u0275\u0275template(2, AccountSwitcherComponent_ng_template_2_Template, 3, 3, "ng-template", null, 0, \u0275\u0275templateRefExtractor);
      \u0275\u0275elementStart(4, "div", 3);
      \u0275\u0275conditionalCreate(5, AccountSwitcherComponent_Conditional_5_Template, 6, 6, "div", 4)(6, AccountSwitcherComponent_Conditional_6_Template, 1, 2, "app-client-selector", 5);
      \u0275\u0275elementEnd();
      \u0275\u0275template(7, AccountSwitcherComponent_ng_template_7_Template, 2, 4, "ng-template", null, 1, \u0275\u0275templateRefExtractor);
      \u0275\u0275elementEnd();
    }
    if (rf & 2) {
      \u0275\u0275property("header", \u0275\u0275pipeBind1(1, 6, "AUTH.CONTEXT_SELECTION.STEP_SELECT_ACCOUNT"))("visible", ctx.display())("modal", true)("closable", !ctx.loading())("closeOnEscape", !ctx.loading());
      \u0275\u0275advance(5);
...
(() => {
  ...
  [header]="'AUTH.CONTEXT_SELECTION.STEP_SELECT_ACCOUNT' | translate"
  [visible]="display()"
  (visibleChange)="display.set($event)"
  (onHide)="closeDialog()"
  [modal]="true"
...
>
  <ng-template #header>
    <span class="font-bold whitespace-nowrap">{{
      'AUTH.CONTEXT_SELECTION.STEP_SELECT_ACCOUNT' | translate
    }}</span>
  </ng-template>
  ...
  <div class="my-3">
    @if (loading()) {
      <div class="flex flex-col items-center justify-center gap-3">
        <p-progress-spinner
          [ariaLabel]="'SHARED.COMMON.STATUS.LOADING' | translate"
        />
        <p>{{ 'AUTH.CONTEXT_SELECTION.SWITCHING_ACCOUNT' | translate }}</p>
      </div>
    } @else {
      <app-client-selector
...
` }]
  }], null, { closeModal: [{ type: Output, args: ["closeModal"] }] });
})();
(() => {
...
function LanguageSwitcherComponent_Conditional_2_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span", 6);
    \u0275\u0275text(1);
    \u0275\u0275elementEnd();
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275textInterpolate(ctx_r2.currentLanguageLabel());
  }
}
function LanguageSwitcherComponent_For_7_Conditional_3_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "span", 10);
  }
}
function LanguageSwitcherComponent_For_7_Template(rf, ctx) {
  if (rf & 1) {
    const _r4 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "button", 8);
    \u0275\u0275listener("click", function LanguageSwitcherComponent_For_7_Template_button_click_0_listener() {
      const lang_r5 = \u0275\u0275restoreView(_r4).$implicit;
      const ctx_r2 = \u0275\u0275nextContext();
      const overlay_r2 = \u0275\u0275reference(4);
      ctx_r2.selectLanguage(lang_r5.value);
      return \u0275\u0275resetView(overlay_r2.hide());
    });
    \u0275\u0275elementStart(1, "span", 9);
    \u0275\u0275text(2);
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(3, LanguageSwitcherComponent_For_7_Conditional_3_Template, 1, 0, "span", 10);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const lang_r5 = ctx.$implicit;
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275classProp("active", ctx_r2.selectedLanguage() === lang_r5.value);
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate(lang_r5.label);
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r2.selectedLanguage() === lang_r5.value ? 3 : -1);
  }
}
var LanguageSwitcherComponent = class _LanguageSwitcherComponent {
  ...
  currentLanguageLabel = computed(() => this.languages.find((l18) => l18.value === this.selectedLanguage())?.label ?? "Language", ...ngDevMode ? [{ debugName: "currentLanguageLabel" }] : (
    /* istanbul ignore next */
    []
  ));
  selectLanguage(lang) {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _LanguageSwitcherComponent, selectors: [["app-language-switcher"]], inputs: { isCollapsed: [1, "isCollapsed"] }, decls: 8, vars: 4, consts: [["overlay", ""], ["type", "button", "tooltipPosition", "right", 1, "w-full", "flex", "items-center", "p-menu-item-link", "language-trigger", 3, "click", "pTooltip"], [1, "pi", "pi-globe", "!text-2xl", "flex", "items-center", "justify-center"], ["appendTo", "body"], [1, "language-options"], ["type", "button", 1, "language-option", "w-full", "flex", "items-center", "gap-3", "px-3", "py-2", 3, "active"], [1, "small-caps-text", "ml-2"], [1, "pi", "pi-angle-up", "ml-auto", "text-sm", "opacity-60"], ["type", "button", 1, "language-option", "w-full", "flex", "items-center", "gap-3", "px-3", "py-2", 3, "click"], [1, "flex-1", "text-left"], [1, "pi", "pi-check", "text-sm"]], template: function LanguageSwitcherComponent_Template(rf, ctx) {
    if (rf & 1) {
      const _r1 = \u0275\u0275getCurrentView();
      \u0275\u0275elementStart(0, "button", 1);
      \u0275\u0275listener("click", function LanguageSwitcherComponent_Template_button_click_0_listener($event) {
        \u0275\u0275restoreView(_r1);
        const overlay_r2 = \u0275\u0275reference(4);
        return \u0275\u0275resetView(overlay_r2.toggle($event));
      });
      \u0275\u0275element(1, "span", 2);
      \u0275\u0275conditionalCreate(2, LanguageSwitcherComponent_Conditional_2_Template, 3, 1);
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(3, "p-popover", 3, 0)(5, "div", 4);
      \u0275\u0275repeaterCreate(6, LanguageSwitcherComponent_For_7_Template, 4, 4, "button", 5, _forTrack0);
      \u0275\u0275elementEnd()();
    }
    if (rf & 2) {
      \u0275\u0275classProp("justify-center", ctx.isCollapsed());
      \u0275\u0275property("pTooltip", ctx.isCollapsed() ? ctx.currentLanguageLabel() : "");
      \u0275\u0275advance(2);
      \u0275\u0275conditional(!ctx.isCollapsed() ? 2 : -1);
      \u0275\u0275advance(4);
      \u0275\u0275repeater(ctx.languages);
    }
  }, dependencies: [PopoverModule, Popover, TooltipModule, Tooltip], styles: ["\n.language-trigger[_ngcontent-%COMP%] {\n  color: var(--navigation-font-color);\n  font-size: var(--navigation-font-size);\n  font-weight: var(--navigation-font-weight);\n  text-transform: uppercase;\n  transition: background-color 0.2s ease;\n  border: none;\n  background: transparent;\n  cursor: pointer;\n}\n.language-trigger[_ngcontent-%COMP%]:hover {\n  background-color: rgba(var(--color-primary-rgb, 66, 133, 244), 0.1);\n}\n.language-trigger[_ngcontent-%COMP%]   .small-caps-text[_ngcontent-%COMP%] {\n  letter-spacing: 0.05em;\n}\n.language-options[_ngcontent-%COMP%] {\n  min-width: 10rem;\n}\n.language-options[_ngcontent-%COMP%]   .language-option[_ngcontent-%COMP%] {\n  border: none;\n  background: transparent;\n  cursor: pointer;\n  border-radius: 4px;\n  transition: background-color 0.15s ease;\n  color: var(--p-text-color);\n  font-size: 0.875rem;\n}\n.language-options[_ngcontent-%COMP%]   .language-option[_ngcontent-%COMP%]:hover {\n  background-color: var(--p-content-hover-background);\n}\n.language-options[_ngcontent-%COMP%]   .language-option.active[_ngcontent-%COMP%] {\n  color: var(--p-primary-color);\n  font-weight: 600;\n}\n/*# sourceMappingURL=language-switcher.component.css.map */"] });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(LanguageSwitcherComponent, [{
    type: Component,
    args: [{ selector: "app-language-switcher", imports: [PopoverModule, TooltipModule], template: `<button
  type="button"
  class="w-full flex items-center p-menu-item-link language-trigger"
  [class.justify-center]="isCollapsed()"
  [pTooltip]="isCollapsed() ? currentLanguageLabel() : ''"
  tooltipPosition="right"
  (click)="overlay.toggle($event)"
>
  <span class="pi pi-globe !text-2xl flex items-center justify-center"></span>
  @if (!isCollapsed()) {
    <span class="small-caps-text ml-2">{{ currentLanguageLabel() }}</span>
    <span class="pi pi-angle-up ml-auto text-sm opacity-60"></span>
  }
</button>
...
<p-popover #overlay appendTo="body">
  <div class="language-options">
    @for (lang of languages; track lang.value) {
      <button
        type="button"
        class="language-option w-full flex items-center gap-3 px-3 py-2"
        [class.active]="selectedLanguage() === lang.value"
        (click)="selectLanguage(lang.value); overlay.hide()"
      >
...
`, styles: ["/* src/app/shared/components/language-switcher/language-switcher.component.scss */\n.language-trigger {\n  color: var(--navigation-font-color);\n  font-size: var(--navigation-font-size);\n  font-weight: var(--navigation-font-weight);\n  text-transform: uppercase;\n  transition: background-color 0.2s ease;\n  border: none;\n  background: transparent;\n  cursor: pointer;\n}\n.language-trigger:hover {\n  background-color: rgba(var(--color-primary-rgb, 66, 133, 244), 0.1);\n}\n.language-trigger .small-caps-text {\n  letter-spacing: 0.05em;\n}\n.language-options {\n  min-width: 10rem;\n}\n.language-options .language-option {\n  border: none;\n  background: transparent;\n  cursor: pointer;\n  border-radius: 4px;\n  transition: background-color 0.15s ease;\n  color: var(--p-text-color);\n  font-size: 0.875rem;\n}\n.language-options .language-option:hover {\n  background-color: var(--p-content-hover-background);\n}\n.language-options .language-option.active {\n  color: var(--p-primary-color);\n  font-weight: 600;\n}\n/*# sourceMappingURL=language-switcher.component.css.map */\n"] }]
  }], null, { isCollapsed: [{ type: Input, args: [{ isSignal: true, alias: "isCollapsed", required: false }] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(LanguageSwitcherComponent, { className: "LanguageSwitcherComponent", filePath: "src/app/shared/components/language-switcher/language-switcher.component.ts", lineNumber: 17 });
})();

// src/app/shared/utils/dummy-data-generators.ts
var SmartPayloadBuilder = class {
  /**
   * Generates a dummy object based on a list of field names.
   * If field names are not available, it returns a generic object.
   */
  static generateForFields(fields) {
    const payload = {};
    fields.forEach((field) => {
      payload[field] = this.getValueForField(field);
    });
    return payload;
  }
  /**
   * Heuristic logic to determine the value of a field based on its name.
   */
  static getValueForField(name) {
    ...
    if (lowered === "notify" || lowered === "notifications" || lowered === "actions" || lowered === "contacts" || lowered === "layers" || lowered === "overlays") {
      return [];
    }
    if (lowered === "costcentre" || lowered === "costcenter" || lowered === "category" || lowered === "subcategory" || lowered === "parent" || lowered === "deviceprovidertype" || lowered === "type" || lowered === "user") {
      return {
        id: "3fa85f64-5717-4562-b3fc-2c953f66afa6",
        name: "Test IdName"
      };
    }
    if (lowered === "state") {
    ...
    if (lowered === "id" || lowered.endsWith("id")) {
      return "3fa85f64-5717-4562-b3fc-2c953f66afa6";
    }
    if (lowered.includes("name") || lowered.includes("title")) {
      return `Test ${name.charAt(0).toUpperCase() + name.slice(1)}`;
    }
    if (lowered.includes("email")) {
      return "test@example.com";
    }
    if (lowered.includes("description") || lowered.includes("comment") || lowered.includes("note")) {
      return "This is a test generated payload for verification purposes.";
    }
    if (lowered.includes("amount") || lowered.includes("count") || lowered.includes("price") || lowered.includes("total") || lowered.includes("index")) {
      return 100;
    }
    if (lowered.startsWith("is") || lowered.startsWith("has") || lowered.startsWith("should") || lowered.includes("active") || lowered.includes("enabled") || lowered.includes("public")) {
      return true;
    }
    if (lowered.includes("date") || lowered.includes("time") || lowered.endsWith("at")) {
      return (/* @__PURE__ */ new Date()).toISOString();
    }
    if (lowered.includes("status") || lowered.includes("type")) {
      return "ACTIVE";
    }
  ...
  /**
   * Helper to get common dummy objects for specific well-known endpoints
   * until the backend can provide property lists for all mappings.
   */
  static getPayloadForEndpoint(endpoint, method) {
    if (method.toUpperCase() === "GET" || method.toUpperCase() === "DELETE") {
      return null;
    }
    if (endpoint.includes("/assets")) {
      return this.generateForFields([
        "name",
        "description",
        "categoryId",
        "ownerId"
    ...
    if (endpoint.includes("/clients") || endpoint.includes("/vendors")) {
      return this.generateForFields([
        "name",
        "email",
        "phoneNumber",
    ...
    return { name: "Test Entity", description: "Generated by QA Tool" };
  }
  /**
   * Generates a contract-aware payload using the API simulation registry.
   */
  static generateFromRegistry(entry, operation, providedOwnerId) {
    if (operation === "delete") {
      return null;
    }
    const fields = operation === "create" ? entry.createFields : entry.updateFields;
    const payload = this.generateForFields(fields);
    if (operation === "create" && entry.requiresOwnerId && providedOwnerId !== void 0 && providedOwnerId !== "") {
      payload["ownerId"] = providedOwnerId;
    }
    if (operation === "update" && payload["ownerId"] !== void 0) {
...
// src/app/core/services/api-verification.service.ts
var ApiVerificationService = class _ApiVerificationService {
  http = inject(HttpClient);
  authData = inject(AuthDataService);
  apiConfig = inject(Configuration);
  /**
   * Fetches the canonical permission mapping from the backend.
   */
  async getPermissionMapping() {
    try {
      const url = `${this.apiConfig.basePath}/permissions/mapping`;
      return await firstValueFrom(this.http.get(url, {
        context: new HttpContext().set(SILENT_ERROR, true)
      }));
    } catch (error) {
      console.error("Failed to fetch permission mapping:", error);
      return [
        {
          method: "GET",
          endpoint: "/entities/assets",
        ...
        { method: "GET", endpoint: "/accounts/users", permission: "user:list" }
      ];
    }
  }
  /**
   ...
   */
  async verifyBackEndEnforcement(targetOwnerId, targetOwnerType) {
    const ownerId = targetOwnerId ?? this.authData.getUserOwnerId();
    const ownerType = targetOwnerType ?? this.authData.getUserOwnerType();
    ...
    for (const entry of mapping) {
      const result = await this.executeTestCase(entry, ownerId);
      results.push(result);
    }
    return results;
  }
  /**
   * Executes a single API verification test case.
   */
  async executeTestCase(mapping, ownerId) {
    const { method, endpoint, permission } = mapping;
    const processedEndpoint = endpoint.replace("{id}", ownerId).replace(":id", ownerId).replace("{ownerId}", ownerId);
    const url = `${this.apiConfig.basePath}${processedEndpoint.startsWith("/") ? "" : "/"}${processedEndpoint}`;
    const body = SmartPayloadBuilder.getPayloadForEndpoint(endpoint, method);
    const headers = new HttpHeaders({
      "x-auth-testing": "true"
    });
    const context = new HttpContext().set(SILENT_ERROR, true);
    try {
      const options = { headers, context };
      switch (method.toUpperCase()) {
        case "GET": {
          const separator = url.includes("?") ? "&" : "?";
          const getUrl = `${url}${separator}limit=1&owner=${ownerId}`;
          await firstValueFrom(this.http.get(getUrl, options));
          break;
        }
        case "POST":
          ...
          break;
        case "PUT":
          await firstValueFrom(this.http.put(url, body, options));
          break;
        case "DELETE":
          await firstValueFrom(this.http.delete(url, options));
          break;
        case "PATCH":
          await firstValueFrom(this.http.patch(url, body, options));
          break;
        default:
      ...
      return {
        ...
        method,
        permission,
        frontendExpected: this.authData.hasRealPermission(permission),
      ...
      return {
        ...
        permission,
        frontendExpected: this.authData.hasRealPermission(permission),
        backendAccess: isForbidden ? "Forbidden" : "Error",
        errorDetails: (error.statusText ?? "") || (error.message ?? "") || "Unknown Error",
        requestBody: body
      };
    }
  }
  static \u0275fac = function ApiVerificationService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _ApiVerificationService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ApiVerificationService, factory: _ApiVerificationService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ApiVerificationService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
// src/app/shared/components/qa-permission-tester/components/qa-test-mode-banner/qa-test-mode-banner.component.ts
var QaTestModeBannerComponent = class _QaTestModeBannerComponent {
  activeProfileName = input.required(...ngDevMode ? [{ debugName: "activeProfileName" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  save = output();
  resetPermissions = output();
  static \u0275fac = function QaTestModeBannerComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _QaTestModeBannerComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _QaTestModeBannerComponent, selectors: [["app-qa-test-mode-banner"]], inputs: { activeProfileName: [1, "activeProfileName"], isSaving: [1, "isSaving"] }, outputs: { save: "save", resetPermissions: "resetPermissions" }, decls: 15, vars: 2, consts: [[1, "flex", "items-center", "justify-between", "p-4", "bg-blue-600", "dark:bg-blue-800", "rounded-xl", "shadow-lg", "border", "border-blue-500/50"], [1, "flex", "items-center", "gap-4"], [1, "bg-white/20", "p-2", "rounded-lg", "backdrop-blur-sm"], [1, "pi", "pi-info-circle", "text-xl"], [1, "text-[10px]", "font-black", "uppercase", "tracking-widest", "opacity-80", "decoration-white/30", "underline", "underline-offset-4", "mb-0.5"], [1, "flex", "items-center", "gap-2"], [1, "font-bold", "text-sm", "tracking-tight"], [1, "px-2", "py-0.5", "bg-white", "text-blue-700", "text-xs", "font-black", "rounded", "uppercase", "shadow-sm"], [1, "flex", "gap-2"], ["label", "Save", "icon", "pi pi-save", "severity", "success", "size", "small", 3, "onClick", "loading"], ["label", "Reset", "icon", "pi pi-refresh", "severity", "secondary", "size", "small", 3, "onClick"]], template: function QaTestModeBannerComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "div", 0)(1, "div", 1)(2, "div", 2);
      \u0275\u0275element(3, "i", 3);
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(4, "div")(5, "div", 4);
      \u0275\u0275text(6, " QA Simulation Active ");
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(7, "div", 5)(8, "span", 6);
      \u0275\u0275text(9, "Browsing as:");
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(10, "span", 7);
      \u0275\u0275text(11);
      \u0275\u0275elementEnd()()()();
      \u0275\u0275elementStart(12, "div", 8)(13, "p-button", 9);
      \u0275\u0275listener("onClick", function QaTestModeBannerComponent_Template_p_button_onClick_13_listener() {
        return ctx.save.emit();
      });
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(14, "p-button", 10);
      \u0275\u0275listener("onClick", function QaTestModeBannerComponent_Template_p_button_onClick_14_listener() {
        return ctx.resetPermissions.emit();
      });
      \u0275\u0275elementEnd()()();
    }
    if (rf & 2) {
      \u0275\u0275advance(11);
      \u0275\u0275textInterpolate1(" ", ctx.activeProfileName(), " ");
      \u0275\u0275advance(2);
      \u0275\u0275property("loading", ctx.isSaving());
    }
  }, dependencies: [CommonModule, ButtonModule, Button], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(QaTestModeBannerComponent, [{
    type: Component,
    args: [{ selector: "app-qa-test-mode-banner", imports: [CommonModule, ButtonModule], template: '<div\n  class="flex items-center justify-between p-4 bg-blue-600 dark:bg-blue-800 rounded-xl shadow-lg border border-blue-500/50"\n>\n  <div class="flex items-center gap-4">\n    <div class="bg-white/20 p-2 rounded-lg backdrop-blur-sm">\n      <i class="pi pi-info-circle text-xl"></i>\n    </div>\n    <div>\n      <div\n        class="text-[10px] font-black uppercase tracking-widest opacity-80 decoration-white/30 underline underline-offset-4 mb-0.5"\n      >\n        QA Simulation Active\n      </div>\n      <div class="flex items-center gap-2">\n        <span class="font-bold text-sm tracking-tight">Browsing as:</span>\n        <span\n          class="px-2 py-0.5 bg-white text-blue-700 text-xs font-black rounded uppercase shadow-sm"\n        >\n          {{ activeProfileName() }}\n        </span>\n      </div>\n    </div>\n  </div>\n\n  <div class="flex gap-2">\n    <p-button\n      label="Save"\n      icon="pi pi-save"\n      severity="success"\n      (onClick)="save.emit()"\n      [loading]="isSaving()"\n      size="small"\n    ></p-button>\n    <p-button\n      label="Reset"\n      icon="pi pi-refresh"\n      severity="secondary"\n      (onClick)="resetPermissions.emit()"\n      size="small"\n    ></p-button>\n  </div>\n</div>\n' }]
  }], null, { activeProfileName: [{ type: Input, args: [{ isSignal: true, alias: "activeProfileName", required: true }] }], isSaving: [{ type: Input, args: [{ isSignal: true, alias: "isSaving", required: false }] }], save: [{ type: Output, args: ["save"] }], resetPermissions: [{ type: Output, args: ["resetPermissions"] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(QaTestModeBannerComponent, { className: "QaTestModeBannerComponent", filePath: "src/app/shared/components/qa-permission-tester/components/qa-test-mode-banner/qa-test-mode-banner.component.ts", lineNumber: 10 });
})();

// src/app/shared/components/qa-permission-tester/components/qa-persona-selector/qa-persona-selector.component.ts
function QaPersonaSelectorComponent_ng_template_11_Conditional_0_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    const selectedOption_r1 = \u0275\u0275nextContext().$implicit;
    \u0275\u0275advance(3);
    \u0275\u0275textInterpolate(selectedOption_r1.name);
  }
}
function QaPersonaSelectorComponent_ng_template_11_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275conditionalCreate(0, QaPersonaSelectorComponent_ng_template_11_Conditional_0_Template, 4, 1, "div", 16);
  }
  if (rf & 2) {
    const selectedOption_r1 = ctx.$implicit;
    \u0275\u0275conditional(selectedOption_r1 ? 0 : -1);
  }
}
function QaPersonaSelectorComponent_ng_template_12_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate(role_r2.id);
  }
}
var QaPersonaSelectorComponent = class _QaPersonaSelectorComponent {
  roles = input.required(...ngDevMode ? [{ debugName: "roles" }] : (
    /* istanbul ignore next */
    []
  ));
  selectedProfileId = model(null, ...ngDevMode ? [{ debugName: "selectedProfileId" }] : (
    /* istanbul ignore next */
    []
  ));
  apply = output();
  getProfileColorClass(color) {
    const colorMap = {
      green: "bg-green-500",
      blue: "bg-blue-500",
  ...
  static \u0275fac = function QaPersonaSelectorComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _QaPersonaSelectorComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _QaPersonaSelectorComponent, selectors: [["app-qa-persona-selector"]], inputs: { roles: [1, "roles"], selectedProfileId: [1, "selectedProfileId"] }, outputs: { selectedProfileId: "selectedProfileIdChange", apply: "apply" }, decls: 21, vars: 6, consts: [["styleClass", "border shadow-sm"], [1, "flex", "flex-col", "gap-4"], [1, "flex", "items-center", "justify-between"], [1, "font-bold", "text-gray-800", "dark:text-gray-200"], [1, "flex", "items-center", "gap-2"], [1, "text-[10px]", "p-1", "bg-gray-100", "dark:bg-gray-800", "text-gray-600", "dark:text-gray-400", "rounded", "font-bold", "uppercase", "tracking-widest"], [1, "flex", "flex-col", "gap-3"], [1, "p-select-wrapper"], ["optionLabel", "name", "optionValue", "id", "placeholder", "Choose a Persona to Simulate", "styleClass", "w-full", 3, "ngModelChange", "onChange", "options", "ngModel"], ["pTemplate", "selectedItem"], ["pTemplate", "item"], [1, "flex", "items-center", "justify-between", "p-3", "rounded-lg", "bg-gray-50", "dark:bg-gray-900", "border", "border-gray-100", "dark:border-gray-800/50"], [1, "flex", "flex-col"], [1, "text-[10px]", "font-black", "uppercase", "tracking-tighter", "text-gray-400"], [1, "text-xs", "font-bold", "text-gray-700", "dark:text-gray-300"], [1, "w-8", "h-8", "rounded-full", "bg-white", "dark:bg-gray-800", "flex", "items-center", "justify-center", "border", "shadow-sm"], [1, "flex", "items-center", "gap-3"], [1, "w-2", "h-2", "rounded-full", "bg-primary", "animate-pulse"], [1, "font-bold", "text-sm"], [1, "flex", "flex-col", "py-1"], [1, "font-bold", "text-sm", "text-gray-800", "dark:text-gray-200"], [1, "text-[10px]", "text-gray-400", "font-mono"]], template: function QaPersonaSelectorComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "p-card", 0)(1, "div", 1)(2, "div", 2)(3, "h3", 3);
      \u0275\u0275text(4, " Persona Selection ");
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(5, "div", 4)(6, "span", 5);
      \u0275\u0275text(7);
      \u0275\u0275elementEnd()()();
      \u0275\u0275elementStart(8, "div", 6)(9, "div", 7)(10, "p-select", 8);
      \u0275\u0275twoWayListener("ngModelChange", function QaPersonaSelectorComponent_Template_p_select_ngModelChange_10_listener($event) {
        \u0275\u0275twoWayBindingSet(ctx.selectedProfileId, $event) || (ctx.selectedProfileId = $event);
        return $event;
      });
      \u0275\u0275listener("onChange", function QaPersonaSelectorComponent_Template_p_select_onChange_10_listener() {
        return ctx.apply.emit();
      });
      \u0275\u0275template(11, QaPersonaSelectorComponent_ng_template_11_Template, 1, 1, "ng-template", 9)(12, QaPersonaSelectorComponent_ng_template_12_Template, 5, 2, "ng-template", 10);
      \u0275\u0275elementEnd()();
      \u0275\u0275elementStart(13, "div", 11)(14, "div", 12)(15, "span", 13);
      \u0275\u0275text(16, "Current Context");
      \u0275\u0275elementEnd();
    ...
    if (rf & 2) {
      \u0275\u0275advance(7);
      \u0275\u0275textInterpolate1(" ", ctx.roles().length, " Roles Available ");
      \u0275\u0275advance(3);
      \u0275\u0275property("options", ctx.roles());
      \u0275\u0275twoWayProperty("ngModel", ctx.selectedProfileId);
      \u0275\u0275advance(8);
      \u0275\u0275textInterpolate1(" ", ctx.selectedProfileId() ? "Simulated Persona" : "Real System Permissions", " ");
      \u0275\u0275advance(2);
      \u0275\u0275classMap(ctx.selectedProfileId() ? "pi pi-user text-primary" : "pi pi-shield text-gray-400");
    }
  }, dependencies: [
    CommonModule,
    FormsModule,
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(QaPersonaSelectorComponent, [{
    type: Component,
    args: [{ selector: "app-qa-persona-selector", imports: [
      CommonModule,
      FormsModule,
      SelectModule,
      ButtonModule,
      CardModule,
      TagModule
    ], template: `<p-card styleClass="border shadow-sm">
  <div class="flex flex-col gap-4">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <span
          class="text-[10px] p-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded font-bold uppercase tracking-widest"
        >
    ...
    <div class="flex flex-col gap-3">
      <div class="p-select-wrapper">
        <p-select
          ...
          optionLabel="name"
          optionValue="id"
          placeholder="Choose a Persona to Simulate"
          styleClass="w-full"
          (onChange)="apply.emit()"
        >
      ...
      <div
        class="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800/50"
      >
        <div class="flex flex-col">
          <span
            class="text-[10px] font-black uppercase tracking-tighter text-gray-400"
        ...
        <div
          class="w-8 h-8 rounded-full bg-white dark:bg-gray-800 flex items-center justify-center border shadow-sm"
        >
          <i
            [class]="
              selectedProfileId()
                ? 'pi pi-user text-primary'
                : 'pi pi-shield text-gray-400'
            "
          ></i>
        </div>
      </div>
...
` }]
  }], null, { roles: [{ type: Input, args: [{ isSignal: true, alias: "roles", required: true }] }], selectedProfileId: [{ type: Input, args: [{ isSignal: true, alias: "selectedProfileId", required: false }] }, { type: Output, args: ["selectedProfileIdChange"] }], apply: [{ type: Output, args: ["apply"] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(QaPersonaSelectorComponent, { className: "QaPersonaSelectorComponent", filePath: "src/app/shared/components/qa-permission-tester/components/qa-persona-selector/qa-persona-selector.component.ts", lineNumber: 23 });
})();

// src/app/shared/components/qa-permission-tester/components/qa-permission-toggles/qa-permission-toggles.component.ts
function QaPermissionTogglesComponent_For_9_Template(rf, ctx) {
  if (rf & 1) {
    const _r1 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "div", 6)(1, "input", 7);
    \u0275\u0275listener("change", function QaPermissionTogglesComponent_For_9_Template_input_change_1_listener($event) {
      const permission_r2 = \u0275\u0275restoreView(_r1).$implicit;
      const ctx_r2 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r2.togglePermission.emit({ permission: permission_r2, enabled: $event.target.checked }));
    });
    \u0275\u0275elementEnd();
  ...
  if (rf & 2) {
    ...
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275advance();
    \u0275\u0275property("id", "perm-" + permission_r2)("checked", ctx_r2.checkPermission()(permission_r2));
    \u0275\u0275advance();
    \u0275\u0275property("for", "perm-" + permission_r2)("pTooltip", permission_r2);
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1(" ", permission_r2, " ");
  }
}
var QaPermissionTogglesComponent = class _QaPermissionTogglesComponent {
  permissions = input.required(...ngDevMode ? [{ debugName: "permissions" }] : (
    /* istanbul ignore next */
    []
  ));
  checkPermission = input.required(...ngDevMode ? [{ debugName: "checkPermission" }] : (
    /* istanbul ignore next */
    []
  ));
  togglePermission = output();
  static \u0275fac = function QaPermissionTogglesComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _QaPermissionTogglesComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _QaPermissionTogglesComponent, selectors: [["app-qa-permission-toggles"]], inputs: { permissions: [1, "permissions"], checkPermission: [1, "checkPermission"] }, outputs: { togglePermission: "togglePermission" }, decls: 10, vars: 1, consts: [["styleClass", "border shadow-sm"], [1, "flex", "flex-col", "gap-3"], [1, "flex", "items-center", "justify-between"], [1, "font-bold", "text-gray-800", "dark:text-gray-200"], [1, "text-[10px]", "p-1", "bg-blue-100", "dark:bg-blue-900", "text-blue-700", "dark:text-blue-300", "rounded", "font-bold", "uppercase", "tracking-widest"], [1, "max-h-60", "overflow-y-auto", "pr-2", "grid", "grid-cols-1", "md:grid-cols-2", "lg:grid-cols-3", "gap-2"], [1, "flex", "items-center", "gap-3", "p-2", "rounded-lg", "border", "border-transparent", "hover:border-gray-100", "dark:hover:border-gray-800", "hover:bg-gray-50", "dark:hover:bg-gray-800/50", "transition-colors", "group"], ["type", "checkbox", 1, "w-4", "h-4", "rounded", "text-primary", "focus:ring-primary", "border-gray-300", "cursor-pointer", 3, "change", "id", "checked"], ["tooltipPosition", "top", 1, "text-xs", "font-medium", "text-gray-700", "dark:text-gray-300", "cursor-pointer", "flex-1", "truncate", 3, "for", "pTooltip"]], template: function QaPermissionTogglesComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "p-card", 0)(1, "div", 1)(2, "div", 2)(3, "h3", 3);
      \u0275\u0275text(4, " Granular Toggles ");
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(5, "span", 4);
      \u0275\u0275text(6);
      \u0275\u0275elementEnd()();
      \u0275\u0275elementStart(7, "div", 5);
      \u0275\u0275repeaterCreate(8, QaPermissionTogglesComponent_For_9_Template, 4, 5, "div", 6, \u0275\u0275repeaterTrackByIdentity);
      \u0275\u0275elementEnd()()();
    }
    if (rf & 2) {
      \u0275\u0275advance(6);
      \u0275\u0275textInterpolate1(" ", ctx.permissions().length, " Available ");
      \u0275\u0275advance(2);
      \u0275\u0275repeater(ctx.permissions());
    }
  }, dependencies: [CommonModule, CardModule, Card, TooltipModule, Tooltip], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(QaPermissionTogglesComponent, [{
    type: Component,
    args: [{ selector: "app-qa-permission-toggles", imports: [CommonModule, CardModule, TooltipModule], template: `<p-card styleClass="border shadow-sm">
  <div class="flex flex-col gap-3">
    <div class="flex items-center justify-between">
      <h3 class="font-bold text-gray-800 dark:text-gray-200">
        Granular Toggles
      ...
      <span
        class="text-[10px] p-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded font-bold uppercase tracking-widest"
      >
        {{ permissions().length }} Available
      </span>
    </div>
    <div
      class="max-h-60 overflow-y-auto pr-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2"
    >
      @for (permission of permissions(); track permission) {
        <div
          class="flex items-center gap-3 p-2 rounded-lg border border-transparent hover:border-gray-100 dark:hover:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group"
        >
          <input
            type="checkbox"
            [id]="'perm-' + permission"
          ...
          <label
            [for]="'perm-' + permission"
            class="text-xs font-medium text-gray-700 dark:text-gray-300 cursor-pointer flex-1 truncate"
            [pTooltip]="permission"
            tooltipPosition="top"
          >
            {{ permission }}
...
` }]
  }], null, { permissions: [{ type: Input, args: [{ isSignal: true, alias: "permissions", required: true }] }], checkPermission: [{ type: Input, args: [{ isSignal: true, alias: "checkPermission", required: true }] }], togglePermission: [{ type: Output, args: ["togglePermission"] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(QaPermissionTogglesComponent, { className: "QaPermissionTogglesComponent", filePath: "src/app/shared/components/qa-permission-tester/components/qa-permission-toggles/qa-permission-toggles.component.ts", lineNumber: 11 });
})();

// src/app/shared/components/qa-permission-tester/components/qa-feature-access-stats/qa-feature-access-stats.component.ts
function QaFeatureAccessStatsComponent_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275domElementStart(0, "div", 1)(1, "div", 4);
    \u0275\u0275text(2, " Client Access ");
    \u0275\u0275domElementEnd();
  ...
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275advance(5);
    \u0275\u0275textInterpolate(ctx_r0.clientStats().accessible);
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate1("/ ", ctx_r0.clientStats().total);
  }
}
function QaFeatureAccessStatsComponent_Conditional_2_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275domElementStart(0, "div", 2)(1, "div", 8);
    \u0275\u0275text(2, " Vendor Access ");
    \u0275\u0275domElementEnd();
    \u0275\u0275domElementStart(3, "div", 5)(4, "span", 9);
    \u0275\u0275text(5);
    \u0275\u0275domElementEnd();
    \u0275\u0275domElementStart(6, "span", 7);
    \u0275\u0275text(7);
  ...
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275advance(5);
    \u0275\u0275textInterpolate(ctx_r0.vendorStats().accessible);
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate1("/ ", ctx_r0.vendorStats().total);
  }
}
function QaFeatureAccessStatsComponent_Conditional_3_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275domElementStart(0, "div", 3)(1, "div", 10);
    \u0275\u0275text(2, " Distributor Access ");
    \u0275\u0275domElementEnd();
    \u0275\u0275domElementStart(3, "div", 5)(4, "span", 11);
    \u0275\u0275text(5);
    \u0275\u0275domElementEnd();
    \u0275\u0275domElementStart(6, "span", 7);
    \u0275\u0275text(7);
  ...
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275advance(5);
    \u0275\u0275textInterpolate(ctx_r0.distributorStats().accessible);
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate1("/ ", ctx_r0.distributorStats().total);
  }
}
var QaFeatureAccessStatsComponent = class _QaFeatureAccessStatsComponent {
  clientStats = input.required(...ngDevMode ? [{ debugName: "clientStats" }] : (
    /* istanbul ignore next */
    []
  ));
  vendorStats = input.required(...ngDevMode ? [{ debugName: "vendorStats" }] : (
    /* istanbul ignore next */
    []
  ));
  distributorStats = input.required(...ngDevMode ? [{ debugName: "distributorStats" }] : (
    /* istanbul ignore next */
    []
  ));
  canViewContext = input.required(...ngDevMode ? [{ debugName: "canViewContext" }] : (
    /* istanbul ignore next */
    []
  ));
  static \u0275fac = function QaFeatureAccessStatsComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _QaFeatureAccessStatsComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _QaFeatureAccessStatsComponent, selectors: [["app-qa-feature-access-stats"]], inputs: { clientStats: [1, "clientStats"], vendorStats: [1, "vendorStats"], distributorStats: [1, "distributorStats"], canViewContext: [1, "canViewContext"] }, decls: 4, vars: 3, consts: [[1, "grid", "grid-cols-1", "md:grid-cols-3", "gap-4"], [1, "p-3", "border", "rounded-lg", "bg-emerald-50/30", "dark:bg-emerald-950/10", "border-emerald-100", "dark:border-emerald-800/50"], [1, "p-3", "border", "rounded-lg", "bg-amber-50/30", "dark:bg-amber-950/10", "border-amber-100", "dark:border-amber-800/50"], [1, "p-3", "border", "rounded-lg", "bg-indigo-50/30", "dark:bg-indigo-950/10", "border-indigo-100", "dark:border-indigo-800/50"], [1, "text-xs", "font-bold", "text-emerald-800", "dark:text-emerald-300", "uppercase", "mb-2"], [1, "flex", "items-baseline", "gap-1"], [1, "text-2xl", "font-black", "text-emerald-600", "dark:text-emerald-400"], [1, "text-xs", "text-gray-400"], [1, "text-xs", "font-bold", "text-amber-800", "dark:text-amber-300", "uppercase", "mb-2"], [1, "text-2xl", "font-black", "text-amber-600", "dark:text-amber-400"], [1, "text-xs", "font-bold", "text-indigo-800", "dark:text-indigo-300", "uppercase", "mb-2"], [1, "text-2xl", "font-black", "text-indigo-600", "dark:text-indigo-400"]], template: function QaFeatureAccessStatsComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275domElementStart(0, "div", 0);
      \u0275\u0275conditionalCreate(1, QaFeatureAccessStatsComponent_Conditional_1_Template, 8, 2, "div", 1);
      \u0275\u0275conditionalCreate(2, QaFeatureAccessStatsComponent_Conditional_2_Template, 8, 2, "div", 2);
      \u0275\u0275conditionalCreate(3, QaFeatureAccessStatsComponent_Conditional_3_Template, 8, 2, "div", 3);
      \u0275\u0275domElementEnd();
    }
    if (rf & 2) {
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.canViewContext()("client") ? 1 : -1);
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.canViewContext()("vendor") ? 2 : -1);
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.canViewContext()("distributor") ? 3 : -1);
    }
  }, dependencies: [CommonModule], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(QaFeatureAccessStatsComponent, [{
    type: Component,
    args: [{ selector: "app-qa-feature-access-stats", imports: [CommonModule], template: `<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
  @if (canViewContext()('client')) {
    <div
      class="p-3 border rounded-lg bg-emerald-50/30 dark:bg-emerald-950/10 border-emerald-100 dark:border-emerald-800/50"
    >
      <div
        class="text-xs font-bold text-emerald-800 dark:text-emerald-300 uppercase mb-2"
      >
        Client Access
      </div>
      <div class="flex items-baseline gap-1">
        <span
          ...
          >{{ clientStats().accessible }}</span
        >
        <span class="text-xs text-gray-400">/ {{ clientStats().total }}</span>
      </div>
    </div>
  }
  @if (canViewContext()('vendor')) {
    <div
      class="p-3 border rounded-lg bg-amber-50/30 dark:bg-amber-950/10 border-amber-100 dark:border-amber-800/50"
    >
      <div
        class="text-xs font-bold text-amber-800 dark:text-amber-300 uppercase mb-2"
      >
        Vendor Access
      </div>
      <div class="flex items-baseline gap-1">
        <span class="text-2xl font-black text-amber-600 dark:text-amber-400">{{
          vendorStats().accessible
        }}</span>
        <span class="text-xs text-gray-400">/ {{ vendorStats().total }}</span>
      </div>
    </div>
  }
  @if (canViewContext()('distributor')) {
    <div
      class="p-3 border rounded-lg bg-indigo-50/30 dark:bg-indigo-950/10 border-indigo-100 dark:border-indigo-800/50"
    >
      <div
        class="text-xs font-bold text-indigo-800 dark:text-indigo-300 uppercase mb-2"
      >
        Distributor Access
      </div>
      <div class="flex items-baseline gap-1">
        <span
          ...
          >{{ distributorStats().accessible }}</span
        >
        <span class="text-xs text-gray-400"
          >/ {{ distributorStats().total }}</span
        >
      </div>
    </div>
  }
...
` }]
  }], null, { clientStats: [{ type: Input, args: [{ isSignal: true, alias: "clientStats", required: true }] }], vendorStats: [{ type: Input, args: [{ isSignal: true, alias: "vendorStats", required: true }] }], distributorStats: [{ type: Input, args: [{ isSignal: true, alias: "distributorStats", required: true }] }], canViewContext: [{ type: Input, args: [{ isSignal: true, alias: "canViewContext", required: true }] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(QaFeatureAccessStatsComponent, { className: "QaFeatureAccessStatsComponent", filePath: "src/app/shared/components/qa-permission-tester/components/qa-feature-access-stats/qa-feature-access-stats.component.ts", lineNumber: 14 });
})();

// src/app/shared/components/qa-permission-tester/components/qa-feature-access-table/qa-feature-access-table.component.ts
var _c04 = () => ({ "min-width": "50rem" });
var _c12 = (a0) => ({ "bg-red-50/30 dark:bg-red-950/10": a0 });
function QaFeatureAccessTableComponent_ng_template_22_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(8, "Required Permissions");
    \u0275\u0275elementEnd()();
  }
}
function QaFeatureAccessTableComponent_ng_template_23_Conditional_10_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275textInterpolate(row_r1.permissions);
  }
}
function QaFeatureAccessTableComponent_ng_template_23_Conditional_11_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span", 26);
    \u0275\u0275text(1, "None required");
    \u0275\u0275elementEnd();
  }
}
function QaFeatureAccessTableComponent_ng_template_23_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "tr", 20)(1, "td")(2, "code", 21);
    \u0275\u0275text(3);
    \u0275\u0275elementEnd()();
    ...
    \u0275\u0275element(8, "p-tag", 24);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(9, "td");
    \u0275\u0275conditionalCreate(10, QaFeatureAccessTableComponent_ng_template_23_Conditional_10_Template, 2, 1, "span", 25)(11, QaFeatureAccessTableComponent_ng_template_23_Conditional_11_Template, 2, 0, "span", 26);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const row_r1 = ctx.$implicit;
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275property("ngClass", \u0275\u0275pureFunction1(7, _c12, !row_r1.hasAccess));
    \u0275\u0275advance(3);
    \u0275\u0275textInterpolate(row_r1.featureId);
    \u0275\u0275advance(3);
    \u0275\u0275textInterpolate(row_r1.label);
    \u0275\u0275advance(2);
    \u0275\u0275property("value", row_r1.hasAccess ? "Accessible" : "Restricted")("severity", ctx_r1.getAccessSeverity(row_r1.hasAccess))("icon", row_r1.hasAccess ? "pi pi-check" : "pi pi-times");
    \u0275\u0275advance(2);
    \u0275\u0275conditional(row_r1.permissions ? 10 : 11);
  }
}
function QaFeatureAccessTableComponent_ng_template_24_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "tr")(1, "td", 27);
    \u0275\u0275text(2, " No features match your search. ");
    \u0275\u0275elementEnd()();
  }
}
var QaFeatureAccessTableComponent = class _QaFeatureAccessTableComponent {
  ...
  get filteredRows() {
    ...
    return this.rows().filter((r85) => r85.featureId.toLowerCase().includes(term) || r85.label.toLowerCase().includes(term) || (r85.permissions ?? "").toLowerCase().includes(term));
  }
  get accessibleCount() {
    return this.rows().filter((r85) => r85.hasAccess).length;
  }
  get restrictedCount() {
    return this.rows().filter((r85) => !r85.hasAccess).length;
  }
   ...
   */
  getAccessSeverity(hasAccess) {
    return hasAccess ? "success" : "danger";
  }
  static \u0275fac = function QaFeatureAccessTableComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _QaFeatureAccessTableComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _QaFeatureAccessTableComponent, selectors: [["app-qa-feature-access-table"]], inputs: { rows: [1, "rows"], context: [1, "context"] }, decls: 25, vars: 7, consts: [[1, "flex", "flex-col", "gap-4", "mt-4"], [1, "flex", "items-center", "justify-between", "border-b", "pb-3", "border-gray-100", "dark:border-gray-700"], [1, "text-base", "font-bold", "flex", "items-center", "gap-2"], [1, "pi", "pi-list", "text-primary"], [1, "text-xs", "text-gray-500", "mt-0.5"], [1, "flex", "items-center", "gap-3"], [1, "inline-flex", "items-center", "gap-1.5", "text-xs"], [1, "w-2", "h-2", "rounded-full", "bg-green-500", "inline-block"], [1, "text-gray-500"], [1, "w-2", "h-2", "rounded-full", "bg-red-500", "inline-block"], [1, "relative"], [1, "pi", "pi-search", "absolute", "left-3", "top-1/2", "-translate-y-1/2", "text-gray-400", "text-sm"], ["type", "text", "pInputText", "", "placeholder", "Search features...", 1, "w-full", "!pl-9", "text-sm", 3, "ngModelChange", "ngModel"], [1, "border", "rounded-xl", "border-gray-100", "dark:border-gray-700", "overflow-hidden"], [1, "p-datatable-sm", "mb-4", 3, "value", "tableStyle"], ["pTemplate", "header"], ["pTemplate", "body"], ["pTemplate", "emptymessage"], ["scope", "col", 1, "col-25"], ["scope", "col", 1, "col-25", "text-center"], [3, "ngClass"], [1, "text-xs"], [1, "text-sm", "font-medium"], [1, "text-center"], [3, "value", "severity", "icon"], [1, "text-xs", "text-gray-600"], [1, "text-[11px]", "text-gray-400", "italic"], ["colspan", "4", 1, "p-6", "text-center", "text-gray-400", "text-sm"]], template: function QaFeatureAccessTableComponent_Template(rf, ctx) {
    if (rf & 1) {
      ...
      \u0275\u0275text(7, " Shows whether the current simulated persona has access to each feature. ");
      \u0275\u0275elementEnd()();
      ...
      \u0275\u0275twoWayListener("ngModelChange", function QaFeatureAccessTableComponent_Template_input_ngModelChange_19_listener($event) {
        \u0275\u0275twoWayBindingSet(ctx.searchTerm, $event) || (ctx.searchTerm = $event);
        return $event;
      });
      \u0275\u0275elementEnd()();
      \u0275\u0275elementStart(20, "div", 13)(21, "p-table", 14);
      \u0275\u0275template(22, QaFeatureAccessTableComponent_ng_template_22_Template, 9, 0, "ng-template", 15)(23, QaFeatureAccessTableComponent_ng_template_23_Template, 12, 9, "ng-template", 16)(24, QaFeatureAccessTableComponent_ng_template_24_Template, 3, 0, "ng-template", 17);
      \u0275\u0275elementEnd()()();
    }
    if (rf & 2) {
      \u0275\u0275advance(5);
      \u0275\u0275textInterpolate1(" ", ctx.context(), " Feature Access Matrix ");
      \u0275\u0275advance(7);
      \u0275\u0275textInterpolate(ctx.accessibleCount);
      \u0275\u0275advance(4);
      \u0275\u0275textInterpolate(ctx.restrictedCount);
      \u0275\u0275advance(3);
      \u0275\u0275twoWayProperty("ngModel", ctx.searchTerm);
      \u0275\u0275advance(2);
      \u0275\u0275property("value", ctx.filteredRows)("tableStyle", \u0275\u0275pureFunction0(6, _c04));
    }
  }, dependencies: [CommonModule, NgClass, TableModule, Table, PrimeTemplate, TagModule, Tag, InputTextModule, InputText, FormsModule, DefaultValueAccessor, NgControlStatus, NgModel], styles: ['\n[_nghost-%COMP%] {\n  display: block;\n  width: 100%;\n  height: 100%;\n}\n.qa-permission-tester[_ngcontent-%COMP%] {\n  max-height: 80vh;\n  overflow-y: auto;\n}\n.qa-permission-toggle-list[_ngcontent-%COMP%] {\n  max-height: 240px;\n  overflow-y: auto;\n  border: 1px solid var(--p-content-border-color);\n  border-radius: 6px;\n  padding: 8px 12px;\n  background-color: var(--p-content-background);\n}\n.col-25[_ngcontent-%COMP%] {\n  width: 25%;\n}\n.col-15[_ngcontent-%COMP%] {\n  width: 15%;\n}\n.col-35[_ngcontent-%COMP%] {\n  width: 35%;\n}\ncode[_ngcontent-%COMP%] {\n  background-color: var(--p-surface-100);\n  color: var(--p-surface-700);\n  padding: 2px 6px;\n  border-radius: 3px;\n  font-family: "Courier New", monospace;\n}\n.dark-theme[_nghost-%COMP%]   code[_ngcontent-%COMP%], .dark-theme   [_nghost-%COMP%]   code[_ngcontent-%COMP%] {\n  background-color: var(--p-surface-800);\n  color: var(--p-surface-200);\n}\n/*# sourceMappingURL=qa-feature-access-table.component.css.map */'] });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(QaFeatureAccessTableComponent, [{
    type: Component,
    args: [{ selector: "app-qa-feature-access-table", imports: [CommonModule, TableModule, TagModule, InputTextModule, FormsModule], template: `<div class="flex flex-col gap-4 mt-4">
  <div
    class="flex items-center justify-between border-b pb-3 border-gray-100 dark:border-gray-700"
  >
    <div>
      <h3 class="text-base font-bold flex items-center gap-2">
        <i class="pi pi-list text-primary"></i>
        {{ context() }} Feature Access Matrix
      </h3>
      <p class="text-xs text-gray-500 mt-0.5">
        Shows whether the current simulated persona has access to each feature.
      </p>
    </div>
    <div class="flex items-center gap-3">
      <span class="inline-flex items-center gap-1.5 text-xs">
        <span class="w-2 h-2 rounded-full bg-green-500 inline-block"></span>
        <span class="text-gray-500">{{ accessibleCount }}</span>
      </span>
      <span class="inline-flex items-center gap-1.5 text-xs">
        <span class="w-2 h-2 rounded-full bg-red-500 inline-block"></span>
        <span class="text-gray-500">{{ restrictedCount }}</span>
      </span>
    </div>
  </div>
  ...
  <div
    class="border rounded-xl border-gray-100 dark:border-gray-700 overflow-hidden"
  >
    <p-table
      [value]="filteredRows"
    ...
    >
      <ng-template pTemplate="header">
        <tr>
          ...
          <th scope="col" class="col-25 text-center">Access</th>
          <th scope="col" class="col-25">Required Permissions</th>
        </tr>
      </ng-template>
      ...
      <ng-template pTemplate="body" let-row>
        <tr
          [ngClass]="{
            'bg-red-50/30 dark:bg-red-950/10': !row.hasAccess,
          }"
        >
          <td>
            <code class="text-xs">{{ row.featureId }}</code>
          </td>
          <td>
          ...
          <td class="text-center">
            <p-tag
              [value]="row.hasAccess ? 'Accessible' : 'Restricted'"
              [severity]="getAccessSeverity(row.hasAccess)"
              [icon]="row.hasAccess ? 'pi pi-check' : 'pi pi-times'"
            ></p-tag>
          </td>
          <td>
            @if (row.permissions) {
              <span class="text-xs text-gray-600">{{ row.permissions }}</span>
            } @else {
...
`, styles: ['/* src/app/shared/components/qa-permission-tester/components/qa-feature-access-table/qa-feature-access-table.component.scss */\n:host {\n  display: block;\n  width: 100%;\n  height: 100%;\n}\n.qa-permission-tester {\n  max-height: 80vh;\n  overflow-y: auto;\n}\n.qa-permission-toggle-list {\n  max-height: 240px;\n  overflow-y: auto;\n  border: 1px solid var(--p-content-border-color);\n  border-radius: 6px;\n  padding: 8px 12px;\n  background-color: var(--p-content-background);\n}\n.col-25 {\n  width: 25%;\n}\n.col-15 {\n  width: 15%;\n}\n.col-35 {\n  width: 35%;\n}\ncode {\n  background-color: var(--p-surface-100);\n  color: var(--p-surface-700);\n  padding: 2px 6px;\n  border-radius: 3px;\n  font-family: "Courier New", monospace;\n}\n:host-context(.dark-theme) code {\n  background-color: var(--p-surface-800);\n  color: var(--p-surface-200);\n}\n/*# sourceMappingURL=qa-feature-access-table.component.css.map */\n'] }]
  }], null, { rows: [{ type: Input, args: [{ isSignal: true, alias: "rows", required: true }] }], context: [{ type: Input, args: [{ isSignal: true, alias: "context", required: true }] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(QaFeatureAccessTableComponent, { className: "QaFeatureAccessTableComponent", filePath: "src/app/shared/components/qa-permission-tester/components/qa-feature-access-table/qa-feature-access-table.component.ts", lineNumber: 21 });
})();

// src/app/shared/components/qa-permission-tester/components/qa-simulation-tab/qa-simulation-tab.component.ts
function QaSimulationTabComponent_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
    const _r1 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "app-qa-test-mode-banner", 10);
    \u0275\u0275listener("save", function QaSimulationTabComponent_Conditional_1_Template_app_qa_test_mode_banner_save_0_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.saveRequested.emit());
    })("resetPermissions", function QaSimulationTabComponent_Conditional_1_Template_app_qa_test_mode_banner_resetPermissions_0_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
  ...
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275property("activeProfileName", ctx_r1.activeProfileName())("isSaving", ctx_r1.isSaving());
  }
}
function QaSimulationTabComponent_Conditional_7_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "app-qa-feature-access-table", 7);
  }
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275property("rows", ctx_r1.clientFeatureAccess());
  }
}
function QaSimulationTabComponent_Conditional_8_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "app-qa-feature-access-table", 8);
  }
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275property("rows", ctx_r1.vendorFeatureAccess());
  }
}
function QaSimulationTabComponent_Conditional_9_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "app-qa-feature-access-table", 9);
  }
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275property("rows", ctx_r1.distributorFeatureAccess());
  }
}
var QaSimulationTabComponent = class _QaSimulationTabComponent {
  // Inputs
  testMode = input.required(...ngDevMode ? [{ debugName: "testMode" }] : (
    /* istanbul ignore next */
    []
  ));
  activeProfileName = input.required(...ngDevMode ? [{ debugName: "activeProfileName" }] : (
  ...
  togglePermissions = input.required(...ngDevMode ? [{ debugName: "togglePermissions" }] : (
    /* istanbul ignore next */
    []
  ));
  checkPermission = input.required(...ngDevMode ? [{ debugName: "checkPermission" }] : (
  ...
  // Feature access rows per tier
  clientFeatureAccess = input([], ...ngDevMode ? [{ debugName: "clientFeatureAccess" }] : (
    /* istanbul ignore next */
    []
  ));
  vendorFeatureAccess = input([], ...ngDevMode ? [{ debugName: "vendorFeatureAccess" }] : (
    /* istanbul ignore next */
    []
  ));
  distributorFeatureAccess = input([], ...ngDevMode ? [{ debugName: "distributorFeatureAccess" }] : (
    /* istanbul ignore next */
    []
  ));
  // Model
  ...
  // Outputs
  saveRequested = output();
  resetRequested = output();
  applyProfileRequested = output();
  togglePermissionRequested = output();
  static \u0275fac = function QaSimulationTabComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _QaSimulationTabComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _QaSimulationTabComponent, selectors: [["app-qa-simulation-tab"]], inputs: { testMode: [1, "testMode"], activeProfileName: [1, "activeProfileName"], isSaving: [1, "isSaving"], roles: [1, "roles"], togglePermissions: [1, "togglePermissions"], checkPermission: [1, "checkPermission"], clientStats: [1, "clientStats"], vendorStats: [1, "vendorStats"], distributorStats: [1, "distributorStats"], canViewContext: [1, "canViewContext"], clientFeatureAccess: [1, "clientFeatureAccess"], vendorFeatureAccess: [1, "vendorFeatureAccess"], distributorFeatureAccess: [1, "distributorFeatureAccess"], selectedProfileId: [1, "selectedProfileId"] }, outputs: { selectedProfileId: "selectedProfileIdChange", saveRequested: "saveRequested", resetRequested: "resetRequested", applyProfileRequested: "applyProfileRequested", togglePermissionRequested: "togglePermissionRequested" }, decls: 10, vars: 12, consts: [[1, "flex", "flex-col", "gap-6", "pt-4"], [3, "activeProfileName", "isSaving"], [1, "grid", "grid-cols-1", "lg:grid-cols-2", "gap-6"], [1, "flex", "flex-col", "gap-6"], [3, "selectedProfileIdChange", "apply", "roles", "selectedProfileId"], [3, "clientStats", "vendorStats", "distributorStats", "canViewContext"], [3, "togglePermission", "permissions", "checkPermission"], ["context", "Client", 3, "rows"], ["context", "Vendor", 3, "rows"], ["context", "Distributor", 3, "rows"], [3, "save", "resetPermissions", "activeProfileName", "isSaving"]], template: function QaSimulationTabComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "div", 0);
      \u0275\u0275conditionalCreate(1, QaSimulationTabComponent_Conditional_1_Template, 1, 2, "app-qa-test-mode-banner", 1);
      \u0275\u0275elementStart(2, "div", 2)(3, "div", 3)(4, "app-qa-persona-selector", 4);
      \u0275\u0275twoWayListener("selectedProfileIdChange", function QaSimulationTabComponent_Template_app_qa_persona_selector_selectedProfileIdChange_4_listener($event) {
        \u0275\u0275twoWayBindingSet(ctx.selectedProfileId, $event) || (ctx.selectedProfileId = $event);
        return $event;
      });
      \u0275\u0275listener("apply", function QaSimulationTabComponent_Template_app_qa_persona_selector_apply_4_listener() {
        return ctx.applyProfileRequested.emit();
      });
      \u0275\u0275elementEnd();
      \u0275\u0275element(5, "app-qa-feature-access-stats", 5);
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(6, "app-qa-permission-toggles", 6);
      \u0275\u0275listener("togglePermission", function QaSimulationTabComponent_Template_app_qa_permission_toggles_togglePermission_6_listener($event) {
        return ctx.togglePermissionRequested.emit($event);
      });
      \u0275\u0275elementEnd()();
      \u0275\u0275conditionalCreate(7, QaSimulationTabComponent_Conditional_7_Template, 1, 1, "app-qa-feature-access-table", 7);
      \u0275\u0275conditionalCreate(8, QaSimulationTabComponent_Conditional_8_Template, 1, 1, "app-qa-feature-access-table", 8);
      \u0275\u0275conditionalCreate(9, QaSimulationTabComponent_Conditional_9_Template, 1, 1, "app-qa-feature-access-table", 9);
      \u0275\u0275elementEnd();
    }
    if (rf & 2) {
      \u0275\u0275advance();
      ...
      \u0275\u0275advance(3);
      \u0275\u0275property("roles", ctx.roles());
      \u0275\u0275twoWayProperty("selectedProfileId", ctx.selectedProfileId);
      \u0275\u0275advance();
      \u0275\u0275property("clientStats", ctx.clientStats())("vendorStats", ctx.vendorStats())("distributorStats", ctx.distributorStats())("canViewContext", ctx.canViewContext());
      \u0275\u0275advance();
      \u0275\u0275property("permissions", ctx.togglePermissions())("checkPermission", ctx.checkPermission());
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.canViewContext()("client") && ctx.clientFeatureAccess().length > 0 ? 7 : -1);
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.canViewContext()("vendor") && ctx.vendorFeatureAccess().length > 0 ? 8 : -1);
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.canViewContext()("distributor") && ctx.distributorFeatureAccess().length > 0 ? 9 : -1);
    }
  }, dependencies: [
    CommonModule,
    QaTestModeBannerComponent,
    QaPersonaSelectorComponent,
    QaPermissionTogglesComponent,
    QaFeatureAccessStatsComponent,
    QaFeatureAccessTableComponent
  ], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(QaSimulationTabComponent, [{
    type: Component,
    args: [{ selector: "app-qa-simulation-tab", imports: [
      CommonModule,
      QaTestModeBannerComponent,
      QaPersonaSelectorComponent,
      QaPermissionTogglesComponent,
      QaFeatureAccessStatsComponent,
      QaFeatureAccessTableComponent
    ], template: `<div class="flex flex-col gap-6 pt-4">
  <!-- Active Profile Banner -->
  @if (testMode()) {
    <app-qa-test-mode-banner
      [activeProfileName]="activeProfileName()"
      [isSaving]="isSaving()"
      (save)="saveRequested.emit()"
      (resetPermissions)="resetRequested.emit()"
    ></app-qa-test-mode-banner>
  }

  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    <!-- Left Column: Persona & Access -->
    <div class="flex flex-col gap-6">
      <app-qa-persona-selector
        [roles]="roles()"
        [(selectedProfileId)]="selectedProfileId"
        (apply)="applyProfileRequested.emit()"
      ></app-qa-persona-selector>

      <app-qa-feature-access-stats
    ...
    <!-- Right Column: Granular Controls -->
    <app-qa-permission-toggles
      [permissions]="togglePermissions()"
      [checkPermission]="checkPermission()"
      (togglePermission)="togglePermissionRequested.emit($event)"
    ></app-qa-permission-toggles>
  </div>

  <!-- Feature Access Tables Per Tier -->
  @if (canViewContext()('client') && clientFeatureAccess().length > 0) {
    <app-qa-feature-access-table
      [rows]="clientFeatureAccess()"
      context="Client"
    ></app-qa-feature-access-table>
  }

  @if (canViewContext()('vendor') && vendorFeatureAccess().length > 0) {
    <app-qa-feature-access-table
      [rows]="vendorFeatureAccess()"
      context="Vendor"
    ></app-qa-feature-access-table>
  }

  @if (
    canViewContext()('distributor') && distributorFeatureAccess().length > 0
  ) {
    <app-qa-feature-access-table
      [rows]="distributorFeatureAccess()"
      context="Distributor"
    ></app-qa-feature-access-table>
  }
</div>
` }]
  }], null, { testMode: [{ type: Input, args: [{ isSignal: true, alias: "testMode", required: true }] }], activeProfileName: [{ type: Input, args: [{ isSignal: true, alias: "activeProfileName", required: true }] }], isSaving: [{ type: Input, args: [{ isSignal: true, alias: "isSaving", required: false }] }], roles: [{ type: Input, args: [{ isSignal: true, alias: "roles", required: true }] }], togglePermissions: [{ type: Input, args: [{ isSignal: true, alias: "togglePermissions", required: true }] }], checkPermission: [{ type: Input, args: [{ isSignal: true, alias: "checkPermission", required: true }] }], clientStats: [{ type: Input, args: [{ isSignal: true, alias: "clientStats", required: true }] }], vendorStats: [{ type: Input, args: [{ isSignal: true, alias: "vendorStats", required: true }] }], distributorStats: [{ type: Input, args: [{ isSignal: true, alias: "distributorStats", required: true }] }], canViewContext: [{ type: Input, args: [{ isSignal: true, alias: "canViewContext", required: true }] }], clientFeatureAccess: [{ type: Input, args: [{ isSignal: true, alias: "clientFeatureAccess", required: false }] }], vendorFeatureAccess: [{ type: Input, args: [{ isSignal: true, alias: "vendorFeatureAccess", required: false }] }], distributorFeatureAccess: [{ type: Input, args: [{ isSignal: true, alias: "distributorFeatureAccess", required: false }] }], selectedProfileId: [{ type: Input, args: [{ isSignal: true, alias: "selectedProfileId", required: false }] }, { type: Output, args: ["selectedProfileIdChange"] }], saveRequested: [{ type: Output, args: ["saveRequested"] }], resetRequested: [{ type: Output, args: ["resetRequested"] }], applyProfileRequested: [{ type: Output, args: ["applyProfileRequested"] }], togglePermissionRequested: [{ type: Output, args: ["togglePermissionRequested"] }] });
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(QaSimulationTabComponent, { className: "QaSimulationTabComponent", filePath: "src/app/shared/components/qa-permission-tester/components/qa-simulation-tab/qa-simulation-tab.component.ts", lineNumber: 29 });
})();

// src/app/shared/utils/api-simulation.registry.ts
var API_SIMULATION_REGISTRY = [
  // --- Alerts & Notifications ---
  {
    id: "alerts",
    domain: "Alerts",
    name: "Alerts",
    basePath: "/entities/alerts",
  ...
  },
  {
    ...
    domain: "Alerts",
    name: "Reminders",
    basePath: "/entities/reminders",
    operations: ["list", "create", "update", "delete"],
    permission: "reminder:create",
    requiresOwnerId: true,
    createFields: ["name", "ownerId", "message", "dueDate", "assetId"],
    updateFields: ["name", "message", "dueDate", "assetId"]
  },
  // --- Assets ---
  {
    id: "assets",
  ...
  },
  {
    ...
    basePath: "/entities/assetcategories",
    operations: ["list", "create", "update", "delete"],
    permission: "assetCategory:create",
    requiresOwnerId: true,
    createFields: ["index", "name", "parent", "contacts", "ownerId"],
    updateFields: ["index", "name", "parent", "contacts"]
  },
  {
    id: "assetGroups",
    domain: "Assets",
    name: "Asset Groups",
    basePath: "/entities/assetgroups",
    operations: ["list", "create", "update", "delete"],
    permission: "assetGroup:create",
    requiresOwnerId: true,
    createFields: ["name", "parent", "ownerId"],
    updateFields: ["name", "parent"]
  },
  {
    id: "assetTags",
    domain: "Assets",
    name: "Asset Tags",
    basePath: "/entities/assettags",
    operations: ["list", "create", "update"],
    permission: "assetTag:create",
    requiresOwnerId: true,
    createFields: ["name", "serialNumber", "ownerId"],
    updateFields: ["name", "serialNumber"]
  },
  // --- External Entities ---
  {
    id: "clients",
    domain: "Accounts",
    name: "Clients",
    basePath: "/accounts/clients",
    operations: ["list", "create", "update"],
    permission: "client:create",
    requiresOwnerId: true,
    createFields: ["name", "email", "phoneNumber", "ownerId"],
    updateFields: ["name", "email", "phoneNumber"]
  },
  {
    id: "vendors",
    domain: "Accounts",
    name: "Vendors",
    basePath: "/accounts/vendors",
    operations: ["list", "create", "update"],
  ...
  },
  {
    ...
    domain: "Accounts",
    name: "Distributors",
    basePath: "/accounts/distributors",
    operations: ["list", "create", "update"],
  ...
  },
  {
    ...
    domain: "Accounts",
    name: "Users",
    basePath: "/accounts/users",
    operations: ["list", "create", "update"],
    permission: "user:create",
    requiresOwnerId: true,
    createFields: ["firstName", "lastName", "email", "roleId", "ownerId"],
    updateFields: ["firstName", "lastName", "email", "roleId"]
  },
  {
    id: "userRoles",
    domain: "Accounts",
    name: "User Roles",
    basePath: "/accounts/userroles",
    operations: ["list", "create", "update", "delete"],
    permission: "userrole:create",
    requiresOwnerId: true,
    createFields: ["name", "permissions", "ownerId"],
  ...
  // --- Hardware & Devices ---
  {
    ...
    domain: "Hardware",
    name: "Devices",
    basePath: "/entities/devices",
    operations: ["list", "create", "update"],
    permission: "device:create",
    requiresOwnerId: true,
    createFields: ["name", "imei", "deviceTypeId", "providerId", "ownerId"],
    updateFields: ["name", "imei", "deviceTypeId", "providerId"]
  },
  {
    id: "deviceTypes",
    domain: "Hardware",
    name: "Device Types",
    basePath: "/entities/devicetypes",
    operations: ["list", "create", "update"],
    permission: "deviceType:create",
    requiresOwnerId: false,
    createFields: [
      "name",
      "shortName",
  ...
  },
  {
    ...
    basePath: "/entities/deviceproviders",
    operations: ["list", "create", "update"],
    permission: "deviceProvider:create",
    requiresOwnerId: true,
    createFields: ["name", "apiEndpoint", "apiKey", "ownerId"],
    updateFields: ["name", "apiEndpoint", "apiKey"]
  },
  {
    id: "simCards",
    domain: "Hardware",
    name: "SIM Cards",
    basePath: "/entities/simcards",
    operations: ["list", "create", "update"],
    permission: "simCard:create",
    requiresOwnerId: true,
    createFields: ["iccid", "phoneNumber", "provider", "ownerId"],
    updateFields: ["iccid", "phoneNumber", "provider"]
  },
  // --- Profiles & Configurations ---
  {
    id: "assetRatingProfiles",
    domain: "Profiles",
    name: "Asset Rating Profiles",
    basePath: "/entities/assetratingprofiles",
    operations: ["list", "create", "update"],
    permission: "assetRatingProfile:create",
    requiresOwnerId: true,
    createFields: ["name", "description", "penalties", "ownerId"],
    updateFields: ["name", "description", "penalties"]
  },
  {
    id: "assetStateProfiles",
    domain: "Profiles",
    name: "Asset State Profiles",
    basePath: "/entities/assetstateprofiles",
    operations: ["list", "create", "update"],
    permission: "assetStateProfile:create",
    requiresOwnerId: true,
    createFields: ["name", "description", "states", "ownerId"],
    updateFields: ["name", "description", "states"]
  },
  {
    id: "deviceConfigProfiles",
    domain: "Profiles",
    name: "Device Config Profiles",
    basePath: "/entities/deviceconfigprofiles",
    operations: ["list", "create", "update"],
    permission: "deviceConfigProfile:create",
    requiresOwnerId: true,
    createFields: [
      "name",
      "description",
    ...
    updateFields: ["name", "description", "deviceTypeId", "configuration"]
  },
  {
    id: "geoLockProfiles",
    domain: "Profiles",
    name: "Geo Lock Profiles",
    basePath: "/entities/geolockprofiles",
    operations: ["list", "create", "update"],
    permission: "geoLockProfile:create",
    requiresOwnerId: true,
    createFields: ["name", "description", "activeHours", "ownerId"],
    updateFields: ["name", "description", "activeHours"]
  },
  {
    id: "overspeedProfiles",
    domain: "Profiles",
    name: "Overspeed Profiles",
    basePath: "/entities/overspeedprofiles",
    operations: ["list", "create", "update"],
    permission: "overspeedProfile:create",
    requiresOwnerId: true,
    createFields: [
      "name",
      "description",
    ...
    updateFields: ["name", "description", "speedLimit", "gracePeriod"]
  },
  {
    id: "roadProfiles",
    domain: "Profiles",
    name: "Road Profiles",
    basePath: "/entities/roadprofiles",
    operations: ["list", "create", "update"],
    permission: "roadProfile:create",
    requiresOwnerId: true,
    createFields: [
      "name",
      "description",
    ...
    updateFields: ["name", "description", "roadType", "speedModifier"]
  },
  {
    id: "ioTypes",
    domain: "Profiles",
    name: "IO Types",
    basePath: "/entities/iotypes",
    operations: ["list", "create", "update"],
    permission: "ioType:create",
    requiresOwnerId: true,
    createFields: ["name", "description", "dataType", "unit", "ownerId"],
    updateFields: ["name", "description", "dataType", "unit"]
  },
  {
    id: "labels",
    domain: "Profiles",
    name: "Labels",
    basePath: "/entities/labels",
    operations: ["list", "create", "update"],
    permission: "label:create",
    requiresOwnerId: true,
    createFields: ["name", "color", "icon", "ownerId"],
  ...
  // --- Reports & Dashboards ---
  {
    ...
    requiresOwnerId: true,
    createFields: ["name", "description", "ownerId"],
  ...
  },
  {
    ...
    basePath: "/entities/dashboardtemplates",
    operations: ["list", "create", "update"],
    permission: "dashboardTemplate:create",
    requiresOwnerId: false,
    createFields: [
      "name",
      "description",
    ...
    updateFields: ["name", "description", "dashboardId"]
  },
  {
    id: "reportTemplates",
    domain: "Reports",
    name: "Report Templates",
    basePath: "/entities/reporttemplates",
    operations: ["list", "create", "update", "delete"],
    permission: "reportTemplate:create",
    requiresOwnerId: false,
    createFields: ["name", "description", "ownerId"],
    updateFields: ["name", "description"]
  },
  {
    ...
    basePath: "/entities/scheduledreports",
    operations: ["list", "create", "update"],
    permission: "scheduledReport:create",
    requiresOwnerId: true,
    createFields: ["name", "templateId", "ownerId"],
    updateFields: ["name", "templateId"]
  },
  // --- Organization & Geography ---
  {
    ...
    domain: "Organization",
    name: "Cost Centres",
    basePath: "/entities/costcentres",
    operations: ["list", "create", "update", "delete"],
    permission: "costCentre:create",
    requiresOwnerId: true,
    createFields: ["name", "code", "budget", "managerId", "ownerId"],
    updateFields: ["name", "code", "budget", "managerId"]
  },
  {
    id: "companyGroups",
    domain: "Organization",
    name: "Company Groups",
    basePath: "/accounts/companygroups",
    operations: ["list", "create", "update", "delete"],
    permission: "companyGroup:create",
    requiresOwnerId: true,
    createFields: ["name", "description", "companies", "ownerId"],
    updateFields: ["name", "description", "companies"]
  },
  {
    id: "zones",
    domain: "Geography",
    name: "Zones (Geofences)",
    basePath: "/entities/zones",
    operations: ["list", "create", "update", "delete"],
    permission: "zone:create",
    requiresOwnerId: true,
    createFields: ["name", "description", "geometry", "color", "ownerId"],
    updateFields: ["name", "description", "geometry", "color"]
  },
  {
    id: "zoneGroups",
    domain: "Geography",
    name: "Zone Groups",
    basePath: "/entities/zonegroups",
    operations: ["list", "create", "update", "delete"],
    permission: "zoneGroup:create",
    requiresOwnerId: true,
    createFields: ["name", "description", "zones", "ownerId"],
    updateFields: ["name", "description", "zones"]
  },
  {
    id: "mapSets",
    domain: "Geography",
    name: "Map Sets",
    basePath: "/entities/mapsets",
    operations: ["list", "create", "update"],
    permission: "mapSet:create",
    requiresOwnerId: true,
    createFields: ["name", "description", "layers", "defaultView", "ownerId"],
    updateFields: ["name", "description", "layers", "defaultView"]
  },
  // --- Integrations ---
  {
    id: "emailProviders",
    ...
    basePath: "/entities/emailproviders",
    operations: ["list", "create", "update"],
    permission: "emailProvider:create",
    requiresOwnerId: true,
    createFields: ["name", "type", "ownerId"],
    updateFields: ["state"]
  },
  {
    id: "smsGatewayProviders",
    domain: "Integrations",
    name: "SMS Gateway Providers",
    basePath: "/entities/smsgatewayproviders",
    operations: ["list", "create", "update"],
    permission: "smsGatewayProvider:create",
    requiresOwnerId: true,
    createFields: ["name", "gatewayUrl", "apiKey", "ownerId"],
    updateFields: ["name", "gatewayUrl", "apiKey"]
  },
  // --- Settings & Miscellaneous ---
  {
    ...
    domain: "Settings",
    name: "Themes",
    basePath: "/entities/themes",
    operations: ["list", "create", "update"],
    permission: "theme:create",
    requiresOwnerId: true,
    createFields: ["name", "colors", "fonts", "logoUrl", "ownerId"],
    updateFields: ["name", "colors", "fonts", "logoUrl"]
  },
  {
    id: "sslCertificates",
    domain: "Settings",
    name: "SSL Certificates",
    basePath: "/entities/sslcertificates",
    operations: ["list", "create", "update"],
    permission: "sslCertificate:create",
    requiresOwnerId: true,
    createFields: ["domain", "certificateString", "privateKey", "ownerId"],
    updateFields: ["domain", "certificateString", "privateKey"]
  },
  {
    id: "exportTasks",
    domain: "Settings",
    name: "Export Tasks",
    basePath: "/entities/exporttasks",
    operations: ["list", "create", "update"],
    permission: "exportTask:create",
    requiresOwnerId: true,
    createFields: ["name", "dataType", "format", "filters", "ownerId"],
    updateFields: ["name", "dataType", "format", "filters"]
  },
  {
    id: "fuelCards",
    domain: "Hardware",
    name: "Fuel Cards",
    basePath: "/entities/fuelcards",
    operations: ["list", "create", "update"],
    permission: "fuelCard:create",
    requiresOwnerId: true,
    createFields: ["cardNumber", "provider", "driverId", "ownerId"],
    updateFields: ["cardNumber", "provider", "driverId"]
  }
];

// src/app/core/services/api-simulation.service.ts
var PERMISSION_TIERS = ["client", "vendor", "distributor"];
var ApiSimulationService = class _ApiSimulationService {
  http = inject(HttpClient);
  authData = inject(AuthDataService);
  apiConfig = inject(Configuration);
  /** Fake UUID used for update/delete path parameters */
  FAKE_ID = "3fa85f64-5717-4562-b3fc-2c963f66afa6";
  /**
   * Returns ALL API entries from the registry.
   * We test every endpoint — the backend will tell us which are
   * Allowed (200) vs Forbidden (401/403) for the current user.
   */
  getSimulatableApis() {
    return API_SIMULATION_REGISTRY;
  }
  /**
   * Comprehensive permission check that accounts for the way legacyRights
   * stores permissions:
   *
   * 1. Direct match: `alert:create` exists in the user's permission array
   * 2. Tier-scoped wildcard: `client:alerts` — having the resource name
   *    (without an action) under a parent tier means full CRUD access
   * 3. Tier-scoped specific: `client:list-alerts` — specific action grant
   *
   * @param permission - The registry permission string (e.g., 'alert:create')
   * @param operation - The CRUD operation being tested (list/create/update/delete)
   * @returns true if the user effectively has this permission
   */
  hasEffectivePermission(permission, operation) {
    const realPermissions = this.authData.getRealUserPermissions();
    if (realPermissions.includes("system:all")) {
      return true;
    }
    if (this.authData.hasRealPermission(permission)) {
      return true;
    }
    const parts = permission.split(":");
    const resource = parts.length >= 2 ? parts[0] : permission;
    const pluralResource = PermissionUtils.getPluralForm(resource);
    for (const tier of PERMISSION_TIERS) {
      if (realPermissions.includes(`${tier}:${pluralResource}`)) {
        return true;
      }
    }
    if (operation === "list") {
      for (const tier of PERMISSION_TIERS) {
        if (realPermissions.includes(`${tier}:list-${pluralResource}`)) {
          return true;
        }
      }
    }
  ...
  /**
   * Hits the real backend with `x-auth-testing: true` for a single
   * CRUD operation and interprets the response as Allowed / Forbidden / Error.
   */
  async verifyOperation(entry, operation, ownerId) {
    const isResourceLevel = operation === "update" || operation === "delete";
    let endpointPath = isResourceLevel ? `${entry.basePath}/${this.FAKE_ID}` : entry.basePath;
    if (operation === "list") {
      const separator = endpointPath.includes("?") ? "&" : "?";
      endpointPath = `${endpointPath}${separator}limit=1&owner=${ownerId}`;
    }
    const url = `${this.apiConfig.basePath}${endpointPath}`;
    const httpMethodMap = {
      list: "GET",
      create: "POST",
      update: "PUT",
    ...
    const httpMethod = httpMethodMap[operation];
    const requestBody = operation === "list" || operation === "delete" ? void 0 : SmartPayloadBuilder.generateFromRegistry(entry, operation, ownerId) ?? {};
    const headersDict = {
      "x-auth-testing": "true",
    ...
    const context = new HttpContext().set(SILENT_ERROR, true);
    const options = { headers, context, observe: "response" };
    const permissionToCheck = operation === "list" && entry.listPermission !== void 0 && entry.listPermission !== "" ? entry.listPermission : entry.permission;
    const frontendExpected = this.hasEffectivePermission(permissionToCheck, operation) ? "Allowed" : "Forbidden";
    const startTime = performance.now();
    try {
      let response;
      switch (httpMethod) {
        case "GET":
          response = await firstValueFrom(this.http.get(url, options));
          break;
        case "POST":
          response = await firstValueFrom(this.http.post(url, requestBody, options));
          break;
        case "PUT":
          response = await firstValueFrom(this.http.put(url, requestBody, options));
          break;
        case "DELETE":
  ...
  /**
   * Parses an error from the HTTP pipeline (which may have been
   * transformed by the auth interceptor) and builds a SimulationResult.
   */
  buildErrorResult(err, ctx) {
    let status;
    let errorBody;
    let errorMessage;
    if (err instanceof HttpErrorResponse) {
      ...
      errorBody = anyErr["error"];
      errorMessage = anyErr["message"];
    }
    const isForbiddenByStatus = status === 403 || status === 401;
    const isForbiddenByMessage = errorMessage !== void 0 && errorMessage !== "" && (/forbidden/i.test(errorMessage) || /not have permission/i.test(errorMessage) || /unauthorized/i.test(errorMessage));
    const isForbidden = isForbiddenByStatus || isForbiddenByMessage;
    return {
      entry: ctx.entry,
      operation: ctx.operation,
      frontendExpected: ctx.frontendExpected,
      backendAccess: isForbidden ? "Forbidden" : "Error",
      url: ctx.url,
      httpMethod: ctx.httpMethod,
      headersSent: ctx.headersSent,
      requestBody: ctx.requestBody,
      responseBody: errorBody ?? void 0,
      durationMs: Math.round(performance.now() - ctx.startTime),
      errorDetails: isForbidden ? `${status ?? "403"} Forbidden` : `${status ?? "N/A"} ${errorMessage ?? "Unknown error"}`
    };
  }
  /**
   * Runs verification for multiple API entries sequentially.
   * Calls `onProgress` after each individual operation completes.
   */
  async verifyMultiple(entries, ownerId, onProgress) {
    const results = [];
    for (const entry of entries) {
      for (const op of entry.operations) {
        const result = await this.verifyOperation(entry, op, ownerId);
        results.push(result);
        if (onProgress) {
          onProgress(result);
        }
  ...
  static \u0275fac = function ApiSimulationService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _ApiSimulationService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ApiSimulationService, factory: _ApiSimulationService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ApiSimulationService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
// src/app/shared/components/qa-permission-tester/components/qa-api-simulation-panel/qa-api-simulation-panel.component.ts
var _c05 = () => ({ "min-width": "50rem" });
var _c13 = (a0) => ({ $implicit: a0, op: "GET" });
var _c2 = (a0) => ({ $implicit: a0, op: "POST" });
var _c3 = (a0) => ({ $implicit: a0, op: "PUT" });
var _c4 = (a0) => ({ $implicit: a0, op: "DELETE" });
var _c5 = (a0, a1, a210) => ({ "bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800": a0, "bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800": a1, "bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800": a210 });
var _c6 = (a0, a1, a210) => ({ "text-green-500": a0, "text-red-500": a1, "text-yellow-500": a210 });
function QaApiSimulationPanelComponent_Conditional_12_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 8)(1, "div", 17)(2, "span", 18);
    \u0275\u0275text(3, "Tested:");
    \u0275\u0275elementEnd();
    \u0275\u0275element(4, "p-tag", 19);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(5, "div", 17)(6, "span", 18);
    \u0275\u0275text(7, "Allowed:");
    \u0275\u0275elementEnd();
    \u0275\u0275element(8, "p-tag", 20);
    \u0275\u0275elementEnd();
    ...
    \u0275\u0275text(15, "Errors:");
    \u0275\u0275elementEnd();
    \u0275\u0275element(16, "p-tag", 22);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(17, "div", 23)(18, "span", 18);
    \u0275\u0275text(19, "Mismatches:");
    \u0275\u0275elementEnd();
    \u0275\u0275element(20, "p-tag", 24);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275advance(4);
    \u0275\u0275property("value", ctx_r0.stats().total.toString());
    \u0275\u0275advance(4);
    \u0275\u0275property("value", ctx_r0.stats().allowed.toString());
    \u0275\u0275advance(4);
    \u0275\u0275property("value", ctx_r0.stats().forbidden.toString());
    \u0275\u0275advance(4);
    \u0275\u0275property("value", ctx_r0.stats().errors.toString());
    \u0275\u0275advance(4);
    \u0275\u0275property("value", ctx_r0.stats().mismatches.toString())("severity", ctx_r0.stats().mismatches > 0 ? "danger" : "success");
  }
}
function QaApiSimulationPanelComponent_ng_template_15_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance(3);
    \u0275\u0275textInterpolate(entry_r2.domain);
  }
}
function QaApiSimulationPanelComponent_ng_template_16_Template(rf, ctx) {
  if (rf & 1) {
...
function QaApiSimulationPanelComponent_ng_template_17_ng_container_7_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementContainer(0);
  }
}
function QaApiSimulationPanelComponent_ng_template_17_ng_container_9_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementContainer(0);
  }
}
function QaApiSimulationPanelComponent_ng_template_17_ng_container_11_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementContainer(0);
  }
}
function QaApiSimulationPanelComponent_ng_template_17_ng_container_13_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementContainer(0);
  }
}
function QaApiSimulationPanelComponent_ng_template_17_Template(rf, ctx) {
  if (rf & 1) {
    const _r3 = \u0275\u0275getCurrentView();
    ...
    \u0275\u0275elementEnd()();
    \u0275\u0275elementStart(6, "td");
    \u0275\u0275template(7, QaApiSimulationPanelComponent_ng_template_17_ng_container_7_Template, 1, 0, "ng-container", 32);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(8, "td");
    \u0275\u0275template(9, QaApiSimulationPanelComponent_ng_template_17_ng_container_9_Template, 1, 0, "ng-container", 32);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(10, "td");
    \u0275\u0275template(11, QaApiSimulationPanelComponent_ng_template_17_ng_container_11_Template, 1, 0, "ng-container", 32);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(12, "td");
    \u0275\u0275template(13, QaApiSimulationPanelComponent_ng_template_17_ng_container_13_Template, 1, 0, "ng-container", 32);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(14, "td")(15, "p-button", 33);
    \u0275\u0275listener("onClick", function QaApiSimulationPanelComponent_ng_template_17_Template_p_button_onClick_15_listener() {
      const entry_r4 = \u0275\u0275restoreView(_r3).$implicit;
      const ctx_r0 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r0.runSingleVerification(entry_r4));
    });
    \u0275\u0275elementEnd()()();
  }
  if (rf & 2) {
    ...
    const ctx_r0 = \u0275\u0275nextContext();
    const opCell_r5 = \u0275\u0275reference(20);
    \u0275\u0275advance(3);
    \u0275\u0275textInterpolate1(" ", entry_r4.name, " ");
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate1(" ", entry_r4.basePath, " ");
    \u0275\u0275advance(2);
    \u0275\u0275property("ngTemplateOutlet", opCell_r5)("ngTemplateOutletContext", \u0275\u0275pureFunction1(13, _c13, ctx_r0.getResultFor(entry_r4.id, "list")));
    \u0275\u0275advance(2);
    \u0275\u0275property("ngTemplateOutlet", opCell_r5)("ngTemplateOutletContext", \u0275\u0275pureFunction1(15, _c2, ctx_r0.getResultFor(entry_r4.id, "create")));
    \u0275\u0275advance(2);
    \u0275\u0275property("ngTemplateOutlet", opCell_r5)("ngTemplateOutletContext", \u0275\u0275pureFunction1(17, _c3, ctx_r0.getResultFor(entry_r4.id, "update")));
    \u0275\u0275advance(2);
    \u0275\u0275property("ngTemplateOutlet", opCell_r5)("ngTemplateOutletContext", \u0275\u0275pureFunction1(19, _c4, ctx_r0.getResultFor(entry_r4.id, "delete")));
    \u0275\u0275advance(2);
    \u0275\u0275property("rounded", true)("text", true)("loading", ctx_r0.isVerifying());
  }
}
function QaApiSimulationPanelComponent_ng_template_18_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "tr")(1, "td", 34);
    \u0275\u0275element(2, "i", 35);
    \u0275\u0275elementStart(3, "p", 36);
    \u0275\u0275text(4, " No APIs are available based on the current simulated permissions. Toggle permissions above or switch persona to unlock APIs. ");
    \u0275\u0275elementEnd()()();
  }
}
function QaApiSimulationPanelComponent_ng_template_19_Conditional_0_Conditional_3_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "i", 42);
  }
}
function QaApiSimulationPanelComponent_ng_template_19_Conditional_0_Conditional_6_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    const result_r7 = \u0275\u0275nextContext(2).$implicit;
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1("Expected: ", result_r7.frontendExpected);
  }
}
function QaApiSimulationPanelComponent_ng_template_19_Conditional_0_Conditional_7_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    const result_r7 = \u0275\u0275nextContext(2).$implicit;
    \u0275\u0275property("title", result_r7.errorDetails ?? "");
    \u0275\u0275advance();
    \u0275\u0275textInterpolate(result_r7.errorDetails);
  }
}
function QaApiSimulationPanelComponent_ng_template_19_Conditional_0_Template(rf, ctx) {
  if (rf & 1) {
    const _r6 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "button", 39);
    \u0275\u0275listener("click", function QaApiSimulationPanelComponent_ng_template_19_Conditional_0_Template_button_click_0_listener() {
      \u0275\u0275restoreView(_r6);
      const result_r7 = \u0275\u0275nextContext().$implicit;
      const ctx_r0 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r0.showDetails(result_r7));
    });
    \u0275\u0275elementStart(1, "div", 40);
    \u0275\u0275element(2, "p-tag", 41);
    \u0275\u0275conditionalCreate(3, QaApiSimulationPanelComponent_ng_template_19_Conditional_0_Conditional_3_Template, 1, 0, "i", 42);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(4, "span", 43);
    \u0275\u0275text(5);
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(6, QaApiSimulationPanelComponent_ng_template_19_Conditional_0_Conditional_6_Template, 2, 1, "span", 44);
    \u0275\u0275conditionalCreate(7, QaApiSimulationPanelComponent_ng_template_19_Conditional_0_Conditional_7_Template, 2, 2, "span", 45);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const ctx_r7 = \u0275\u0275nextContext();
    const result_r7 = ctx_r7.$implicit;
    const op_r9 = ctx_r7.op;
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275advance(2);
    \u0275\u0275property("severity", ctx_r0.getAccessSeverity(result_r7.backendAccess))("value", result_r7.backendAccess);
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r0.isMismatch(result_r7) ? 3 : -1);
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate2("", result_r7.durationMs, "ms \xB7 ", op_r9);
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r0.isMismatch(result_r7) ? 6 : -1);
    \u0275\u0275advance();
    \u0275\u0275conditional(result_r7.backendAccess === "Error" ? 7 : -1);
  }
}
function QaApiSimulationPanelComponent_ng_template_19_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
...
function QaApiSimulationPanelComponent_ng_template_19_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275conditionalCreate(0, QaApiSimulationPanelComponent_ng_template_19_Conditional_0_Template, 8, 7, "button", 37)(1, QaApiSimulationPanelComponent_ng_template_19_Conditional_1_Template, 2, 0, "span", 38);
  }
  if (rf & 2) {
    const result_r7 = ctx.$implicit;
    \u0275\u0275conditional(result_r7 ? 0 : 1);
  }
}
function QaApiSimulationPanelComponent_Conditional_23_Conditional_4_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "i", 63);
...
function QaApiSimulationPanelComponent_Conditional_23_Conditional_5_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "i", 65);
  ...
  if (rf & 2) {
    ...
    \u0275\u0275property("ngClass", \u0275\u0275pureFunction3(1, _c6, r_r10.backendAccess === "Allowed", r_r10.backendAccess === "Forbidden", r_r10.backendAccess === "Error"));
  }
}
function QaApiSimulationPanelComponent_Conditional_23_Conditional_32_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 58)(1, "pre", 66);
    \u0275\u0275text(2);
    \u0275\u0275pipe(3, "json");
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const r_r10 = \u0275\u0275nextContext();
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate(\u0275\u0275pipeBind1(3, 1, r_r10.requestBody));
  }
}
function QaApiSimulationPanelComponent_Conditional_23_Conditional_33_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "p", 59);
    \u0275\u0275text(1, "No body (DELETE request)");
    \u0275\u0275elementEnd();
  }
}
function QaApiSimulationPanelComponent_Conditional_23_Conditional_39_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 58)(1, "pre", 66);
    \u0275\u0275text(2);
    \u0275\u0275pipe(3, "json");
  ...
  if (rf & 2) {
    const r_r10 = \u0275\u0275nextContext();
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate(\u0275\u0275pipeBind1(3, 1, r_r10.responseBody));
  }
}
function QaApiSimulationPanelComponent_Conditional_23_Conditional_40_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    const r_r10 = \u0275\u0275nextContext();
    \u0275\u0275advance();
    \u0275\u0275textInterpolate(r_r10.errorDetails);
  }
}
function QaApiSimulationPanelComponent_Conditional_23_Conditional_41_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "p", 59);
    \u0275\u0275text(1, "Empty response");
    \u0275\u0275elementEnd();
  }
}
function QaApiSimulationPanelComponent_Conditional_23_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 16)(1, "div", 46)(2, "div", 47)(3, "div", 48);
    \u0275\u0275conditionalCreate(4, QaApiSimulationPanelComponent_Conditional_23_Conditional_4_Template, 3, 0)(5, QaApiSimulationPanelComponent_Conditional_23_Conditional_5_Template, 3, 5);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(6, "div", 49);
    \u0275\u0275text(7, " Frontend expected: ");
    \u0275\u0275elementStart(8, "span", 50);
    \u0275\u0275text(9);
    \u0275\u0275elementEnd();
    \u0275\u0275text(10, " \xB7 Backend returned: ");
    ...
    \u0275\u0275text(14);
    \u0275\u0275elementEnd()();
    \u0275\u0275elementStart(15, "div", 52)(16, "h4", 53);
    \u0275\u0275text(17, "Request");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(18, "div", 54)(19, "p")(20, "span", 55);
    \u0275\u0275text(21);
    \u0275\u0275elementEnd();
    \u0275\u0275text(22);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(23, "p", 56)(24, "span", 50);
    \u0275\u0275text(25, "Headers:");
    \u0275\u0275elementEnd()();
    \u0275\u0275elementStart(26, "pre", 57);
    \u0275\u0275text(27);
    \u0275\u0275pipe(28, "json");
    \u0275\u0275elementEnd()()();
    \u0275\u0275elementStart(29, "div", 52)(30, "h4", 53);
    \u0275\u0275text(31, " Request Body ");
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(32, QaApiSimulationPanelComponent_Conditional_23_Conditional_32_Template, 4, 3, "div", 58)(33, QaApiSimulationPanelComponent_Conditional_23_Conditional_33_Template, 2, 0, "p", 59);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(34, "div", 52)(35, "h4", 60)(36, "span");
    \u0275\u0275text(37, "Response");
    \u0275\u0275elementEnd();
    \u0275\u0275element(38, "p-tag", 61);
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(39, QaApiSimulationPanelComponent_Conditional_23_Conditional_39_Template, 4, 3, "div", 58)(40, QaApiSimulationPanelComponent_Conditional_23_Conditional_40_Template, 2, 1, "p", 62)(41, QaApiSimulationPanelComponent_Conditional_23_Conditional_41_Template, 2, 0, "p", 59);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const r_r10 = ctx;
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275advance();
    \u0275\u0275property("ngClass", \u0275\u0275pureFunction3(15, _c5, r_r10.backendAccess === "Allowed" && !ctx_r0.isMismatch(r_r10), r_r10.backendAccess === "Forbidden" || ctx_r0.isMismatch(r_r10), r_r10.backendAccess === "Error"));
    \u0275\u0275advance(3);
    \u0275\u0275conditional(ctx_r0.isMismatch(r_r10) ? 4 : 5);
    \u0275\u0275advance(5);
    \u0275\u0275textInterpolate(r_r10.frontendExpected);
    \u0275\u0275advance(3);
    \u0275\u0275textInterpolate(r_r10.backendAccess);
    \u0275\u0275advance(2);
    \u0275\u0275textInterpolate1("", r_r10.durationMs, "ms");
    \u0275\u0275advance(7);
    \u0275\u0275textInterpolate(r_r10.httpMethod);
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1(" ", r_r10.url, " ");
    \u0275\u0275advance(5);
    \u0275\u0275textInterpolate(\u0275\u0275pipeBind1(28, 13, r_r10.headersSent));
    \u0275\u0275advance(5);
    \u0275\u0275conditional(r_r10.requestBody ? 32 : 33);
    \u0275\u0275advance(6);
    \u0275\u0275property("severity", ctx_r0.getAccessSeverity(r_r10.backendAccess))("value", r_r10.backendAccess)("rounded", true);
    \u0275\u0275advance();
    \u0275\u0275conditional(r_r10.responseBody ? 39 : r_r10.errorDetails ? 40 : 41);
  }
}
var QaApiSimulationPanelComponent = class _QaApiSimulationPanelComponent {
  simulationService = inject(ApiSimulationService);
  authData = inject(AuthDataService);
  ...
  selectedResult = signal(null, ...ngDevMode ? [{ debugName: "selectedResult" }] : (
    /* istanbul ignore next */
    []
  ));
  availableApis = computed(() => {
    return this.simulationService.getSimulatableApis();
  }, ...ngDevMode ? [{ debugName: "availableApis" }] : (
    /* istanbul ignore next */
    []
  ));
  stats = computed(() => {
    const res = this.results();
    const matches = res.filter((r85) => r85.backendAccess !== "Pending" && r85.frontendExpected === r85.backendAccess);
    const mismatches = res.filter((r85) => r85.backendAccess !== "Pending" && r85.backendAccess !== "Error" && r85.frontendExpected !== r85.backendAccess);
    return {
      total: res.length,
      allowed: res.filter((r85) => r85.backendAccess === "Allowed").length,
      forbidden: res.filter((r85) => r85.backendAccess === "Forbidden").length,
      errors: res.filter((r85) => r85.backendAccess === "Error").length,
  ...
  /** PrimeNG severity tag for backend access status */
  getAccessSeverity(access) {
    if (access === "Allowed")
      return "success";
    if (access === "Forbidden")
      return "danger";
  ...
  /** Returns true when frontend expectation ≠ backend result (a mismatch) */
  isMismatch(result) {
    return result.backendAccess !== "Pending" && result.backendAccess !== "Error" && result.frontendExpected !== result.backendAccess;
  }
  async runAllVerifications() {
    this.isVerifying.set(true);
    this.results.set([]);
    const apis = this.availableApis();
    const ownerId = this.authData.getUserOwnerId() ?? "";
    try {
      await this.simulationService.verifyMultiple(apis, ownerId, (result) => {
        this.results.update((curr) => [...curr, result]);
      });
    } finally {
      this.isVerifying.set(false);
    }
  }
  async runSingleVerification(entry) {
    this.isVerifying.set(true);
    this.results.update((curr) => curr.filter((r85) => r85.entry.id !== entry.id));
    const ownerId = this.authData.getUserOwnerId() ?? "";
    try {
      await this.simulationService.verifyMultiple([entry], ownerId, (result) => {
        this.results.update((curr) => [...curr, result]);
      });
    } finally {
      this.isVerifying.set(false);
  ...
  getResultFor(entryId, operation) {
    return this.results().find((r85) => r85.entry.id === entryId && r85.operation === operation);
  }
  showDetails(result) {
    this.selectedResult.set(result);
  }
  static \u0275fac = function QaApiSimulationPanelComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _QaApiSimulationPanelComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _QaApiSimulationPanelComponent, selectors: [["app-qa-api-simulation-panel"]], decls: 24, vars: 14, consts: [["opCell", ""], [1, "flex", "flex-col", "gap-6", "mt-6"], [1, "flex", "items-center", "justify-between", "border-b", "pb-4", "border-gray-100", "dark:border-gray-700"], [1, "text-lg", "font-bold", "flex", "items-center", "gap-2"], [1, "pi", "pi-bolt", "text-yellow-500"], [1, "text-sm", "text-gray-500"], [1, "text-[11px]", "bg-gray-100", "dark:bg-gray-700", "px-1", "rounded"], ["type", "button", "label", "Verify All", "icon", "pi pi-play", "severity", "warn", 3, "onClick", "loading", "disabled"], [1, "flex", "flex-wrap", "gap-4", "p-4", "bg-gray-50", "dark:bg-gray-800", "rounded-xl"], [1, "border", "rounded-xl", "border-gray-100", "dark:border-gray-700", "overflow-hidden"], ["groupRowsBy", "domain", "rowGroupMode", "subheader", 3, "value", "tableStyle"], ["pTemplate", "groupheader"], ["pTemplate", "header"], ["pTemplate", "body"], ["pTemplate", "emptymessage"], ["styleClass", "w-full md:w-[50vw]", 3, "visibleChange", "header", "visible", "modal", "dismissableMask", "draggable"], [1, "flex", "flex-col", "gap-4"], [1, "flex", "items-center", "gap-2"], [1, "text-xs", "font-bold", "text-gray-500", "uppercase"], ["severity", "info", 3, "value"], ["severity", "success", 3, "value"], ["severity", "danger", 3, "value"], ["severity", "warn", 3, "value"], [1, "ml-auto", "flex", "items-center", "gap-2"], [3, "value", "severity"], [1, "p-rowgroup-header", "bg-gray-50", "dark:bg-gray-800/50", "uppercase"], ["colspan", "6"], [1, "font-bold", "text-gray-500", "text-xs", "tracking-wider"], ["scope", "col"], ["scope", "col", 1, "w-24"], [1, "font-medium"], [1, "text-xs", "text-gray-500", "font-mono"], [4, "ngTemplateOutlet", "ngTemplateOutletContext"], ["type", "button", "icon", "pi pi-play", "size", "small", "pTooltip", "Verify this entity", 3, "onClick", "rounded", "text", "loading"], ["colspan", "5", 1, "p-8", "text-center", "bg-gray-50", "dark:bg-gray-800"], [1, "pi", "pi-info-circle", "text-4xl", "text-gray-300", "mb-4"], [1, "text-gray-500"], ["type", "button", 1, "flex", "flex-col", "gap-1", "items-start", "cursor-pointer", "bg-transparent", "border-none", "p-0", "text-left"], [1, "text-gray-300", "dark:text-gray-600"], ["type", "button", 1, "flex", "flex-col", "gap-1", "items-start", "cursor-pointer", "bg-transparent", "border-none", "p-0", "text-left", 3, "click"], [1, "flex", "items-center", "gap-1"], [3, "severity", "value"], ["pTooltip", "Frontend/Backend mismatch!", 1, "pi", "pi-exclamation-triangle", "text-yellow-500", "text-xs"], [1, "text-[10px]", "text-gray-400", "font-mono"], [1, "text-[10px]", "text-yellow-600", "dark:text-yellow-400", "font-bold"], [1, "text-[10px]", "text-red-500", "truncate", "w-32", 3, "title"], [1, "flex", "items-center", "gap-3", "p-3", "rounded-lg", 3, "ngClass"], [1, "flex-1"], [1, "flex", "items-center", "gap-2", "text-sm", "font-bold"], [1, "text-xs", "text-gray-500", "mt-1"], [1, "font-bold"], [1, "text-xs", "text-gray-400", "font-mono"], [1, "p-3", "bg-gray-50", "border", "border-gray-200", "dark:bg-gray-800", "dark:border-gray-700", "rounded-lg"], [1, "text-xs", "font-bold", "text-gray-500", "uppercase", "mb-2"], [1, "text-sm", "font-mono", "break-all", "text-gray-700", "dark:text-gray-300"], [1, "font-bold", "text-primary"], [1, "mt-2"], [1, "bg-white", "dark:bg-gray-900", "border", "dark:border-gray-600", "p-2", "text-xs", "rounded", "mt-1"], [1, "max-h-48", "overflow-y-auto", "w-full"], [1, "text-sm", "text-gray-400", "italic"], [1, "text-xs", "font-bold", "text-gray-500", "uppercase", "mb-2", "flex", "items-center", "justify-between"], [3, "severity", "value", "rounded"], [1, "text-sm", "text-red-500"], [1, "pi", "pi-exclamation-triangle", "text-yellow-500"], [1, "text-yellow-700", "dark:text-yellow-400"], [1, "pi", "pi-check-circle", 3, "ngClass"], [1, "text-xs", "font-mono", "bg-white", "dark:bg-gray-900", "border", "dark:border-gray-600", "p-2", "rounded", "m-0", "break-words", "whitespace-pre-wrap"]], template: function QaApiSimulationPanelComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "div", 1)(1, "div", 2)(2, "div")(3, "h3", 3);
      \u0275\u0275element(4, "i", 4);
      \u0275\u0275text(5, " CRUD Permission Verification ");
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(6, "p", 5);
      \u0275\u0275text(7, " Hits the real backend with ");
      \u0275\u0275elementStart(8, "code", 6);
      \u0275\u0275text(9, "x-auth-testing: true");
      \u0275\u0275elementEnd();
      \u0275\u0275text(10, " to verify create / update / delete access without persisting data. ");
      \u0275\u0275elementEnd()();
      \u0275\u0275elementStart(11, "p-button", 7);
      \u0275\u0275listener("onClick", function QaApiSimulationPanelComponent_Template_p_button_onClick_11_listener() {
        return ctx.runAllVerifications();
      });
      \u0275\u0275elementEnd()();
      \u0275\u0275conditionalCreate(12, QaApiSimulationPanelComponent_Conditional_12_Template, 21, 6, "div", 8);
      \u0275\u0275elementStart(13, "div", 9)(14, "p-table", 10);
      \u0275\u0275template(15, QaApiSimulationPanelComponent_ng_template_15_Template, 4, 1, "ng-template", 11)(16, QaApiSimulationPanelComponent_ng_template_16_Template, 13, 0, "ng-template", 12)(17, QaApiSimulationPanelComponent_ng_template_17_Template, 16, 21, "ng-template", 13)(18, QaApiSimulationPanelComponent_ng_template_18_Template, 5, 0, "ng-template", 14);
      \u0275\u0275elementEnd()()();
      \u0275\u0275template(19, QaApiSimulationPanelComponent_ng_template_19_Template, 2, 1, "ng-template", null, 0, \u0275\u0275templateRefExtractor);
      \u0275\u0275elementStart(21, "p-dialog", 15);
      \u0275\u0275pipe(22, "uppercase");
      \u0275\u0275listener("visibleChange", function QaApiSimulationPanelComponent_Template_p_dialog_visibleChange_21_listener() {
        return ctx.selectedResult.set(null);
      });
      \u0275\u0275conditionalCreate(23, QaApiSimulationPanelComponent_Conditional_23_Template, 42, 19, "div", 16);
      \u0275\u0275elementEnd();
    }
    if (rf & 2) {
      let tmp_6_0;
      let tmp_11_0;
      \u0275\u0275advance(11);
      \u0275\u0275property("loading", ctx.isVerifying())("disabled", ctx.availableApis().length === 0);
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.results().length > 0 ? 12 : -1);
      \u0275\u0275advance(2);
      \u0275\u0275property("value", ctx.availableApis())("tableStyle", \u0275\u0275pureFunction0(13, _c05));
      \u0275\u0275advance(7);
      \u0275\u0275property("header", (((tmp_6_0 = ctx.selectedResult()) == null ? null : tmp_6_0.entry == null ? null : tmp_6_0.entry.name) ?? "API") + " \xB7 " + (((tmp_6_0 = ctx.selectedResult()) == null ? null : tmp_6_0.httpMethod) ?? "") + " " + \u0275\u0275pipeBind1(22, 11, ((tmp_6_0 = ctx.selectedResult()) == null ? null : tmp_6_0.operation) ?? ""))("visible", ctx.selectedResult() !== null)("modal", true)("dismissableMask", true)("draggable", false);
      \u0275\u0275advance(2);
      \u0275\u0275conditional((tmp_11_0 = ctx.selectedResult()) ? 23 : -1, tmp_11_0);
    }
  }, dependencies: [CommonModule, NgClass, NgTemplateOutlet, ButtonModule, Button, PrimeTemplate, TagModule, Tag, TableModule, Table, DialogModule, Dialog, TooltipModule, Tooltip, UpperCasePipe, JsonPipe], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(QaApiSimulationPanelComponent, [{
    type: Component,
    args: [{ selector: "app-qa-api-simulation-panel", imports: [
      CommonModule,
      ButtonModule,
      TagModule,
      TableModule,
      DialogModule,
      TooltipModule
    ], template: `<div class="flex flex-col gap-6 mt-6">
  <!-- Header -->
  <div
    class="flex items-center justify-between border-b pb-4 border-gray-100 dark:border-gray-700"
  >
    <div>
      ...
      <p class="text-sm text-gray-500">
        Hits the real backend with
        <code class="text-[11px] bg-gray-100 dark:bg-gray-700 px-1 rounded"
          >x-auth-testing: true</code
        >
        to verify create / update / delete access without persisting data.
      </p>
    </div>
  ...
  @if (results().length > 0) {
    <div
      class="flex flex-wrap gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl"
    >
      <div class="flex items-center gap-2">
        <span class="text-xs font-bold text-gray-500 uppercase">Tested:</span>
        <p-tag [value]="stats().total.toString()" severity="info"></p-tag>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-xs font-bold text-gray-500 uppercase">Allowed:</span>
        <p-tag [value]="stats().allowed.toString()" severity="success"></p-tag>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-xs font-bold text-gray-500 uppercase"
          >Forbidden:</span
        >
        <p-tag [value]="stats().forbidden.toString()" severity="danger"></p-tag>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-xs font-bold text-gray-500 uppercase">Errors:</span>
        <p-tag [value]="stats().errors.toString()" severity="warn"></p-tag>
      </div>
      <div class="ml-auto flex items-center gap-2">
        ...
        <p-tag
          ...
          [severity]="stats().mismatches > 0 ? 'danger' : 'success'"
        ></p-tag>
      </div>
    </div>
  }
  ...
  >
    ...
    >
      <ng-template pTemplate="groupheader" let-entry>
        <tr class="p-rowgroup-header bg-gray-50 dark:bg-gray-800/50 uppercase">
          <td colspan="6">
            <span class="font-bold text-gray-500 text-xs tracking-wider">{{
              entry.domain
            }}</span>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            <ng-container
              *ngTemplateOutlet="
                opCell;
                context: {
                  $implicit: getResultFor(entry.id, 'list'),
                  op: 'GET',
                }
              "
            ></ng-container>
          ...
          <td>
            <ng-container
              *ngTemplateOutlet="
                opCell;
                context: {
                  $implicit: getResultFor(entry.id, 'create'),
                  op: 'POST',
                }
              "
            ></ng-container>
          ...
          <td>
            <ng-container
              *ngTemplateOutlet="
                opCell;
                context: {
                  $implicit: getResultFor(entry.id, 'update'),
                  op: 'PUT',
                }
              "
            ></ng-container>
          ...
          <td>
            <ng-container
              *ngTemplateOutlet="
                opCell;
                context: {
                  $implicit: getResultFor(entry.id, 'delete'),
                  op: 'DELETE',
                }
              "
            ></ng-container>
          ...
          <td>
            <p-button
              ...
              size="small"
              [loading]="isVerifying()"
              (onClick)="runSingleVerification(entry)"
              pTooltip="Verify this entity"
            ></p-button>
          </td>
        </tr>
      </ng-template>
      ...
      <ng-template pTemplate="emptymessage">
        <tr>
          <td colspan="5" class="p-8 text-center bg-gray-50 dark:bg-gray-800">
            <i class="pi pi-info-circle text-4xl text-gray-300 mb-4"></i>
            <p class="text-gray-500">
              No APIs are available based on the current simulated permissions.
              Toggle permissions above or switch persona to unlock APIs.
            </p>
          </td>
        </tr>
      </ng-template>
...
<!-- \u2500\u2500 Reusable cell template for each operation column \u2500\u2500 -->
<ng-template #opCell let-result let-op="op">
  @if (result) {
    <button
      type="button"
      class="flex flex-col gap-1 items-start cursor-pointer bg-transparent border-none p-0 text-left"
      (click)="showDetails(result)"
    >
      <div class="flex items-center gap-1">
        <p-tag
          [severity]="getAccessSeverity(result.backendAccess)"
          [value]="result.backendAccess"
        ></p-tag>
        @if (isMismatch(result)) {
          <i
            class="pi pi-exclamation-triangle text-yellow-500 text-xs"
            pTooltip="Frontend/Backend mismatch!"
          ></i>
        }
      </div>
      <span class="text-[10px] text-gray-400 font-mono"
        >{{ result.durationMs }}ms \xB7 {{ op }}</span
      >
      @if (isMismatch(result)) {
        <span class="text-[10px] text-yellow-600 dark:text-yellow-400 font-bold"
          >Expected: {{ result.frontendExpected }}</span
        >
      }
      @if (result.backendAccess === 'Error') {
        <span
          class="text-[10px] text-red-500 truncate w-32"
          [title]="result.errorDetails ?? ''"
          >{{ result.errorDetails }}</span
        >
      }
    </button>
  } @else {
...
<p-dialog
  [header]="
    (selectedResult()?.entry?.name ?? 'API') +
    ' \xB7 ' +
    (selectedResult()?.httpMethod ?? '') +
    ' ' +
    (selectedResult()?.operation ?? '' | uppercase)
  "
  [visible]="selectedResult() !== null"
  [modal]="true"
  [dismissableMask]="true"
  styleClass="w-full md:w-[50vw]"
  (visibleChange)="selectedResult.set(null)"
  [draggable]="false"
>
  @if (selectedResult(); as r) {
    <div class="flex flex-col gap-4">
      ...
      <div
        ...
        [ngClass]="{
          'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800':
            r.backendAccess === 'Allowed' && !isMismatch(r),
          'bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800':
            r.backendAccess === 'Forbidden' || isMismatch(r),
          'bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800':
            r.backendAccess === 'Error',
        }"
      >
        <div class="flex-1">
          <div class="flex items-center gap-2 text-sm font-bold">
            @if (isMismatch(r)) {
              ...
              <span class="text-yellow-700 dark:text-yellow-400">MISMATCH</span>
            } @else {
              <i
                class="pi pi-check-circle"
                [ngClass]="{
                  'text-green-500': r.backendAccess === 'Allowed',
                  'text-red-500': r.backendAccess === 'Forbidden',
                  'text-yellow-500': r.backendAccess === 'Error',
                }"
              ></i>
              <span>MATCH</span>
            }
          ...
          <div class="text-xs text-gray-500 mt-1">
            Frontend expected:
            <span class="font-bold">{{ r.frontendExpected }}</span> \xB7 Backend
            returned:
            <span class="font-bold">{{ r.backendAccess }}</span>
          </div>
        </div>
        <span class="text-xs text-gray-400 font-mono"
          >{{ r.durationMs }}ms</span
        >
      </div>

      <!-- Request Info -->
      <div
        class="p-3 bg-gray-50 border border-gray-200 dark:bg-gray-800 dark:border-gray-700 rounded-lg"
      >
        <h4 class="text-xs font-bold text-gray-500 uppercase mb-2">Request</h4>
        <div
          class="text-sm font-mono break-all text-gray-700 dark:text-gray-300"
        >
          <p>
            <span class="font-bold text-primary">{{ r.httpMethod }}</span>
            {{ r.url }}
          </p>
          <p class="mt-2">
          ...
          <pre
            class="bg-white dark:bg-gray-900 border dark:border-gray-600 p-2 text-xs rounded mt-1"
            >{{ r.headersSent | json }}</pre
          >
        </div>
      </div>
      ...
      >
        ...
        @if (r.requestBody) {
          <div class="max-h-48 overflow-y-auto w-full">
            <pre
              class="text-xs font-mono bg-white dark:bg-gray-900 border dark:border-gray-600 p-2 rounded m-0 break-words whitespace-pre-wrap"
              >{{ r.requestBody | json }}</pre
            >
          </div>
        } @else {
          <p class="text-sm text-gray-400 italic">No body (DELETE request)</p>
        }
      </div>
      ...
      >
        <h4
          class="text-xs font-bold text-gray-500 uppercase mb-2 flex items-center justify-between"
        >
          <span>Response</span>
          <p-tag
            [severity]="getAccessSeverity(r.backendAccess)"
            [value]="r.backendAccess"
            [rounded]="true"
          ></p-tag>
        </h4>
        @if (r.responseBody) {
          <div class="max-h-48 overflow-y-auto w-full">
            <pre
              class="text-xs font-mono bg-white dark:bg-gray-900 border dark:border-gray-600 p-2 rounded m-0 break-words whitespace-pre-wrap"
              >{{ r.responseBody | json }}</pre
            >
          </div>
        } @else if (r.errorDetails) {
          <p class="text-sm text-red-500">{{ r.errorDetails }}</p>
        } @else {
          <p class="text-sm text-gray-400 italic">Empty response</p>
        }
      </div>
    </div>
  }
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(QaApiSimulationPanelComponent, { className: "QaApiSimulationPanelComponent", filePath: "src/app/shared/components/qa-permission-tester/components/qa-api-simulation-panel/qa-api-simulation-panel.component.ts", lineNumber: 31 });
})();

// src/app/shared/components/qa-permission-tester/components/qa-api-verification-tab/qa-api-verification-tab.component.ts
var QaApiVerificationTabComponent = class _QaApiVerificationTabComponent {
  static \u0275fac = function QaApiVerificationTabComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _QaApiVerificationTabComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _QaApiVerificationTabComponent, selectors: [["app-qa-api-verification-tab"]], decls: 12, vars: 0, consts: [[1, "flex", "flex-col", "gap-6", "pt-4"], [1, "p-4", "rounded-xl", "border", "border-blue-100", "dark:border-blue-800", "flex", "items-start", "gap-4"], [1, "w-8", "h-8", "rounded-full", "flex", "items-center", "justify-center", "flex-shrink-0"], [1, "pi", "pi-info-circle", "text-blue-600", "dark:text-blue-400"], [1, "text-xs", "leading-relaxed"], [1, "font-bold", "mb-1"], [1, "bg-white/50", "dark:bg-black/20", "p-0.5", "rounded"]], template: function QaApiVerificationTabComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "div", 0);
      \u0275\u0275element(1, "app-qa-api-simulation-panel");
      \u0275\u0275elementStart(2, "div", 1)(3, "div", 2);
      \u0275\u0275element(4, "i", 3);
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(5, "div", 4)(6, "p", 5);
      \u0275\u0275text(7, "Security Enforcement Guard:");
      \u0275\u0275elementEnd();
      \u0275\u0275text(8, " This tool uses the ");
      \u0275\u0275elementStart(9, "code", 6);
      \u0275\u0275text(10, "x-auth-testing");
      \u0275\u0275elementEnd();
      \u0275\u0275text(11, " bypass header to verify rules without triggering side effects like auditing or resource modification. Each endpoint is tested with the current logged-in user's real access token. Mismatches indicate a lack of backend enforcement for the checked permission. ");
      \u0275\u0275elementEnd()()();
    }
  }, dependencies: [CommonModule, QaApiSimulationPanelComponent], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(QaApiVerificationTabComponent, [{
    type: Component,
    args: [{ selector: "app-qa-api-verification-tab", imports: [CommonModule, QaApiSimulationPanelComponent], template: `<div class="flex flex-col gap-6 pt-4">
  <!-- CRUD Permission Verification Panel -->
  <app-qa-api-simulation-panel></app-qa-api-simulation-panel>

  <!-- Security info banner -->
  <div
    class="p-4 rounded-xl border border-blue-100 dark:border-blue-800 flex items-start gap-4"
  >
    <div
    ...
    <div class="text-xs leading-relaxed">
      <p class="font-bold mb-1">Security Enforcement Guard:</p>
      This tool uses the
      <code class="bg-white/50 dark:bg-black/20 p-0.5 rounded"
        >x-auth-testing</code
      >
      bypass header to verify rules without triggering side effects like
      auditing or resource modification. Each endpoint is tested with the
      current logged-in user's real access token. Mismatches indicate a lack of
      backend enforcement for the checked permission.
    </div>
  </div>
</div>
` }]
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(QaApiVerificationTabComponent, { className: "QaApiVerificationTabComponent", filePath: "src/app/shared/components/qa-permission-tester/components/qa-api-verification-tab/qa-api-verification-tab.component.ts", lineNumber: 10 });
})();

// src/app/shared/components/qa-permission-tester/qa-permission-tester.component.ts
var QaPermissionTesterComponent = class _QaPermissionTesterComponent {
  ...
  apiVerificationService = inject(ApiVerificationService);
  // Selected profile for dropdown
  selectedProfileId = signal(null, ...ngDevMode ? [{ debugName: "selectedProfileId" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  // Verification state
  verificationResults = signal([], ...ngDevMode ? [{ debugName: "verificationResults" }] : (
    /* istanbul ignore next */
    []
  ));
  isVerifying = signal(false, ...ngDevMode ? [{ debugName: "isVerifying" }] : (
  ...
  // Statistics
  verificationStats = computed(() => {
    const results = this.verificationResults();
    const total = results.length;
    const allowed = results.filter((r85) => r85.backendAccess === "Allowed").length;
    const forbidden = results.filter((r85) => r85.backendAccess === "Forbidden").length;
    const errors = results.filter((r85) => r85.backendAccess === "Error").length;
    const mismatches = results.filter((r85) => r85.backendAccess === "Allowed" !== r85.frontendExpected).length;
    return { total, allowed, forbidden, errors, mismatches };
  }, ...ngDevMode ? [{ debugName: "verificationStats" }] : (
    /* istanbul ignore next */
    []
  ));
  // Filtered Results
  filteredResults = computed(() => {
    ...
    if (query) {
      results = results.filter((r85) => r85.endpoint.toLowerCase().includes(query) || r85.permission.toLowerCase().includes(query) || r85.method.toLowerCase().includes(query));
    }
    if (status !== "all") {
      if (status === "Mismatch") {
        results = results.filter((r85) => r85.backendAccess === "Allowed" !== r85.frontendExpected);
      } else {
        results = results.filter((r85) => r85.backendAccess === status);
      }
    }
    return results;
  ...
  groupedResults = computed(() => {
    ...
    const groups = {};
    results.forEach((r85) => {
      const module = r85.endpoint.split("/")[2] || "other";
      if (isNullOrUndefined(groups[module]))
        groups[module] = [];
      groups[module].push(r85);
    });
    return Object.entries(groups).map(([name, items]) => ({ name, items }));
  }, ...ngDevMode ? [{ debugName: "groupedResults" }] : (
    /* istanbul ignore next */
    []
  ));
  // Actual owner type (ignoring QA overrides)
  realOwnerType = computed(() => this.authDataService.getRealUserOwnerType(), ...ngDevMode ? [{ debugName: "realOwnerType" }] : (
    /* istanbul ignore next */
    []
  ));
  // Current owner type (respecting QA overrides)
  ownerType = computed(() => this.authDataService.getUserOwnerType(), ...ngDevMode ? [{ debugName: "ownerType" }] : (
    /* istanbul ignore next */
    []
  ));
  // Available roles for dropdown
  roles = this.qaTools.availableRoles;
  isLoading = this.qaTools.isLoadingRoles;
  ...
  clientFeatureAccess = computed(() => {
    this.authDataService.userPermissions();
    return this.computeFeatureAccess(CLIENT_ADMIN_FEATURES, "client");
  }, ...ngDevMode ? [{ debugName: "clientFeatureAccess" }] : (
    /* istanbul ignore next */
    []
  ));
  vendorFeatureAccess = computed(() => {
    this.authDataService.userPermissions();
    return this.computeFeatureAccess(VENDOR_ADMIN_FEATURES, "vendor");
  }, ...ngDevMode ? [{ debugName: "vendorFeatureAccess" }] : (
    /* istanbul ignore next */
    []
  ));
  distributorFeatureAccess = computed(() => {
    this.authDataService.userPermissions();
    return this.computeFeatureAccess(DISTRIBUTOR_ADMIN_FEATURES, "distributor");
  }, ...ngDevMode ? [{ debugName: "distributorFeatureAccess" }] : (
    /* istanbul ignore next */
    []
  ));
  // Permission toggles (scoped to real owner tier)
  togglePermissions = computed(() => this.collectPermissionsByOwner(this.realOwnerType()), ...ngDevMode ? [{ debugName: "togglePermissions" }] : (
    /* istanbul ignore next */
    []
  ));
  // Statistics for features
  clientStats = computed(() => this.computeStats(this.clientFeatureAccess()), ...ngDevMode ? [{ debugName: "clientStats" }] : (
    /* istanbul ignore next */
    []
  ));
  vendorStats = computed(() => this.computeStats(this.vendorFeatureAccess()), ...ngDevMode ? [{ debugName: "vendorStats" }] : (
    /* istanbul ignore next */
    []
  ));
  distributorStats = computed(() => this.computeStats(this.distributorFeatureAccess()), ...ngDevMode ? [{ debugName: "distributorStats" }] : (
    /* istanbul ignore next */
    []
  ));
  // Hierarchical Testing Signals (Exposed from QAToolsService)
  selectedDistributorId = this.qaTools.selectedDistributorId;
  selectedVendorId = this.qaTools.selectedVendorId;
  selectedClientId = this.qaTools.selectedClientId;
  availableDistributors = this.qaTools.availableDistributors;
  availableVendors = this.qaTools.availableVendors;
  availableClients = this.qaTools.availableClients;
  constructor() {
    const personaValue = this.qaTools.activePersona();
    if (personaValue) {
      this.selectedProfileId.set(personaValue.id);
    }
    effect(async () => {
      try {
  ...
  canViewContext(context) {
    ...
    if (currentOwner === null || !(context in rank) || !(currentOwner in rank)) {
      return false;
    }
  ...
  lastVerifiedLevel = signal(null, ...ngDevMode ? [{ debugName: "lastVerifiedLevel" }] : (
    /* istanbul ignore next */
    []
  ));
  /**
   ...
   */
  async runApiVerification(level) {
    const targetLevel = level ?? this.realOwnerType();
    this.isVerifying.set(true);
    try {
      let targetOwnerId;
      if (level === "distributor")
        targetOwnerId = this.selectedDistributorId() ?? void 0;
      else if (level === "vendor")
        targetOwnerId = this.selectedVendorId() ?? void 0;
      else if (level === "client")
        targetOwnerId = this.selectedClientId() ?? void 0;
      const results = await this.apiVerificationService.verifyBackEndEnforcement(targetOwnerId, targetLevel);
      this.verificationResults.set(results);
      this.lastVerifiedLevel.set(targetLevel);
    } catch (err) {
      console.error("QA Tools: API Verification failed:", err);
    } finally {
      this.isVerifying.set(false);
    }
  }
   ...
   */
  computeFeatureAccess(featuresConfig, context) {
   ...
   */
  checkFeatureAccess(config, _context) {
   ...
   */
  evaluatePermissionExpression(expression, permissions) {
    ...
    if (typeof expression === "object" && expression !== null) {
      ...
      if ("and" in query && Array.isArray(query["and"])) {
        return query["and"].every((cond) => this.evaluatePermissionExpression(cond, permissions));
      }
      if ("or" in query && Array.isArray(query["or"])) {
        return query["or"].some((cond) => this.evaluatePermissionExpression(cond, permissions));
      }
      if ("not" in query && query["not"] !== void 0) {
        return !this.evaluatePermissionExpression(query["not"], permissions);
      }
    }
    return false;
  }
   ...
   */
  formatPermissions(config) {
    if (!("permissions" in config)) {
      return "No permissions required";
    }
    const perms = config.permissions;
    if (perms === null || perms === void 0) {
      return "No permissions required";
    }
  ...
  computeStats(features) {
    ...
    const accessible = features.filter((f11) => f11.hasAccess).length;
    const restricted = total - accessible;
    return { total, accessible, restricted };
  }
  collectPermissionsByOwner(ownerType) {
    if (!ownerType) {
      return [];
    }
    ...
    return [...permissions].sort((a43, b5) => a43.localeCompare(b5));
  }
  extractPermissionsFromConfig(config) {
    const permissions = [];
    for (const feature of Object.values(config)) {
      const permissionsValue = "permissions" in feature ? feature.permissions : void 0;
      if (permissionsValue !== null && permissionsValue !== void 0) {
        permissions.push(...this.flattenPermissions(permissionsValue));
      }
    }
    return permissions;
  }
  flattenPermissions(expression) {
    if (typeof expression === "string") {
      return [expression];
    }
    if (Array.isArray(expression)) {
    ...
    if (isDefined(expression.and)) {
      permissions.push(...expression.and.flatMap((value) => this.flattenPermissions(value)));
    }
    if (isDefined(expression.or)) {
      permissions.push(...expression.or.flatMap((value) => this.flattenPermissions(value)));
    }
  ...
  static \u0275fac = function QaPermissionTesterComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _QaPermissionTesterComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _QaPermissionTesterComponent, selectors: [["app-qa-permission-tester"]], decls: 27, vars: 19, consts: [[1, "qa-permission-tester", "flex", "flex-col", "gap-4", "p-4", "border", "rounded-xl", "shadow-2xl", "bg-white", "dark:bg-gray-900", "overflow-hidden", "h-full"], [1, "flex", "items-center", "justify-between", "p-4", "bg-gray-50", "dark:bg-gray-800", "border-b", "border-gray-100", "dark:border-gray-700"], [1, "flex", "items-center", "gap-3"], [1, "p-2", "bg-primary/10", "rounded-lg"], [1, "pi", "pi-shield", "text-primary", "text-xl"], [1, "text-xl", "font-bold", "leading-tight"], [1, "text-xs", "text-gray-500", "font-medium"], [1, "flex-1", "overflow-y-auto", "px-4", "pb-4"], [3, "valueChange", "value"], [3, "value"], [1, "pi", "pi-desktop", "mr-2"], [1, "pi", "pi-shield", "mr-2"], [3, "selectedProfileIdChange", "saveRequested", "resetRequested", "applyProfileRequested", "togglePermissionRequested", "testMode", "activeProfileName", "isSaving", "roles", "togglePermissions", "checkPermission", "clientStats", "vendorStats", "distributorStats", "canViewContext", "clientFeatureAccess", "vendorFeatureAccess", "distributorFeatureAccess", "selectedProfileId"], [1, "p-3", "bg-gray-50", "dark:bg-gray-800", "border-t", "border-gray-100", "dark:border-gray-700", "flex", "justify-center"], [1, "text-[10px]", "font-bold", "text-gray-400", "uppercase", "tracking-widest"]], template: function QaPermissionTesterComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "div", 0)(1, "div", 1)(2, "div", 2)(3, "div", 3);
      \u0275\u0275element(4, "i", 4);
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(5, "div")(6, "h2", 5);
      \u0275\u0275text(7, "QA Permission Center");
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(8, "p", 6);
      \u0275\u0275text(9, " Verify frontend simulation and backend enforcement ");
      \u0275\u0275elementEnd()()()();
      \u0275\u0275elementStart(10, "div", 7)(11, "p-tabs", 8);
      \u0275\u0275twoWayListener("valueChange", function QaPermissionTesterComponent_Template_p_tabs_valueChange_11_listener($event) {
        \u0275\u0275twoWayBindingSet(ctx.activeTabIndex, $event) || (ctx.activeTabIndex = $event);
        return $event;
      });
      \u0275\u0275elementStart(12, "p-tablist")(13, "p-tab", 9);
      \u0275\u0275element(14, "i", 10);
      \u0275\u0275text(15, " Simulation ");
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(16, "p-tab", 9);
      \u0275\u0275element(17, "i", 11);
      \u0275\u0275text(18, " API Verification ");
      \u0275\u0275elementEnd()();
      \u0275\u0275elementStart(19, "p-tabpanels")(20, "p-tabpanel", 9)(21, "app-qa-simulation-tab", 12);
      \u0275\u0275twoWayListener("selectedProfileIdChange", function QaPermissionTesterComponent_Template_app_qa_simulation_tab_selectedProfileIdChange_21_listener($event) {
        \u0275\u0275twoWayBindingSet(ctx.selectedProfileId, $event) || (ctx.selectedProfileId = $event);
        return $event;
      });
      \u0275\u0275listener("saveRequested", function QaPermissionTesterComponent_Template_app_qa_simulation_tab_saveRequested_21_listener() {
        return ctx.saveToRole();
      })("resetRequested", function QaPermissionTesterComponent_Template_app_qa_simulation_tab_resetRequested_21_listener() {
        return ctx.resetPermissions();
      })("applyProfileRequested", function QaPermissionTesterComponent_Template_app_qa_simulation_tab_applyProfileRequested_21_listener() {
        return ctx.applyProfile();
      })("togglePermissionRequested", function QaPermissionTesterComponent_Template_app_qa_simulation_tab_togglePermissionRequested_21_listener($event) {
        return ctx.togglePermission($event.permission, $event.enabled);
      });
      \u0275\u0275elementEnd()();
      \u0275\u0275elementStart(22, "p-tabpanel", 9);
      \u0275\u0275element(23, "app-qa-api-verification-tab");
      \u0275\u0275elementEnd()()()();
      \u0275\u0275elementStart(24, "div", 13)(25, "p", 14);
      \u0275\u0275text(26, " CypherView QA Internal Infrastructure v2.1 ");
      \u0275\u0275elementEnd()()();
    }
    if (rf & 2) {
      let tmp_5_0;
      \u0275\u0275advance(11);
      \u0275\u0275twoWayProperty("value", ctx.activeTabIndex);
      \u0275\u0275advance(2);
      \u0275\u0275property("value", 0);
      \u0275\u0275advance(3);
      \u0275\u0275property("value", 1);
      ...
      \u0275\u0275property("value", 0);
      \u0275\u0275advance();
      \u0275\u0275property("testMode", ctx.qaTools.activePersona() !== null)("activeProfileName", ((tmp_5_0 = ctx.qaTools.activePersona()) == null ? null : tmp_5_0.name) || "Standard User")("isSaving", ctx.isSaving())("roles", ctx.qaTools.availableRoles())("togglePermissions", ctx.togglePermissions())("checkPermission", ctx.permissionService.canAccess.bind(ctx.permissionService))("clientStats", ctx.clientStats())("vendorStats", ctx.vendorStats())("distributorStats", ctx.distributorStats())("canViewContext", ctx.canViewContext.bind(ctx))("clientFeatureAccess", ctx.clientFeatureAccess())("vendorFeatureAccess", ctx.vendorFeatureAccess())("distributorFeatureAccess", ctx.distributorFeatureAccess());
      \u0275\u0275twoWayProperty("selectedProfileId", ctx.selectedProfileId);
      \u0275\u0275advance();
      \u0275\u0275property("value", 1);
    }
    ...
    TabList,
    Tab,
    QaSimulationTabComponent,
    QaApiVerificationTabComponent
  ], styles: ["\n[_nghost-%COMP%] {\n  display: block;\n  width: 100%;\n  height: 100%;\n}\n.qa-permission-tester[_ngcontent-%COMP%] {\n  -webkit-user-select: none;\n  user-select: none;\n}\n.qa-permission-toggle-list[_ngcontent-%COMP%] {\n  display: grid;\n  grid-template-columns: repeat(auto-fill, minmax(200px, 1fr));\n  gap: 0.5rem;\n}\n  .verification-results .p-accordion-panel {\n  margin-bottom: 0.5rem;\n  border: 1px solid var(--surface-border);\n  border-radius: 0.75rem;\n  overflow: hidden;\n}\n  .verification-results .p-accordion-panel .p-accordion-header {\n  background: transparent;\n  transition: all 0.2s ease;\n}\n  .verification-results .p-accordion-panel .p-accordion-header:hover {\n  background: rgba(var(--primary-color-rgb), 0.05);\n}\n  .verification-results .p-accordion-panel.p-accordion-panel-active {\n  border-color: var(--primary-color);\n}\n  .verification-results .p-accordion-panel.p-accordion-panel-active .p-accordion-header {\n  background: var(--surface-card);\n  border-bottom: 1px solid var(--surface-border);\n}\n  .verification-results .p-accordion-panel.p-accordion-panel-active .p-accordion-content {\n  padding: 1rem;\n}\n  .trace-accordion .p-accordion-panel {\n  box-shadow: none !important;\n  border: none !important;\n  margin-bottom: 0 !important;\n}\n  .trace-accordion .p-accordion-panel .p-accordion-header {\n  padding: 0.25rem 0.5rem;\n  border: none !important;\n  background: transparent !important;\n}\n  .trace-accordion .p-accordion-panel .p-accordion-content {\n  padding: 0.5rem !important;\n  border: none !important;\n  background: #000 !important;\n}\n.overflow-y-auto[_ngcontent-%COMP%]::-webkit-scrollbar {\n  width: 6px;\n}\n.overflow-y-auto[_ngcontent-%COMP%]::-webkit-scrollbar-track {\n  background: transparent;\n}\n.overflow-y-auto[_ngcontent-%COMP%]::-webkit-scrollbar-thumb {\n  background: #e2e8f0;\n  border-radius: 10px;\n}\n.overflow-y-auto[_ngcontent-%COMP%]:hover::-webkit-scrollbar-thumb {\n  background: #cbd5e1;\n}\n.dark[_ngcontent-%COMP%]   .overflow-y-auto[_ngcontent-%COMP%]::-webkit-scrollbar-thumb {\n  background: #334155;\n}\n/*# sourceMappingURL=qa-permission-tester.component.css.map */"] });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(QaPermissionTesterComponent, [{
    type: Component,
    args: [{ selector: "app-qa-permission-tester", imports: [
      CommonModule,
      TagModule,
      TabsModule,
      QaSimulationTabComponent,
  ...
  class="qa-permission-tester flex flex-col gap-4 p-4 border rounded-xl shadow-2xl bg-white dark:bg-gray-900 overflow-hidden h-full"
>
  <!-- Header with Close Icon -->
  <div
    class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700"
  >
    <div class="flex items-center gap-3">
      ...
      <div>
        <h2 class="text-xl font-bold leading-tight">QA Permission Center</h2>
        <p class="text-xs text-gray-500 font-medium">
          Verify frontend simulation and backend enforcement
        </p>
      </div>
    </div>
  </div>
  ...
  <div class="flex-1 overflow-y-auto px-4 pb-4">
    <p-tabs [(value)]="activeTabIndex">
      ...
      <p-tabpanels>
        <!-- Simulation Tab -->
        <p-tabpanel [value]="0">
          <app-qa-simulation-tab
            [testMode]="qaTools.activePersona() !== null"
            [activeProfileName]="
              qaTools.activePersona()?.name || 'Standard User'
            "
            [isSaving]="isSaving()"
            [roles]="qaTools.availableRoles()"
            [togglePermissions]="togglePermissions()"
            [checkPermission]="
              permissionService.canAccess.bind(permissionService)
            "
            [clientStats]="clientStats()"
            [vendorStats]="vendorStats()"
            [distributorStats]="distributorStats()"
            [canViewContext]="canViewContext.bind(this)"
            [clientFeatureAccess]="clientFeatureAccess()"
            [vendorFeatureAccess]="vendorFeatureAccess()"
            [distributorFeatureAccess]="distributorFeatureAccess()"
            [(selectedProfileId)]="selectedProfileId"
            (saveRequested)="saveToRole()"
            (resetRequested)="resetPermissions()"
            (applyProfileRequested)="applyProfile()"
            (togglePermissionRequested)="
              togglePermission($any($event).permission, $any($event).enabled)
            "
          ></app-qa-simulation-tab>
        </p-tabpanel>
        ...
        <p-tabpanel [value]="1">
          <app-qa-api-verification-tab></app-qa-api-verification-tab>
        </p-tabpanel>
      </p-tabpanels>
    </p-tabs>
  </div>
  ...
  <div
    class="p-3 bg-gray-50 dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 flex justify-center"
  >
    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
      CypherView QA Internal Infrastructure v2.1
    </p>
  </div>
</div>
`, styles: ["/* src/app/shared/components/qa-permission-tester/qa-permission-tester.component.scss */\n:host {\n  display: block;\n  width: 100%;\n  height: 100%;\n}\n.qa-permission-tester {\n  -webkit-user-select: none;\n  user-select: none;\n}\n.qa-permission-toggle-list {\n  display: grid;\n  grid-template-columns: repeat(auto-fill, minmax(200px, 1fr));\n  gap: 0.5rem;\n}\n::ng-deep .verification-results .p-accordion-panel {\n  margin-bottom: 0.5rem;\n  border: 1px solid var(--surface-border);\n  border-radius: 0.75rem;\n  overflow: hidden;\n}\n::ng-deep .verification-results .p-accordion-panel .p-accordion-header {\n  background: transparent;\n  transition: all 0.2s ease;\n}\n::ng-deep .verification-results .p-accordion-panel .p-accordion-header:hover {\n  background: rgba(var(--primary-color-rgb), 0.05);\n}\n::ng-deep .verification-results .p-accordion-panel.p-accordion-panel-active {\n  border-color: var(--primary-color);\n}\n::ng-deep .verification-results .p-accordion-panel.p-accordion-panel-active .p-accordion-header {\n  background: var(--surface-card);\n  border-bottom: 1px solid var(--surface-border);\n}\n::ng-deep .verification-results .p-accordion-panel.p-accordion-panel-active .p-accordion-content {\n  padding: 1rem;\n}\n::ng-deep .trace-accordion .p-accordion-panel {\n  box-shadow: none !important;\n  border: none !important;\n  margin-bottom: 0 !important;\n}\n::ng-deep .trace-accordion .p-accordion-panel .p-accordion-header {\n  padding: 0.25rem 0.5rem;\n  border: none !important;\n  background: transparent !important;\n}\n::ng-deep .trace-accordion .p-accordion-panel .p-accordion-content {\n  padding: 0.5rem !important;\n  border: none !important;\n  background: #000 !important;\n}\n.overflow-y-auto::-webkit-scrollbar {\n  width: 6px;\n}\n.overflow-y-auto::-webkit-scrollbar-track {\n  background: transparent;\n}\n.overflow-y-auto::-webkit-scrollbar-thumb {\n  background: #e2e8f0;\n  border-radius: 10px;\n}\n.overflow-y-auto:hover::-webkit-scrollbar-thumb {\n  background: #cbd5e1;\n}\n.dark .overflow-y-auto::-webkit-scrollbar-thumb {\n  background: #334155;\n}\n/*# sourceMappingURL=qa-permission-tester.component.css.map */\n"] }]
  }], () => [], null);
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(QaPermissionTesterComponent, { className: "QaPermissionTesterComponent", filePath: "src/app/shared/components/qa-permission-tester/qa-permission-tester.component.ts", lineNumber: 50 });
})();

// src/app/layouts/components/sidebar/sidebar.component.ts
var _c06 = (a0, a1, a210, a310) => ({ "w-16": a0, "w-aside": a1, "w-1/2": a210, open: a310 });
var _c14 = (a0) => ({ "cursor-not-allowed opacity-50": a0 });
function SidebarComponent_Conditional_0_Template(rf, ctx) {
  if (rf & 1) {
    const _r1 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "div", 0)(1, "i", 9);
...
function SidebarComponent_ng_template_5_Conditional_0_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1(" ", \u0275\u0275pipeBind1(2, 1, item_r3.label), " ");
  }
}
function SidebarComponent_ng_template_5_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275conditionalCreate(0, SidebarComponent_ng_template_5_Conditional_0_Template, 3, 3, "span", 10);
  }
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275conditional(!ctx_r1.isCollapsed() ? 0 : -1);
  }
}
function SidebarComponent_ng_template_6_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "app-menu-item", 11);
  }
  if (rf & 2) {
    const item_r4 = ctx.$implicit;
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275property("item", item_r4)("isCollapsed", ctx_r1.isCollapsed());
  }
}
function SidebarComponent_ng_template_8_Conditional_0_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span", 10);
    \u0275\u0275text(1);
    \u0275\u0275pipe(2, "uppercase");
  ...
  if (rf & 2) {
    ...
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1(" ", \u0275\u0275pipeBind1(2, 1, item_r5.label), " ");
  }
}
function SidebarComponent_ng_template_8_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275conditionalCreate(0, SidebarComponent_ng_template_8_Conditional_0_Template, 3, 3, "span", 10);
  }
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275conditional(!ctx_r1.isCollapsed() ? 0 : -1);
...
function SidebarComponent_ng_template_9_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275property("item", item_r6)("isCollapsed", ctx_r1.isCollapsed());
  }
}
function SidebarComponent_Conditional_12_Template(rf, ctx) {
  if (rf & 1) {
    const _r7 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "app-client-switcher", 12);
    \u0275\u0275listener("onClosed", function SidebarComponent_Conditional_12_Template_app_client_switcher_onClosed_0_listener() {
      \u0275\u0275restoreView(_r7);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.toggleSwitchClient());
    });
    \u0275\u0275elementEnd();
  }
}
function SidebarComponent_Conditional_13_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275elementStart(0, "app-account-switcher", 13);
    \u0275\u0275listener("closeModal", function SidebarComponent_Conditional_13_Template_app_account_switcher_closeModal_0_listener() {
      \u0275\u0275restoreView(_r8);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.toggleSwitchAccount());
    });
    \u0275\u0275elementEnd();
  }
}
var SidebarComponent = class _SidebarComponent {
  ...
  qaTools = inject(QAToolsService);
  // Computed menu items - automatically reactive to changes
  ...
  }, ...ngDevMode ? [{ debugName: "topMenuItems" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  }, ...ngDevMode ? [{ debugName: "bottomMenuItems" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  clientLogo = signal(null, ...ngDevMode ? [{ debugName: "clientLogo" }] : (
    /* istanbul ignore next */
    []
  ));
  isSwitchClient = signal(false, ...ngDevMode ? [{ debugName: "isSwitchClient" }] : (
    /* istanbul ignore next */
    []
  ));
  isSwitchAccount = signal(false, ...ngDevMode ? [{ debugName: "isSwitchAccount" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  addQaPermissionMenuItem(menuItems) {
    ...
    const developmentGroup = menuItems.find((item) => item?.["data"]?.["groupId"] === "development");
    if (developmentGroup) {
  ...
  reactToSelectedClientChanges() {
    ...
    effect(() => {
      ...
      this.clientsService.getClientLogo(clientId).pipe(take(1)).subscribe({
        next: (response) => {
  ...
  createUserMenuItem() {
    ...
    if (this.userSessionService.userSessions().length > 1) {
      items.push({
        label: this.translate.instant("NAV.MENU.SWITCH_ACCOUNT"),
        icon: "pi pi-arrow-right-arrow-left",
        command: () => this.toggleSwitchAccount()
      });
    }
    items.push({
      label: this.translate.instant("NAV.MENU.PROFILE"),
      icon: "pi pi-user",
      route: "/user/profile"
    });
    return {
      label: isDefined(userProfile) && hasProperty(userProfile, "name") && isString(userProfile.name) ? userProfile.name : this.translate.instant("NAV.MENU.USER_FALLBACK"),
      items
    };
  }
  createBottomMenuItems(recentClients = [], selectedClientId) {
    const accountNavigationItems = [];
    if (this.canListClients()) {
      accountNavigationItems.push({
        label: this.translate.instant("NAV.MENU.CLIENTS"),
        icon: "pi pi-search",
        command: () => this.toggleSwitchClient()
      });
    }
    const recentClientItems = recentClients.filter((client) => client.id !== selectedClientId?.toString()).slice(0, 3).map((client) => ({
      label: client.name,
      icon: "pi pi-clock",
      command: () => this.selectRecentClient(client.id, client.name)
    }));
    if (recentClientItems.length > 0) {
      accountNavigationItems.push(...recentClientItems);
    }
    accountNavigationItems.push({
      label: this.translate.instant("NAV.MENU.LOGOUT"),
    ...
    return [
      ...
      {
        label: this.translate.instant("NAV.MENU.ACCOUNT_NAVIGATION"),
   ...
   * Matches {@link ClientSwitcherComponent.selectClient}: no Flutter message is
   * sent — FlutterFrameService reboots the frame on client change and the new
   * client id reaches Flutter via the auth-token handshake.
   */
  selectRecentClient(clientId, clientName) {
    this.clientDataService.setSelectedClientById(clientId).then(() => {
      if (clientName.trim() !== "") {
        this.recentClientsService.addRecentClient(clientId, clientName);
      }
    }).catch((error) => {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _SidebarComponent, selectors: [["app-sidebar"]], features: [\u0275\u0275ProvidersFeature([DialogService])], decls: 14, vars: 26, consts: [[1, "p-4", "md:hidden"], [1, "flex", "top-0", "bottom-0", "left-0", "flex-col", "h-full", "sidebar-container", 3, "ngClass"], [3, "click", "keydown.enter", "isCollapsed", "userProfile", "clientLogo", "clientName", "ngClass", "pTooltip", "showDelay"], [1, "overflow-y-auto", "overflow-x-hidden", "w-full", "flex-1", "top-menu", 3, "model"], ["pTemplate", "submenuheader"], ["pTemplate", "item"], [1, "mt-auto", "w-full", 3, "model"], [3, "isCollapsed"], [3, "toggleChange", "isCollapsed"], [1, "text-2xl", "cursor-pointer", "pi", "pi-bars", 3, "click", "keydown.enter"], [1, "flex", "justify-center", "items-center", "text-xs", "opacity-50", "text-nav-client-font-color"], [3, "item", "isCollapsed"], [3, "onClosed"], [3, "closeModal"]], template: function SidebarComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275conditionalCreate(0, SidebarComponent_Conditional_0_Template, 2, 0, "div", 0);
      \u0275\u0275elementStart(1, "div", 1)(2, "app-user-profile", 2);
      \u0275\u0275pipe(3, "translate");
      \u0275\u0275listener("click", function SidebarComponent_Template_app_user_profile_click_2_listener() {
        return ctx.canListClients() && ctx.toggleSwitchClient();
      })("keydown.enter", function SidebarComponent_Template_app_user_profile_keydown_enter_2_listener() {
        return ctx.canListClients() && ctx.toggleSwitchClient();
      });
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(4, "p-menu", 3);
      \u0275\u0275template(5, SidebarComponent_ng_template_5_Template, 1, 1, "ng-template", 4)(6, SidebarComponent_ng_template_6_Template, 1, 2, "ng-template", 5);
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(7, "p-menu", 6);
      \u0275\u0275template(8, SidebarComponent_ng_template_8_Template, 1, 1, "ng-template", 4)(9, SidebarComponent_ng_template_9_Template, 1, 2, "ng-template", 5);
      \u0275\u0275elementEnd();
      \u0275\u0275element(10, "app-language-switcher", 7);
      \u0275\u0275elementStart(11, "app-toggle-button", 8);
      \u0275\u0275listener("toggleChange", function SidebarComponent_Template_app_toggle_button_toggleChange_11_listener() {
        return ctx.toggleSidebar();
      });
      \u0275\u0275elementEnd()();
      \u0275\u0275conditionalCreate(12, SidebarComponent_Conditional_12_Template, 1, 0, "app-client-switcher");
      \u0275\u0275conditionalCreate(13, SidebarComponent_Conditional_13_Template, 1, 0, "app-account-switcher");
    }
    if (rf & 2) {
      let tmp_6_0;
      \u0275\u0275conditional(!ctx.isSidebarOpen() ? 0 : -1);
      \u0275\u0275advance();
      \u0275\u0275property("ngClass", \u0275\u0275pureFunction4(19, _c06, ctx.isCollapsed(), !ctx.isCollapsed() && !ctx.isMobile(), ctx.isMobile(), ctx.isSidebarOpen()));
      \u0275\u0275advance();
      \u0275\u0275classProp("hover:cursor-pointer", ctx.canListClients());
      \u0275\u0275property("isCollapsed", ctx.isCollapsed())("userProfile", ctx.userProfileSignal())("clientLogo", ctx.clientLogo())("clientName", (tmp_6_0 = ctx.selectedClientSignal()) == null ? null : tmp_6_0.name)("ngClass", \u0275\u0275pureFunction1(24, _c14, !ctx.canListClients()))("pTooltip", \u0275\u0275pipeBind1(3, 17, ctx.canListClients() ? "NAV.SIDEBAR.SWITCH_CLIENT" : "NAV.SIDEBAR.NO_PERMISSION_SWITCH"))("showDelay", 1250);
      \u0275\u0275advance(2);
      ...
      \u0275\u0275property("isCollapsed", ctx.isCollapsed());
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.isSwitchClient() ? 12 : -1);
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.isSwitchAccount() ? 13 : -1);
    }
  }, dependencies: [
    BadgeModule,
    PrimeTemplate,
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(SidebarComponent, [{
    ...
    args: [{ selector: "app-sidebar", imports: [
      ...
      LanguageSwitcherComponent,
      TranslatePipe
    ], providers: [DialogService], template: `<!-- Hamburger Icon for Mobile -->
@if (!isSidebarOpen()) {
...
<div
  class="flex top-0 bottom-0 left-0 flex-col h-full sidebar-container"
  [ngClass]="{
    'w-16': isCollapsed(),
...
>
  <app-user-profile
    ...
    [pTooltip]="
      (canListClients()
        ? 'NAV.SIDEBAR.SWITCH_CLIENT'
        : 'NAV.SIDEBAR.NO_PERMISSION_SWITCH'
      ) | translate
    "
    [showDelay]="1250"
  ...
  <p-menu
    [model]="topMenuItems()"
    class="overflow-y-auto overflow-x-hidden w-full flex-1 top-menu"
  >
    <ng-template pTemplate="submenuheader" let-item>
      @if (!isCollapsed()) {
        <span
          class="flex justify-center items-center text-xs opacity-50 text-nav-client-font-color"
        >
  ...
  <!-- Language switcher -->
  <app-language-switcher [isCollapsed]="isCollapsed()" />
...
`, styles: ['/* src/app/layouts/components/sidebar/sidebar.component.scss */\n:host ::ng-deep .p-menu {\n  min-width: 3.5rem;\n  background: var(--navigation-background-color);\n  border: none;\n}\n:host ::ng-deep .p-menu-item-link {\n  color: var(--navigation-font-color);\n  font-size: var(--navigation-font-size);\n  font-weight: var(--navigation-font-weight);\n  text-transform: uppercase;\n  position: relative;\n  transition: all 0.2s ease;\n}\n:host ::ng-deep .p-menu-item-link:hover {\n  background-color: rgba(var(--color-primary-rgb, 66, 133, 244), 0.1) !important;\n}\n:host ::ng-deep .p-menu-item-link:hover .small-caps-text {\n  color: var(--navigation-font-color);\n}\n:host ::ng-deep .p-menuitem-link-active {\n  border-radius: 0 !important;\n  color: var(--color-primary-color) !important;\n  background: rgba(var(--color-primary-rgb, 66, 133, 244), 0.08) !important;\n}\n:host ::ng-deep .p-menuitem-link-active .small-caps-text {\n  color: var(--color-primary-color) !important;\n}\n:host ::ng-deep .p-menuitem-link-active::before {\n  content: "";\n  position: absolute;\n  top: 0;\n  right: 0;\n  bottom: 0;\n  width: 3px;\n  background-color: var(--color-primary-color);\n}\n:host ::ng-deep .p-menu-list {\n  padding: 0;\n}\n:host ::ng-deep .p-menu-item:not(.p-disabled) .p-menu-item-content:hover {\n  opacity: 1;\n  background: inherit;\n}\n:host ::ng-deep .p-menu-item.p-focus .p-menu-item-content {\n  background: inherit;\n}\n:host ::ng-deep .w-aside {\n  width: var(--width-aside);\n}\n.sidebar-container {\n  border-right: solid var(--navigation-separator-width) var(--navigation-separator-color);\n  border-color: var(--navigation-border-color);\n  border-width: var(--navigation-border-width);\n  background: var(--navigation-background-color);\n  transition: width 0.3s;\n}\n@media (max-width: 767px) {\n  .sidebar-container {\n    display: none;\n    z-index: 70;\n  }\n  .sidebar-container.open {\n    display: flex;\n    position: fixed;\n    top: 0;\n    left: 0;\n    right: 0;\n    bottom: 0;\n  }\n}\n/*# sourceMappingURL=sidebar.component.css.map */\n'] }]
  }], () => [], null);
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(SidebarComponent, { className: "SidebarComponent", filePath: "src/app/layouts/components/sidebar/sidebar.component.ts", lineNumber: 66 });
})();

// src/app/core/services/flutter-frame.service.ts
var FlutterFrameService = class _FlutterFrameService {
  /** Delay before booting the frame in the background on a non-Flutter route */
  static WARM_UP_DELAY_MS = 2e3;
  static MAX_INITIALIZATION_ATTEMPTS = 5;
  static RETRY_DELAY = 2e3;
  // 2 seconds
  ...
  destroyRef = inject(DestroyRef);
  /**
   * The iframe src. Refreshed on every activation so each boot carries a
   * fresh frameId query param, which addresses this frame on the shared
   * Angular<->Flutter channel (other tabs' frames ignore its messages).
   */
  safeUrl;
  /** Whether the iframe currently exists in the DOM */
  active = signal(false, ...ngDevMode ? [{ debugName: "active" }] : (
    /* istanbul ignore next */
    []
  ));
  /** Whether the current iframe's document has loaded (boot feedback) */
  frameLoaded = signal(false, ...ngDevMode ? [{ debugName: "frameLoaded" }] : (
    /* istanbul ignore next */
    []
  ));
  frameVisible = signal(false, ...ngDevMode ? [{ debugName: "frameVisible" }] : (
    /* istanbul ignore next */
    []
  ));
  anchorRect = signal(null, ...ngDevMode ? [{ debugName: "anchorRect" }] : (
    /* istanbul ignore next */
    []
  ));
  containerStyle = computed(() => {
    ...
    if (this.frameVisible() && rect !== null) {
      style2["display"] = "block";
      style2["left"] = `${rect.left}px`;
      style2["top"] = `${rect.top}px`;
      style2["width"] = `${rect.width}px`;
      style2["height"] = `${rect.height}px`;
    }
    return style2;
  ...
  pendingTokenSends = /* @__PURE__ */ new Set();
  boundHandleIframeLoad = this.handleIframeLoad.bind(this);
  boundSyncToAnchor = () => this.syncToAnchor();
  flutterLoaded = false;
  initializationAttempts = 0;
  frameClientId = null;
  currentFrameId = null;
  constructor() {
    this.safeUrl = signal(this.sanitizer.bypassSecurityTrustResourceUrl(environment.flutterUrl), ...ngDevMode ? [{ debugName: "safeUrl" }] : (
      /* istanbul ignore next */
      []
    ));
    window.addEventListener("resize", this.boundSyncToAnchor);
    this.destroyRef.onDestroy(() => {
      window.removeEventListener("resize", this.boundSyncToAnchor);
      this.destroyFrame();
      this.detachCurrentAnchor();
    });
    this.router.events.pipe(filter((event) => event instanceof NavigationEnd), takeUntilDestroyed()).subscribe((event) => {
    ...
    effect(() => {
      const clientId = this.clientDataService.getSelectedClientSignal()()?.id ?? null;
      if (!this.active() || clientId === null) {
        return;
      }
      if (this.frameClientId === null) {
        this.frameClientId = clientId;
        return;
      }
      if (clientId !== this.frameClientId) {
        this.destroyFrame();
  ...
  /**
   * Registers the layout element the iframe should be positioned over.
   * While the anchor is hidden (display: none) or absent, the frame is hidden
   * but kept alive.
   */
  attachAnchor(anchor) {
    this.detachCurrentAnchor();
    this.anchor = anchor;
    this.resizeObserver = new ResizeObserver(this.boundSyncToAnchor);
    this.resizeObserver.observe(anchor);
    this.syncToAnchor();
    if (!this.active()) {
      this.scheduleActivation(_FlutterFrameService.WARM_UP_DELAY_MS);
    }
  }
  detachAnchor(anchor) {
  ...
  /** Called by FlutterFrameComponent when the iframe's document has loaded. */
  onFrameLoad() {
    this.frameLoaded.set(true);
  }
  /**
   * Removes the iframe entirely. The next activation boots Flutter from
   * scratch. Called on logout and on client change.
   */
  destroyFrame() {
    this.cancelScheduledActivation();
    if (!this.active()) {
      return;
    }
    this.stopBootMessaging();
    this.frameClientId = null;
  ...
  syncToAnchor() {
    ...
    this.anchorRect.set(isVisible ? {
      left: rect.left,
      top: rect.top,
      width: rect.width,
      height: rect.height
    } : null);
    this.setFrameVisible(isVisible);
  }
  setFrameVisible(visible) {
    if (this.frameVisible() === visible) {
      return;
    }
    this.frameVisible.set(visible);
    if (visible && !this.active()) {
      this.activate();
      return;
    }
    if (this.active()) {
      this.messageService.sendAction(ActionType.visibility_changed, {
        visible,
        timestamp: Date.now()
      });
    }
  }
  scheduleActivation(delayMs) {
    this.cancelScheduledActivation();
    this.activationTimeout = setTimeout(() => {
      this.activationTimeout = null;
      if (this.anchor !== null && !this.active()) {
        this.activate();
      }
    }, delayMs);
  }
  ...
  activate() {
    ...
    this.cancelScheduledActivation();
    this.frameClientId = this.clientDataService.getSelectedClient()?.id ?? null;
    this.flutterLoaded = false;
    this.initializationAttempts = 0;
    this.frameLoaded.set(false);
    this.currentFrameId = crypto.randomUUID();
    this.messageService.setCurrentFrameId(this.currentFrameId);
    this.safeUrl.set(this.sanitizer.bypassSecurityTrustResourceUrl(this.buildFrameUrl(this.currentFrameId)));
    this.active.set(true);
    this.startBootMessaging();
  }
  buildFrameUrl(frameId) {
    const separator = environment.flutterUrl.includes("?") ? "&" : "?";
    return `${environment.flutterUrl}${separator}frameId=${frameId}`;
  }
  startBootMessaging() {
    window.addEventListener("message", this.boundHandleIframeLoad);
    let lastReadyTimestamp = 0;
    const subscription = new Subscription();
    subscription.add(this.messageService.messages$.subscribe((messages) => {
    ...
    subscription.add(this.messageService.connectionStatus$.subscribe((status) => {
    ...
    subscription.add(this.createFallbackTimer());
    this.bootSubscription = subscription;
  }
  stopBootMessaging() {
    window.removeEventListener("message", this.boundHandleIframeLoad);
    this.bootSubscription?.unsubscribe();
    this.bootSubscription = null;
    for (const handle of this.pendingTokenSends) {
      clearTimeout(handle);
    }
    this.pendingTokenSends.clear();
    this.flutterLoaded = false;
    this.initializationAttempts = 0;
  }
  ...
  sendAuthTokensToFlutter(delayMs = 500) {
    const handle = setTimeout(() => {
      this.pendingTokenSends.delete(handle);
      this.messageService.sendAuthTokensToFlutter();
    }, delayMs);
    this.pendingTokenSends.add(handle);
  }
  createFallbackTimer() {
    const fallbackTimer = timer(5e3, _FlutterFrameService.RETRY_DELAY).subscribe(() => {
      if (this.initializationAttempts >= _FlutterFrameService.MAX_INITIALIZATION_ATTEMPTS) {
  ...
  static \u0275fac = function FlutterFrameService_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _FlutterFrameService)();
  };
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _FlutterFrameService, factory: _FlutterFrameService.\u0275fac, providedIn: "root" });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(FlutterFrameService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
var FlutterLayoutComponent = class _FlutterLayoutComponent {
  frameService = inject(FlutterFrameService);
  elementRef = inject(ElementRef);
  ngOnInit() {
    this.frameService.attachAnchor(this.elementRef.nativeElement);
  }
  ngOnDestroy() {
    this.frameService.detachAnchor(this.elementRef.nativeElement);
  }
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _FlutterLayoutComponent, selectors: [["app-flutter-layout"]], decls: 0, vars: 0, template: function FlutterLayoutComponent_Template(rf, ctx) {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(FlutterLayoutComponent, [{
    type: Component,
    args: [{ selector: "app-flutter-layout", template: "", styles: ["/* angular:styles/component:scss;1f4b1abed7f7811421e32793bd7b13adbbd593ca9524d18b4d967220ed74944d;/opt/atlassian/pipelines/agent/build/src/app/layouts/components/flutter-layout/flutter-layout.component.ts */\n:host {\n  display: block;\n  width: 100%;\n  height: 100%;\n}\n/*# sourceMappingURL=flutter-layout.component.css.map */\n"] }]
  }], null, null);
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(FlutterLayoutComponent, { className: "FlutterLayoutComponent", filePath: "src/app/layouts/components/flutter-layout/flutter-layout.component.ts", lineNumber: 24 });
})();

// src/app/layouts/components/main-layout/main-layout.component.ts
var _c07 = (a0, a1) => ({ hidden: a0, block: a1 });
var _c15 = (a0) => ({ profile: a0 });
function MainLayoutComponent_Conditional_3_Template(rf, ctx) {
  if (rf & 1) {
    const _r1 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "div", 8);
    \u0275\u0275listener("click", function MainLayoutComponent_Conditional_3_Template_div_click_0_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.toggleSidebar());
    });
...
function MainLayoutComponent_Conditional_5_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275pipe(8, "translate");
    \u0275\u0275elementEnd()();
    \u0275\u0275elementStart(9, "div", 9)(10, "button", 13);
    \u0275\u0275listener("click", function MainLayoutComponent_Conditional_5_Template_button_click_10_listener() {
      \u0275\u0275restoreView(_r3);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.openQAPanel());
    });
    \u0275\u0275element(11, "i", 14);
    \u0275\u0275text(12);
    \u0275\u0275pipe(13, "translate");
    ...
    \u0275\u0275listener("click", function MainLayoutComponent_Conditional_5_Template_button_click_14_listener() {
      \u0275\u0275restoreView(_r3);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.qaTools.resetQAState());
    });
    \u0275\u0275element(15, "i", 16);
    \u0275\u0275text(16);
    \u0275\u0275pipe(17, "translate");
  ...
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275advance(4);
    \u0275\u0275textInterpolate(\u0275\u0275pipeBind1(5, 4, "LAYOUT.MAIN.QA_BANNER.ACTIVE"));
    \u0275\u0275advance(3);
    \u0275\u0275textInterpolate1(" ", \u0275\u0275pipeBind2(8, 6, "LAYOUT.MAIN.QA_BANNER.SIMULATING", \u0275\u0275pureFunction1(13, _c15, ctx_r1.qaTools.activeProfileName())), " ");
    \u0275\u0275advance(5);
    \u0275\u0275textInterpolate1(" ", \u0275\u0275pipeBind1(13, 9, "LAYOUT.MAIN.QA_BANNER.MANAGE"), " ");
    \u0275\u0275advance(4);
    \u0275\u0275textInterpolate1(" ", \u0275\u0275pipeBind1(17, 11, "LAYOUT.MAIN.QA_BANNER.RESET"), " ");
  }
}
function MainLayoutComponent_Conditional_10_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275property("src", ctx_r1.iframeUrl(), \u0275\u0275sanitizeResourceUrl)("title", \u0275\u0275pipeBind1(1, 2, "LAYOUT.MAIN.IFRAME_TITLE"));
  }
}
var MainLayoutComponent = class _MainLayoutComponent {
  ...
  dialogService = inject(DialogService);
  qaTools = inject(QAToolsService);
  isFlutterRoute = signal(false, ...ngDevMode ? [{ debugName: "isFlutterRoute" }] : (
    /* istanbul ignore next */
    []
  ));
  isIframeRoute = signal(false, ...ngDevMode ? [{ debugName: "isIframeRoute" }] : (
    /* istanbul ignore next */
    []
  ));
  iframeUrl = signal(null, ...ngDevMode ? [{ debugName: "iframeUrl" }] : (
    /* istanbul ignore next */
    []
  ));
  /**
   ...
   */
  scrollPanelClass = computed(() => {
    return this.qaTools.testMode() === true ? "w-full h-[calc(100%-48px)]" : "w-full h-full";
  }, ...ngDevMode ? [{ debugName: "scrollPanelClass" }] : (
    /* istanbul ignore next */
    []
  ));
  layoutService = inject(LayoutService);
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _MainLayoutComponent, selectors: [["app-main-layout"]], hostBindings: function MainLayoutComponent_HostBindings(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275listener("keydown.control.shift.p", function MainLayoutComponent_keydown_control_shift_p_HostBindingHandler($event) {
        return ctx.handleQAPanelShortcut($event);
      }, \u0275\u0275resolveWindow)("keydown.meta.shift.p", function MainLayoutComponent_keydown_meta_shift_p_HostBindingHandler($event) {
        return ctx.handleQAPanelShortcut($event);
      }, \u0275\u0275resolveWindow);
    }
  }, features: [\u0275\u0275ProvidersFeature([DialogService])], decls: 13, vars: 16, consts: [[1, "flex", "h-full"], [1, "fixed", "inset-0", "z-10", "bg-black/50"], [1, "w-full", "min-w-0"], [1, "flex", "justify-between", "items-center", "px-4", "py-2", "text-white", "bg-orange-500", "border-b-2", "border-orange-600", "qa-test-banner"], [3, "ngClass"], [1, "h-full", "flutter-layout", 3, "ngClass"], [1, "h-full", 3, "ngClass"], ["sandbox", "allow-same-origin allow-scripts allow-forms allow-popups", 1, "w-full", "h-full", "border-0", 3, "src", "title"], [1, "fixed", "inset-0", "z-10", "bg-black/50", 3, "click"], [1, "flex", "gap-2", "items-center"], [1, "pi", "pi-exclamation-triangle"], [1, "font-semibold"], [1, "text-sm", "opacity-90"], [1, "px-3", "py-1", "text-sm", "text-orange-600", "bg-white", "rounded", "transition-colors", "hover:bg-orange-50", 3, "click"], [1, "mr-1", "pi", "pi-cog"], [1, "px-3", "py-1", "text-sm", "text-white", "bg-orange-700", "rounded", "transition-colors", "hover:bg-orange-800", 3, "click"], [1, "mr-1", "pi", "pi-refresh"]], template: function MainLayoutComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275elementStart(0, "app-theme")(1, "div", 0);
      \u0275\u0275element(2, "app-sidebar");
      \u0275\u0275conditionalCreate(3, MainLayoutComponent_Conditional_3_Template, 1, 0, "div", 1);
      \u0275\u0275elementStart(4, "div", 2);
      \u0275\u0275conditionalCreate(5, MainLayoutComponent_Conditional_5_Template, 18, 15, "div", 3);
      \u0275\u0275elementStart(6, "p-scrollpanel", 4)(7, "div", 5);
      \u0275\u0275element(8, "app-flutter-layout");
      \u0275\u0275elementEnd();
      \u0275\u0275elementStart(9, "div", 6);
      \u0275\u0275conditionalCreate(10, MainLayoutComponent_Conditional_10_Template, 2, 4, "iframe", 7);
      \u0275\u0275elementEnd();
    ...
    if (rf & 2) {
      \u0275\u0275advance(3);
      \u0275\u0275conditional(ctx.isMobile() && ctx.isSidebarOpen() ? 3 : -1);
      \u0275\u0275advance(2);
      \u0275\u0275conditional(ctx.qaTools.testMode() ? 5 : -1);
      \u0275\u0275advance();
      \u0275\u0275property("ngClass", ctx.scrollPanelClass());
      \u0275\u0275advance();
      \u0275\u0275property("ngClass", \u0275\u0275pureFunction2(7, _c07, !ctx.isFlutterRoute(), ctx.isFlutterRoute()));
      \u0275\u0275advance(2);
      \u0275\u0275property("ngClass", \u0275\u0275pureFunction2(10, _c07, !ctx.isIframeRoute(), ctx.isIframeRoute()));
      \u0275\u0275advance();
      \u0275\u0275conditional(ctx.iframeUrl() ? 10 : -1);
      \u0275\u0275advance();
      \u0275\u0275property("ngClass", \u0275\u0275pureFunction2(13, _c07, ctx.isFlutterRoute() || ctx.isIframeRoute(), !ctx.isFlutterRoute() && !ctx.isIframeRoute()));
    }
  }, dependencies: [
    RouterModule,
    RouterOutlet,
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(MainLayoutComponent, [{
    ...
    args: [{ selector: "app-main-layout", imports: [
      ...
      CommonModule,
      TranslatePipe
    ], providers: [DialogService], template: `<app-theme>
  <div class="flex h-full">
    ...
    <div class="w-full min-w-0">
      ...
      @if (qaTools.testMode()) {
        <div
          class="flex justify-between items-center px-4 py-2 text-white bg-orange-500 border-b-2 border-orange-600 qa-test-banner"
        >
          <div class="flex gap-2 items-center">
            <i class="pi pi-exclamation-triangle"></i>
            <span class="font-semibold">{{
              'LAYOUT.MAIN.QA_BANNER.ACTIVE' | translate
            }}</span>
            <span class="text-sm opacity-90">
              {{
                'LAYOUT.MAIN.QA_BANNER.SIMULATING'
                  | translate: { profile: qaTools.activeProfileName() }
              }}
            </span>
          </div>
          <div class="flex gap-2 items-center">
            <button
              class="px-3 py-1 text-sm text-orange-600 bg-white rounded transition-colors hover:bg-orange-50"
              (click)="openQAPanel()"
            >
              <i class="mr-1 pi pi-cog"></i>
              {{ 'LAYOUT.MAIN.QA_BANNER.MANAGE' | translate }}
            </button>
            <button
              class="px-3 py-1 text-sm text-white bg-orange-700 rounded transition-colors hover:bg-orange-800"
              (click)="qaTools.resetQAState()"
            >
              <i class="mr-1 pi pi-refresh"></i>
              {{ 'LAYOUT.MAIN.QA_BANNER.RESET' | translate }}
            </button>
          </div>
        </div>
      }
      ...
      <p-scrollpanel [ngClass]="scrollPanelClass()">
        <!-- Flutter Layer: anchor only. The iframe itself is hosted at
             app-shell level (app-flutter-frame in AppComponent) so it survives
             route changes; no @defer/spinner is needed here because the anchor
             is an empty element and boot feedback (spinner until the iframe
             loads) is rendered by app-flutter-frame. -->
        <div
        ...
        >
          @if (iframeUrl()) {
            <iframe
              [src]="iframeUrl()"
              [title]="'LAYOUT.MAIN.IFRAME_TITLE' | translate"
...
`, styles: ["/* src/app/layouts/components/main-layout/main-layout.component.scss */\n.layout-splitter {\n  height: 100vh;\n}\n.main-content {\n  padding: 20px;\n  overflow: auto;\n}\n@media (max-width: 768px) {\n  .layout-splitter {\n    flex-direction: column;\n  }\n  .main-content {\n    padding: 10px;\n    height: 100%;\n    overflow: hidden;\n  }\n}\n.flutter-layout {\n  height: calc(var(--vh, 1vh) * 100);\n}\n/*# sourceMappingURL=main-layout.component.css.map */\n"] }]
  }], () => [], { handleQAPanelShortcut: [{
    type: HostListener,
    args: ["window:keydown.control.shift.p", ["$event"]]
  }, {
    type: HostListener,
    args: ["window:keydown.meta.shift.p", ["$event"]]
  }] });
})();
(() => {
...
function ExternalTabComponent_Conditional_0_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const ctx_r0 = \u0275\u0275nextContext();
    \u0275\u0275domProperty("src", ctx_r0.iframeUrl(), \u0275\u0275sanitizeResourceUrl)("title", \u0275\u0275pipeBind1(2, 2, "FEATURES.EXTERNAL_TAB.TITLE"));
  }
}
function ExternalTabComponent_Conditional_1_Template(rf, ctx) {
  if (rf & 1) {
  ...
  if (rf & 2) {
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1(" ", \u0275\u0275pipeBind1(2, 1, "FEATURES.EXTERNAL_TAB.UNAVAILABLE"), " ");
  }
}
var ExternalTabComponent = class _ExternalTabComponent {
  ...
  navItem = computed(() => {
    ...
    if (!isDefined(id))
      return null;
    return this.navigationService.getNavigationItemById(id) ?? null;
  }, ...ngDevMode ? [{ debugName: "navItem" }] : (
    /* istanbul ignore next */
    []
  ));
  iframeUrl = computed(() => {
    const externalUrl = this.navItem()?.externalUrl;
    if (isDefined(externalUrl) && externalUrl.trim() !== "") {
      try {
        const parsed = new URL(externalUrl);
        if (parsed.protocol === "https:" || isDevMode() && parsed.protocol === "http:") {
    ...
    return null;
  }, ...ngDevMode ? [{ debugName: "iframeUrl" }] : (
    /* istanbul ignore next */
    []
  ));
  constructor() {
    this.setupAppStoreHandshake();
  }
  setupAppStoreHandshake() {
    fromEvent(window, "message").pipe(filter(() => isDefined(this.navItem()?.appStoreAppId)), filter((event) => event.data === "READY" || event.data?.type === "READY"), filter((event) => {
      const externalUrl = this.navItem()?.externalUrl;
      if (!isDefined(externalUrl))
        return false;
      ...
      const appId = this.navItem()?.appStoreAppId ?? "";
      const ownerId = this.clientDataService.getSelectedClient()?.id ?? "";
      return this.authService.delegateToken(appId, ownerId);
    }), takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (res) => this.postWidgetConfig(res.token),
      error: (err) => console.error("[ExternalTab] delegate token failed:", err)
    });
  }
  postWidgetConfig(accessToken) {
    const iframeWindow = this.iframeEl?.nativeElement?.contentWindow;
    const externalUrl = this.navItem()?.externalUrl;
    if (!isDefined(iframeWindow) || !isDefined(externalUrl))
      return;
    try {
      const targetOrigin = new URL(externalUrl).origin;
      iframeWindow.postMessage({ type: "WIDGET_CONFIG", payload: { accessToken } }, targetOrigin);
    } catch {
      console.error("[ExternalTab] Could not determine target origin");
    }
  }
  static \u0275fac = function ExternalTabComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _ExternalTabComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _ExternalTabComponent, selectors: [["app-external-tab"]], viewQuery: function ExternalTabComponent_Query(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275viewQuery(_c08, 5);
    }
    if (rf & 2) {
  ...
  }, decls: 2, vars: 1, consts: [["iframeEl", ""], [1, "external-iframe", 3, "src", "title"], [1, "external-unavailable"]], template: function ExternalTabComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275conditionalCreate(0, ExternalTabComponent_Conditional_0_Template, 3, 4, "iframe", 1)(1, ExternalTabComponent_Conditional_1_Template, 3, 3, "p", 2);
    }
    if (rf & 2) {
      \u0275\u0275conditional(ctx.iframeUrl() ? 0 : 1);
    }
  }, dependencies: [TranslatePipe], styles: ["\n.external-iframe[_ngcontent-%COMP%] {\n  border: none;\n  width: 100%;\n  height: 100%;\n}\n.external-unavailable[_ngcontent-%COMP%] {\n  padding: 1rem;\n  text-align: center;\n}\n/*# sourceMappingURL=external-tab.component.css.map */"], changeDetection: 0 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ExternalTabComponent, [{
    type: Component,
    args: [{ selector: "app-external-tab", imports: [TranslatePipe], changeDetection: ChangeDetectionStrategy.OnPush, template: `@if (iframeUrl()) {
  <iframe
    #iframeEl
    [src]="iframeUrl()"
    class="external-iframe"
    [title]="'FEATURES.EXTERNAL_TAB.TITLE' | translate"
  ></iframe>
} @else {
  <p class="external-unavailable">
    {{ 'FEATURES.EXTERNAL_TAB.UNAVAILABLE' | translate }}
  </p>
}
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(ExternalTabComponent, { className: "ExternalTabComponent", filePath: "src/app/features/external-tab/external-tab.component.ts", lineNumber: 32 });
})();
...
var clientSelectedGuard = () => {
  ...
  if (!isDefined(userSessionService.currentUserSession())) {
...
// src/app/core/guards/admin-permissions.guard.ts
var adminPermissionsGuard = () => {
...
// src/app/core/guards/non-production.guard.ts
var nonProductionGuard = () => {
  const router = inject(Router);
...
var routes = [
  {
    ...
    children: [
      ...
      },
      {
        ...
        loadChildren: () => import("./chunk-626RHRQX.js").then((m5) => m5.POSITION_ROUTES)
      },
      {
        path: "alerts",
        data: { isFlutterRoute: true, featureFlag: "page.alerts.enabled" },
        canActivate: [featureGuard],
        loadChildren: () => import("./chunk-YDNO5D62.js").then((m5) => m5.ALERTS_ROUTES)
      },
      {
        path: "media",
        data: { isFlutterRoute: true, featureFlag: "page.videos.enabled" },
        canActivate: [featureGuard],
        loadChildren: () => import("./chunk-R3Q3PFEA.js").then((m5) => m5.MEDIA_ROUTES)
      },
      {
        path: "history",
        data: { isFlutterRoute: true, featureFlag: "page.replay.enabled" },
        canActivate: [featureGuard],
        loadChildren: () => import("./chunk-6QZR2X3I.js").then((m5) => m5.HISTORY_ROUTES)
      },
      {
        path: "user",
        loadChildren: () => import("./chunk-6SE2GSBW.js").then((m5) => m5.USERS_ROUTES)
      },
      {
        path: "admin",
        canActivate: [adminPermissionsGuard],
        loadChildren: () => import("./chunk-QNP3OAER.js").then((m5) => m5.ADMINISTRATION_ROUTES)
      },
      {
        path: "analytical-report",
        loadComponent: () => import("./chunk-EEA7N72X.js").then((m5) => m5.AnalyticalReportComponent)
      },
      {
        path: "app-store",
        data: { featureFlag: "page.moreapps.enabled" },
        canActivate: [featureGuard],
        loadChildren: () => import("./chunk-SGONRAI3.js").then((m5) => m5.APP_STORE_ROUTES)
      },
      {
        path: "development",
        canActivate: [nonProductionGuard],
        loadChildren: () => import("./chunk-NPKOZUXB.js").then((m5) => m5.DEVELOPMENT_ROUTES)
      },
      {
        path: "external/:id",
  ...
  {
    path: "auth",
    loadChildren: () => import("./chunk-UHRD2QGT.js").then((m5) => m5.AUTH_ROUTES)
  },
  {
    path: "unauthorized",
    loadComponent: () => import("./chunk-47445UNA.js").then((m5) => m5.UnauthorizedComponent)
  }
];
...
var last401ToastTime = 0;
var TOAST_THROTTLE_MS = 3e3;
  ...
  switch (error.status) {
    case 401:
      return {
        ...
        errorMessage: req.url.includes("/auth/signin") ? "Invalid username or password." : "Your session has expired. Please log in again.",
        token: null,
        valid: false
      };
    case 403:
...
function isTargetApiUrl(requestUrl, allowedUrls) {
  return allowedUrls.some((apiUrl) => {
    try {
      ...
      const portMatch = allowedUrl.port === "" || reqUrl.port === allowedUrl.port;
...
var AuthInterceptor = (req, next) => {
  ...
  function showToastForError(errorObject, versionUpdateService2, request2) {
    if (request2.context.get(SILENT_ERROR)) {
      return;
    }
    if (versionUpdateService2.isVersionMismatchError(errorObject.errorMessage)) {
      versionUpdateService2.handleVersionMismatch("HTTP error: " + errorObject.errorReason);
      return;
    }
  ...
  function handle401Error(req2, next2, refreshToken) {
    ...
    if (tokenRefreshCoordinator.refreshTokenInProgress === false) {
      ...
      return authService.refreshToken(refreshToken).pipe(timeout(TOKEN_REFRESH_TIMEOUT_MS), switchMap((authResponse) => {
        authTokenManagerService.storeTokens(authResponse);
        tokenRefreshCoordinator.notifyRefreshSuccess(authResponse.accessToken);
        tokenRefreshCoordinator.resetRefreshState();
        return next2(addToken(req2, authResponse.accessToken));
      }), catchError((error) => {
        const errorMessage = error instanceof TimeoutError ? "Token refresh timed out" : `Token refresh error: ${error.message}`;
      ...
      return tokenRefreshCoordinator.refreshTokenSubject.pipe(filter((state) => state.status === "success" || state.status === "error"), take(1), timeout({
        each: TOKEN_REFRESH_TIMEOUT_MS,
        with: () => throwError(() => new Error("Timeout waiting for token refresh. Please try again."))
      }), switchMap((state) => {
        if (state.status === "success") {
          return next2(addToken(req2, state.token));
        } else {
          return throwError(() => state.error);
        }
      }), catchError((error) => {
        if (error instanceof Error && error.message?.includes("Timeout waiting for token refresh")) {
          authDataService.forceLogout();
        }
  ...
  function handleError(req2, next2, error) {
    if (error.status === 404 && req2.url.includes("/entities/assets/") && req2.url.includes("/avatar")) {
      return throwError(() => error);
    }
    const errorObject = createErrorObject(error, req2);
    errorObject.originalError = error;
    if (error.status === 401) {
      if (req2.url.includes("/auth/signin") || req2.url.includes("/auth/refresh")) {
        showToastForError(errorObject, versionUpdateService, req2);
        return throwError(() => errorObject);
      }
      if (req2.context.get(SKIP_AUTH_REFRESH) || req2.headers.has("x-base-url")) {
        return throwError(() => error);
      }
  ...
  return next(request).pipe(retry({
    // Non-idempotent requests opt out: retrying "queue this report" after a
    // gateway 504 would enqueue the job twice.
    count: request.context.get(NO_RETRY) ? 0 : 1,
...
function startupInitializer() {
  return async () => {
    const coreAuth = inject(AuthService2);
...
// src/app/features/administration/services/entity/entity-adapters.provider.ts
var ADAPTERS = [
  {
  ...
  {
    types: ["vendor-export-tasks", "distributor-export-tasks", "export-task"],
    adapter: ExportTasksDataAdapter
  },
  {
    types: [
      "vendor-map-sets",
      "distributor-map-sets",
      "super-admin-map-sets",
      "map-set"
    ],
    adapter: MapSetsDataAdapter
  },
  {
    types: ["client-contacts", "contact"],
    adapter: ContactsDataAdapter
  }
];
var DEFAULT_ENTITY_ADAPTERS = {
...
var appConfig = {
  providers: [
    ...
    provideServiceWorker("ngsw-worker.js", {
      enabled: true,
      // Enable in all environments for PWA testing
      registrationStrategy: "registerImmediately"
    }),
    // Activate new deployments instead of leaving the tab on the build it
    // booted with, whose lazy chunks are gone from the server after a deploy.
    provideAppInitializer(() => inject(VersionUpdateService).watchForUpdates()),
    provideTranslateService({
...
var ToastComponent = class _ToastComponent {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _ToastComponent, selectors: [["app-toast"]], decls: 1, vars: 4, consts: [[3, "breakpoints"]], template: function ToastComponent_Template(rf, ctx) {
    ...
    if (rf & 2) {
      \u0275\u0275property("breakpoints", \u0275\u0275pureFunction1(2, _c16, \u0275\u0275pureFunction0(1, _c09)));
    }
...
// src/app/layouts/components/flutter-frame/flutter-frame.component.ts
function FlutterFrameComponent_Conditional_0_Conditional_2_Template(rf, ctx) {
  if (rf & 1) {
...
function FlutterFrameComponent_Conditional_0_Template(rf, ctx) {
  if (rf & 1) {
    const _r1 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "iframe", 0);
    \u0275\u0275pipe(1, "translate");
    \u0275\u0275listener("load", function FlutterFrameComponent_Conditional_0_Template_iframe_load_0_listener() {
      \u0275\u0275restoreView(_r1);
      const ctx_r1 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r1.frame.onFrameLoad());
    });
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(2, FlutterFrameComponent_Conditional_0_Conditional_2_Template, 2, 0, "div", 1);
  }
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275property("src", ctx_r1.frame.safeUrl(), \u0275\u0275sanitizeResourceUrl)("title", \u0275\u0275pipeBind1(1, 3, "LAYOUT.FLUTTER.EMBEDDED_TITLE"));
    \u0275\u0275advance(2);
    \u0275\u0275conditional(!ctx_r1.frame.frameLoaded() ? 2 : -1);
  }
}
var FlutterFrameComponent = class _FlutterFrameComponent {
  frame = inject(FlutterFrameService);
  static \u0275fac = function FlutterFrameComponent_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _FlutterFrameComponent)();
  };
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _FlutterFrameComponent, selectors: [["app-flutter-frame"]], hostVars: 2, hostBindings: function FlutterFrameComponent_HostBindings(rf, ctx) {
    if (rf & 2) {
      \u0275\u0275styleMap(ctx.frame.containerStyle());
    }
  }, decls: 1, vars: 1, consts: [["id", "flutter-app", "allow", "same-origin; scripts; fullscreen", "sandbox", "allow-scripts allow-same-origin allow-forms allow-popups allow-downloads", 1, "flutter-app-iframe", 3, "load", "src", "title"], [1, "flutter-frame-loading"], ["styleClass", "w-[50px] h-[50px]", "strokeWidth", "4"]], template: function FlutterFrameComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275conditionalCreate(0, FlutterFrameComponent_Conditional_0_Template, 3, 5);
    }
    if (rf & 2) {
      \u0275\u0275conditional(ctx.frame.active() ? 0 : -1);
    }
  }, dependencies: [ProgressSpinnerModule, ProgressSpinner, TranslatePipe], styles: ["\n[_nghost-%COMP%] {\n  position: fixed;\n  z-index: 5;\n}\n.flutter-app-iframe[_ngcontent-%COMP%] {\n  width: 100%;\n  height: 100%;\n  border: none;\n  display: block;\n}\n.flutter-frame-loading[_ngcontent-%COMP%] {\n  position: absolute;\n  inset: 0;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  pointer-events: none;\n}\n/*# sourceMappingURL=flutter-frame.component.css.map */"] });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(FlutterFrameComponent, [{
    type: Component,
    args: [{ selector: "app-flutter-frame", imports: [ProgressSpinnerModule, TranslatePipe], host: {
      "[style]": "frame.containerStyle()"
    }, template: `@if (frame.active()) {
  <iframe
    id="flutter-app"
    [src]="frame.safeUrl()"
    class="flutter-app-iframe"
    [title]="'LAYOUT.FLUTTER.EMBEDDED_TITLE' | translate"
    allow="same-origin; scripts; fullscreen"
    sandbox="allow-scripts allow-same-origin allow-forms allow-popups allow-downloads"
    (load)="frame.onFrameLoad()"
  ></iframe>
  @if (!frame.frameLoaded()) {
    <div class="flutter-frame-loading">
      <p-progressSpinner styleClass="w-[50px] h-[50px]" strokeWidth="4">
      </p-progressSpinner>
    </div>
  }
}
`, styles: ["/* src/app/layouts/components/flutter-frame/flutter-frame.component.scss */\n:host {\n  position: fixed;\n  z-index: 5;\n}\n.flutter-app-iframe {\n  width: 100%;\n  height: 100%;\n  border: none;\n  display: block;\n}\n.flutter-frame-loading {\n  position: absolute;\n  inset: 0;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  pointer-events: none;\n}\n/*# sourceMappingURL=flutter-frame.component.css.map */\n"] }]
  }], null, null);
})();
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(FlutterFrameComponent, { className: "FlutterFrameComponent", filePath: "src/app/layouts/components/flutter-frame/flutter-frame.component.ts", lineNumber: 26 });
})();
...
var AppComponent = class _AppComponent {
  ...
  updateTitleEffect = effect(() => {
    ...
    if (isDefined(clientName)) {
      ...
      const ownerName = owner?.id === DEFAULT_SYSTEM_ID ? null : owner?.name?.trim();
      this.titleService.setTitle(ownerName ?? "Welcome back");
    }
  }, ...ngDevMode ? [{ debugName: "updateTitleEffect" }] : (
    /* istanbul ignore next */
    []
  ));
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _AppComponent, selectors: [["app-root"]], decls: 3, vars: 0, template: function AppComponent_Template(rf, ctx) {
    if (rf & 1) {
      \u0275\u0275element(0, "router-outlet")(1, "app-toast")(2, "app-flutter-frame");
    }
  }, dependencies: [RouterOutlet, ToastComponent, FlutterFrameComponent], encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(AppComponent, [{
    type: Component,
    args: [{ selector: "app-root", imports: [RouterOutlet, ToastComponent, FlutterFrameComponent], template: "<router-outlet></router-outlet>\n<app-toast></app-toast>\n<app-flutter-frame></app-flutter-frame>\n" }]
  }], null, null);
})();
(() => {
