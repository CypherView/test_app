...
var AdminTreeEventHandlerService = class _AdminTreeEventHandlerService {
  ...
  async navigateToSearchResult(result) {
    ...
    this.scheduleNodeSelection(result.id, result.type);
  }
  /**
   * Schedules node selection to happen when the node becomes available
   * Utilizes existing loading state tracking and callback infrastructure
   */
  scheduleNodeSelection(nodeId, nodeType) {
    this.pendingNodeSelection = {
      nodeId,
      nodeType,
      timestamp: Date.now()
    };
    this.trySelectPendingNode();
    if (this.pendingNodeSelection !== null) {
  ...
  async handleClientCreated(clientId) {
    try {
      ...
      this.scheduleNodeSelection(clientId, ADMIN_TREE.NODE_TYPES.CLIENT);
    } catch (error) {
...
var AdminContainerComponent = class _AdminContainerComponent {
  ...
  async updateTreeSelectionFromRoute(keyToSelect, typeToSelect) {
    const treeNodes = this.hierarchyTree();
    const nodeToSelect = isDefined(typeToSelect) ? this.findNodeByTypeAndKey(treeNodes, keyToSelect, typeToSelect) : this.findNodeByKeyRecursive(treeNodes, keyToSelect);
    if (nodeToSelect) {
      this.treeEventHandler.setTreeSelection(nodeToSelect);
      this.updateStateBasedOnNodeType(nodeToSelect);
      if (!this.treeCollapsed()) {
      ...
      console.warn("Could not find node to select:", keyToSelect, typeToSelect);
    }
  }
  /**
    ...
    children: [
      {
        ...
        loadChildren: () => import("./chunk-KOYCHNAT.js").then((m) => m.SuperAdminModule)
      },
      {
        ...
        loadChildren: () => import("./chunk-GNCNPSU5.js").then((m) => m.VendorsModule)
      },
      {
        ...
        loadChildren: () => import("./chunk-IQGT4HBZ.js").then((m) => m.DistributorsModule)
      },
      {
        ...
        loadChildren: () => import("./chunk-K3E7ZWQV.js").then((m) => m.ClientsModule)
      }
    ]
  }
];
...
//# sourceMappingURL=chunk-A4BLOJZY.js.map
