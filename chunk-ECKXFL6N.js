...
} from "./chunk-UCEG7X2Z.js";
import {
  AppRatingDisplayComponent
} from "./chunk-4WNAVHK2.js";
import {
  AppStatusBadgeComponent
} from "./chunk-2WGYFY54.js";
import {
  AppStoreService,
  mapAppReviewFromApi
} from "./chunk-3W2ISD6M.js";
import {
  TimelineModule
} from "./chunk-3HMCVQYQ.js";
import {
  ModalListFormComponent
} from "./chunk-ZLLI6SMD.js";
import {
  CardModule
...
var AppDetailComponent = class _AppDetailComponent {
  ...
  userInstallation = toSignal(toObservable(computed(() => ({
    ...
    if (isNullOrUndefined(appId) || isNullOrUndefined(clientId))
      return of(null);
    return this.appInstallationService.getInstallations({ owner: clientId }).pipe(map((installations) => installations.find((inst) => inst.appId === appId) ?? null), catchError(() => of(null)));
  })), { initialValue: null });
  ...
  installApp() {
    const app = this.app();
    const clientId = this.clientDataService.getSelectedClient()?.id;
    if (!app || !this.canInstall() || isNullOrUndefined(clientId))
      return;
    this.installationError.set(null);
    this.isPerformingInstallationAction.set(true);
    this.appInstallationService.installApp(app.id, clientId, {}).pipe(tap(() => {
  ...
  uninstallApp() {
    ...
    const clientId = this.clientDataService.getSelectedClient()?.id;
    if (!installation || isNullOrUndefined(clientId))
      return;
    this.installationError.set(null);
    this.isPerformingInstallationAction.set(true);
    this.appInstallationService.uninstallApp(installation.id, clientId).pipe(tap(() => {
      this.refreshInstallationTrigger.update((n) => n + 1);
      this.installationError.set(null);
    }), catchError((err) => {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AppDetailComponent, { className: "AppDetailComponent", filePath: "src/app/features/app-store/pages/app-detail/app-detail.component.ts", lineNumber: 61 });
})();
export {
  AppDetailComponent
};
//# sourceMappingURL=chunk-ECKXFL6N.js.map
