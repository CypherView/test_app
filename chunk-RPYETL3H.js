...
import {
  ...
  UsersDataAdapter,
  VendorsDataAdapter
} from "./chunk-UMSHCNJL.js";
import {
  SEARCH_DEBOUNCE_TIME
} from "./chunk-TBFE2KEL.js";
import {
  ...
  handleTreeRefreshRequired(message) {
    ...
    switch (m.entityType) {
      case "vendor": {
        ...
        if (isDefined(distributorId)) {
          const isDistributorExpanded = this.adminState.isDistributorExpanded(distributorId);
          if (isDistributorExpanded) {
      ...
      case "distributor": {
        ...
        if (superAdminExpanded) {
          this.adminTreeService.listDistributors(DEFAULT_SYSTEM_ID).then((distributors) => {
            this.adminState.setDistributors(distributors);
          }).catch((error) => {
            console.error("Error reloading distributors after creation:", error);
          });
        }
        break;
      }
      case "client": {
        ...
        let distributorId = m.distributorId;
        if (isNullOrUndefined(vendorId) || isNullOrUndefined(distributorId)) {
          ...
          if (selected?.type === ADMIN_TREE.NODE_TYPES.VENDOR && isDefined(selected.key)) {
            vendorId = vendorId ?? selected.key;
            distributorId = distributorId ?? this.adminDataService.getDistributorContextFromNode(selected);
          } else if (selected?.type === ADMIN_TREE.NODE_TYPES.CLIENT) {
            const hierarchy = this.adminSearchService.findClientHierarchy(selected.key);
            if (hierarchy) {
              vendorId = vendorId ?? hierarchy.vendorId;
              distributorId = distributorId ?? hierarchy.distributorId;
            }
          }
        }
        distributorId = distributorId ?? this.adminState.getCurrentDistributorId();
        if (isNullOrUndefined(vendorId) || isNullOrUndefined(distributorId)) {
          console.warn("Client created but no vendorId or distributorId available to refresh.");
          return;
        }
    ...
    children: [
      {
        ...
        loadChildren: () => import("./chunk-R4H34LD6.js").then((m) => m.SuperAdminModule)
      },
      {
        ...
        loadChildren: () => import("./chunk-73N2SUQF.js").then((m) => m.VendorsModule)
      },
      {
        ...
        loadChildren: () => import("./chunk-MK6RMM2R.js").then((m) => m.DistributorsModule)
      },
      {
        ...
        loadChildren: () => import("./chunk-IZ4V34HP.js").then((m) => m.ClientsModule)
      }
    ]
  }
];
...
//# sourceMappingURL=chunk-RPYETL3H.js.map
