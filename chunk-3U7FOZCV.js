...
var AdminSearchService = class _AdminSearchService {
  ...
  async prepareClientForTreeSelectionWithOwner(client) {
    ...
    try {
      this.containerState.temporarilySuppressScrollToClient(4e3);
  ...
  async handleGroupOwnedClient(client) {
    if (!isDefined(client.owner?.id) || !isDefined(client.id))
      return null;
    const groupId = client.owner.id;
    const vendorId = this.findVendorIdForGroup(groupId);
    if (!isDefined(vendorId)) {
      console.warn("Could not find vendor for group:", groupId);
      return null;
    }
    await this.ensureVendorLoadedAndExpanded(vendorId);
  ...
  async loadClientsUntilFound(targetClientId, vendorId, groupId) {
    let foundClient = this.findClientInLoadedData(targetClientId, vendorId, groupId);
    if (foundClient) {
      return foundClient;
    }
    const paginationKey = isDefined(groupId) && groupId !== DEFAULT_SYSTEM_ID ? `${vendorId}_${groupId}` : vendorId;
    let currentPagination = this.adminState.clientPagination()[paginationKey];
    if (isNullOrUndefined(currentPagination)) {
      await this.loadInitialClients(vendorId, groupId);
      foundClient = this.findClientInLoadedData(targetClientId, vendorId, groupId);
      if (foundClient) {
        return foundClient;
      }
      currentPagination = this.adminState.clientPagination()[paginationKey];
    }
    while (isDefined(currentPagination) && currentPagination.hasMore && isNullOrUndefined(foundClient)) {
      await this.loadMoreClientsPage(vendorId, groupId);
      foundClient = this.findClientInLoadedData(targetClientId, vendorId, groupId);
      if (foundClient) {
        return foundClient;
      }
  ...
  /**
   * Loads initial clients for a vendor or group
   */
  async loadInitialClients(vendorId, groupId) {
    try {
      const clientResponse = await this.adminTreeService.listVendorClients(vendorId, groupId, 0, 100);
      ...
      if (isDefined(groupId)) {
        this.adminState.addGroupClients(vendorId, groupId, clientResponse.clients, paginationUpdate, false);
      } else {
        this.adminState.addVendorClients(vendorId, clientResponse.clients, paginationUpdate, false);
      }
    } catch (error) {
   ...
   */
  async loadMoreClientsPage(vendorId, groupId) {
    const paginationKey = isDefined(groupId) && groupId !== DEFAULT_SYSTEM_ID ? `${vendorId}_${groupId}` : vendorId;
...
var AdminDataService = class _AdminDataService {
  ...
  async loadMoreClients(vendorId, groupId) {
    ...
    this.containerState.temporarilySuppressScrollToClient(3e3);
    const nextOffset = currentPagination.offset + currentPagination.limit;
    try {
      this.adminState.setLoadingMoreClients(vendorId, groupId, true);
    ...
    if (isVendorExpanded) {
      ...
      vendorNode.children = [
        ...groups.map((group) => {
          ...
          const groupNode = this.adminTreeService.createGroupNode(group, isGroupExpanded, [], selectedClient?.id === group.id);
...
var AdminTreeEventHandlerService = class _AdminTreeEventHandlerService {
  ...
  // Tree panel state
  treeCollapsed = signal(true);
  // Callback references for component to set
  scrollToClientCallback;
  temporarilySuppressScrollCallback;
  toggleTreePanelCallback;
  hierarchyTreeGetter;
  findNodeByKeyRecursiveCallback;
  findNodeRecursivelyCallback;
  handleErrorCallback;
  /**
   * Initialize the service with callback functions from the component
   */
  initialize(callbacks) {
    this.scrollToClientCallback = callbacks.scrollToClient;
    this.temporarilySuppressScrollCallback = callbacks.temporarilySuppressScroll;
    this.toggleTreePanelCallback = callbacks.toggleTreePanel;
    this.hierarchyTreeGetter = callbacks.getHierarchyTree;
    this.findNodeByKeyRecursiveCallback = callbacks.findNodeByKeyRecursive;
    this.findNodeRecursivelyCallback = callbacks.findNodeRecursively;
    this.handleErrorCallback = callbacks.handleError;
  ...
  async onNodeExpand(node) {
    try {
      this.temporarilySuppressScrollCallback?.(2e3);
      switch (node.type) {
        case ADMIN_TREE.NODE_TYPES.VENDOR:
  ...
  async onNodeSelect(node) {
    ...
    if (node.type === ADMIN_TREE.NODE_TYPES.LOAD_MORE) {
      this.temporarilySuppressScrollCallback?.(3e3);
      ...
      if (node.type === ADMIN_TREE.NODE_TYPES.CLIENT) {
        this.scrollToClientCallback?.(node.key);
      }
    }
  }
  /**
  ...
  async navigateToSearchResult(result) {
    ...
    const nodeToSelect = this.findNodeByKeyRecursiveCallback?.(finalTree, result.id);
    if (nodeToSelect) {
      this.selectedTreeNode.set(nodeToSelect);
      this.adminDataService.selectNode(nodeToSelect);
      this.scrollToClientCallback?.(result.id);
    } else {
  ...
  async handleClientCreated(clientId) {
    try {
      ...
      setTimeout(() => {
        const treeNodes = this.hierarchyTreeGetter?.() ?? [];
        const newClientNode = this.findNodeRecursivelyCallback?.(treeNodes, clientId, ADMIN_TREE.NODE_TYPES.CLIENT);
        if (newClientNode) {
          this.selectedTreeNode.set(newClientNode);
          this.adminDataService.selectNode(newClientNode);
          this.scrollToClientCallback?.(clientId);
        }
      }, 100);
    } catch (error) {
  ...
  handleError(error, message) {
    this.handleErrorCallback?.(error, message);
  }
...
function TreeNavigationPanelComponent_Conditional_1_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275advance(9);
    \u0275\u0275property("nodes", ctx_r2.hierarchyTree()())("loading", ctx_r2.isLoading()())("searchingForNode", ctx_r2.searchingForNode()())("searchingNodeKey", ctx_r2.searchingNodeKey()())("selection", ctx_r2.findSelectedNode())("scrollToNodeKey", ctx_r2.scrollToNodeKey()())("contextExtractor", ctx_r2.contextExtractor);
  }
}
...
var AdminRouteHandlerService = class _AdminRouteHandlerService {
  ...
  loadInitialDataAndSetup(callbacks) {
    ...
    this.adminDataService.loadInitialData(routeClientId).then(() => {
      ...
      callbacks.syncTreeSelectionWithRouteAndLocalStorage().catch((err) => console.error("Error syncing tree selection:", err));
...
function AdminContainerComponent_ng_template_3_Template(rf, ctx) {
  ...
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275property("treeCollapsed", ctx_r1.treeCollapsed)("hierarchyTree", ctx_r1.hierarchyTree)("isLoading", ctx_r1.isLoading)("searchingForNode", ctx_r1.searchingForNode)("searchingNodeKey", ctx_r1.searchingNodeKey)("selectedTreeNode", ctx_r1.selectedTreeNode)("scrollToNodeKey", ctx_r1.scrollToNodeKey)("searchResults", ctx_r1.searchResults)("isSearching", ctx_r1.isSearching);
  }
}
...
var AdminContainerComponent = class _AdminContainerComponent {
  ...
  constructor() {
    this.scrollManagement.initialize({
      ...
      setSelectedTreeNode: (node) => this.selectedTreeNode.set(node),
  ...
  ngOnInit() {
    this.treeEventHandler.initialize({
      scrollToClient: (clientId) => this.scrollToClient(clientId),
      temporarilySuppressScroll: (durationMs) => this.temporarilySuppressScrollToClient(durationMs),
    ...
    this.routeHandler.initializeRouteHandling({
      ...
      syncTreeSelectionWithRouteAndLocalStorage: () => this.syncTreeSelectionWithRouteAndLocalStorage(),
      ensureSuperAdminNodeExpansion: () => this.ensureSuperAdminNodeExpansion(),
      onChangeDetection: () => this.cdr.detectChanges()
    });
  }
  /**
   * Scrolls the tree to center on the specified client node
   ...
   * Simplified to leverage existing effect system for handling navigation logic
   */
  async navigateToSelectedClient() {
    ...
    if (!isDefined(selectedClient.owner?.id)) {
      try {
        ...
        if (fullClient) {
          this.clientDataService.setSelectedClient(fullClient);
        } else {
      ...
      this.clientDataService.setSelectedClient(__spreadValues({}, selectedClient));
    }
  }
  /**
  ...
  async handleSelectedClientChange(selectedClient) {
    ...
    try {
      ...
      if (searchResult) {
        ...
        this.scrollManagement.handleClientNavigationScroll(selectedClient.id);
      } else {
   ...
   */
  async onNodeExpand(node) {
    return this.treeEventHandler.onNodeExpand(node);
  }
  async onGroupExpand(groupNode) {
    return this.treeEventHandler.onNodeExpand(groupNode);
  }
  onNodeCollapse(node) {
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _AdminContainerComponent, selectors: [["app-admin-container"]], decls: 7, vars: 20, consts: [["treeNavigationTemplate", ""], ["mainContentTemplate", ""], [1, "flex", "h-full", "overflow-hidden", 3, "ngClass"], ["stateKey", "main-layout", 1, "h-full", "w-full", 3, "ngClass", "panels", "panelSizes", "minSizes", "defaultLayout", "responsive"], [1, "fixed", "inset-0", "bg-black", "bg-opacity-30", "z-40"], ["styleClass", "w-8 h-8", "strokeWidth", "2", 1, "fixed", "z-50", "top-1/2", "left-1/2", "-translate-x-1/2", "-translate-y-1/2"], [3, "toggleTreePanel", "navigateToSelectedClient", "nodeExpand", "nodeCollapse", "nodeSelect", "nodeNotFoundInData", "searchQueryChange", "searchResultSelect", "searchClear", "treeCollapsed", "hierarchyTree", "isLoading", "searchingForNode", "searchingNodeKey", "selectedTreeNode", "scrollToNodeKey", "searchResults", "isSearching"], [3, "routerOutletActivate", "nestedContentScrollable", "detailsPanelClass"]], template: function AdminContainerComponent_Template(rf, ctx) {
    ...
    if (rf & 2) {
      ...
      \u0275\u0275advance();
      \u0275\u0275property("ngClass", ctx.mainSplitterClass())("panels", \u0275\u0275pureFunction2(13, _c23, \u0275\u0275pureFunction2(8, _c06, treeNavigationTemplate_r4, ctx.treeCollapsed() ? 2 : 15), \u0275\u0275pureFunction1(11, _c15, mainContentTemplate_r5)))("panelSizes", ctx.treeCollapsed() ? \u0275\u0275pureFunction0(16, _c33) : \u0275\u0275pureFunction0(17, _c42))("minSizes", \u0275\u0275pureFunction1(18, _c52, ctx.treeCollapsed() ? 2 : 15))("defaultLayout", "horizontal")("responsive", true);
    }
  }, dependencies: [
    AdaptiveLayoutComponent,
    CommonModule,
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AdminContainerComponent, { className: "AdminContainerComponent", filePath: "src/app/features/administration/components/admin-container/admin-container.component.ts", lineNumber: 65 });
})();
    ...
    children: [
      {
        ...
        loadChildren: () => import("./chunk-OJLIKLPX.js").then((m) => m.SuperAdminModule)
      },
      {
        ...
        loadChildren: () => import("./chunk-MWWGFRJ7.js").then((m) => m.VendorsModule)
      },
      {
        ...
        loadChildren: () => import("./chunk-N4RPN6ZC.js").then((m) => m.DistributorsModule)
      },
      {
        ...
        loadChildren: () => import("./chunk-EVEUGIY7.js").then((m) => m.ClientsModule)
      }
    ]
  }
];
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(AssetCreateWizardComponent, { className: "AssetCreateWizardComponent", filePath: "src/app/features/administration/components/assets/asset-create-wizard/asset-create-wizard.component.ts", lineNumber: 72 });
})();
...
var _c019 = ["ioTypeDetailsStep"];
var _c117 = ["confirmationTemplate"];
var _c214 = ["ioTypeDetailsForm"];
...
var _c020 = ["overspeedProfileDetailsStep"];
var _c118 = ["confirmationTemplate"];
var _c215 = ["overspeedProfileDetailsForm"];
...
var _c021 = ["smsGatewayProviderDetailsStep"];
var _c119 = ["smsGatewayProviderParametersStep"];
var _c216 = ["confirmationTemplate"];
...
var _c022 = ["deviceConfigProfileDetailsStep"];
var _c120 = ["confirmationTemplate"];
var _c217 = ["deviceConfigProfileDetailsForm"];
...
var _c023 = ["customFieldDetailsStep"];
var _c121 = ["confirmationTemplate"];
var _c218 = ["customFieldDetailsForm"];
...
var _c024 = ["userRoleDetailsStep"];
var _c122 = ["confirmationTemplate"];
var _c219 = ["userRoleDetailsForm"];
...
var _c025 = ["userDetailsStep"];
var _c123 = ["userRolesStep"];
var _c220 = ["confirmationTemplate"];
...
var _c026 = ["clientDetailsStep"];
var _c124 = ["confirmationTemplate"];
var _c221 = ["clientDetailsForm"];
...
var _c027 = ["distributorDetailsStep"];
var _c125 = ["confirmationTemplate"];
var _c222 = ["distributorDetailsForm"];
...
var _c028 = ["assetCategoryDetailsStep"];
var _c126 = ["confirmationTemplate"];
var _c223 = ["assetCategoryDetailsForm"];
...
var _c029 = ["assetGroupDetailsStep"];
var _c127 = ["confirmationTemplate"];
var _c224 = ["assetGroupDetailsForm"];
...
var _c030 = ["costCentreDetailsStep"];
var _c128 = ["confirmationTemplate"];
var _c225 = ["costCentreDetailsForm"];
...
var _c031 = ["geofenceGroupDetailsStep"];
var _c129 = ["confirmationTemplate"];
var _c226 = ["geofenceGroupDetailsForm"];
...
//# sourceMappingURL=chunk-3U7FOZCV.js.map
