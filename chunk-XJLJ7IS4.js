...
import {
  AppCardComponent
} from "./chunk-W4I5SSJX.js";
import "./chunk-4WNAVHK2.js";
import {
  AppStoreService
...
var MyInstalledAppsComponent = class _MyInstalledAppsComponent {
  ...
  installedApps = toSignal(toObservable(this.refreshTrigger).pipe(tap(() => this.isLoading.set(true)), switchMap(() => {
    if (isNullOrUndefined(this.currentClientId())) {
      this.isLoading.set(false);
      return of([]);
    }
    return this.appInstallationService.getInstallations({ owner: this.currentClientId() }).pipe(switchMap((installations) => {
      ...
      const appRequests = deduplicatedInstallations.map((installation) => this.appStoreService.getAppById(installation.appId, this.currentClientId() ?? "").pipe(catchError((err) => {
  ...
  uninstallApp(installationId) {
    if (isNullOrUndefined(this.currentClientId()))
      return;
    this.appInstallationService.uninstallApp(installationId, this.currentClientId() ?? "").pipe(tap(() => this.refresh()), catchError((err) => {
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(MyInstalledAppsComponent, { className: "MyInstalledAppsComponent", filePath: "src/app/features/app-store/pages/my-installed-apps/my-installed-apps.component.ts", lineNumber: 39 });
})();
export {
  MyInstalledAppsComponent
};
//# sourceMappingURL=chunk-XJLJ7IS4.js.map
