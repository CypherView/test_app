...
var DetailsUrlService = class _DetailsUrlService {
  ...
  setupBroadcastListener() {
    this.broadcastService.on("grid-item-selected").pipe(takeUntilDestroyed(this.destroyRef)).subscribe((data) => {
      this.openPanelThenLoad(data.item, data.componentType);
      this.urlManager.updateUrlForDetails(data.item).catch(() => {
      });
    });
    this.broadcastService.on("grid-items-selected").pipe(takeUntilDestroyed(this.destroyRef)).subscribe((data) => {
      this.stateService.openBulkDetailsPanel(data.items, data.componentType);
    });
    this.broadcastService.on("tree-items-selected").pipe(takeUntilDestroyed(this.destroyRef)).subscribe((data) => {
      this.stateService.openBulkDetailsPanel(data.items, data.componentType);
    });
  ...
  /**
   * Opens the details panel immediately with the partial list/tree row, then
   * fetches the full entity in the background.
   *
   * Opening up front (rather than waiting for the fetch) gives instant feedback:
   * the panel appears with the row's known fields, and because `selectedItem` is
   * now set, `isSoftRefreshing` becomes true — the panel shows an overlay spinner
   * over the partial content instead of a blank/late panel. `handleLoadSuccess`
   * then re-opens the panel with the complete entity when the fetch resolves.
   */
  openPanelThenLoad(item, componentType) {
    if (isDefined(item.id) && isDefined(item.entityType)) {
      this.stateService.openDetailsPanel(item, componentType);
      this.stateService.isLoading.set(true);
      this.loaderService.loadEntityDetails(String(item.entityType), String(item.id), item);
    } else {
      this.stateService.openDetailsPanel(item, componentType ?? GenericDetailsContainerComponent);
    }
  }
  /**
...
//# sourceMappingURL=chunk-A5X3UN4J.js.map
